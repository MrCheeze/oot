...
u32 Effect_Ss_En_Ice_ct(PlayState* play, u32 index, EffectSs* this, void* initParamsx);
void Effect_EN_Ice_disp_mode(PlayState* play, u32 index, EffectSs* this);
static void Effect_SS_Ice_Piece_func_proc(PlayState* play, u32 index, EffectSs* this);
void Effect_EN_Ice_func_proc(PlayState* play, u32 index, EffectSs* this);
...
u32 Effect_Ss_En_Ice_ct(PlayState* play, u32 index, EffectSs* this, void* initParamsx) {
    ...
    if (initParams->type == 0) {
        ...
        this->draw = Effect_EN_Ice_disp_mode;
        this->update = Effect_EN_Ice_func_proc;
        ...
        this->draw = Effect_EN_Ice_disp_mode;
        this->update = Effect_SS_Ice_Piece_func_proc;
        this->rLifespan = initParams->life;
        this->rScale = initParams->scale * 100.0f;
        this->rYaw = atans_table(initParams->velocity.z, initParams->velocity.x);
        this->rPitch = 0;
        this->rPrimColorR = initParams->primColor.r;
        this->rPrimColorG = initParams->primColor.g;
        this->rPrimColorB = initParams->primColor.b;
...
void Effect_EN_Ice_disp_mode(PlayState* play, u32 index, EffectSs* this) {
    ...
    HiliteReflect_xlu_init(&this->pos, &play->view.eye, &hiliteLightDir, play->state.gfxCtx);
    gSPSegment(POLY_XLU_DISP++, 0x08,
               two_tex_scroll(play->state.gfxCtx, G_TX_RENDERTILE, 0, gameplayFrames & 0xFF, 0x20, 0x10, 1, 0,
...
void Effect_EN_Ice_func_proc(PlayState* play, u32 index, EffectSs* this) {
    ...
    if ((this->actor != NULL) && (this->actor->update != NULL)) {
        ...
        if ((this->life >= 9) && (this->actor->colorFilterTimer != 0) && !(this->actor->colorFilterParams & 0xC000)) {
            ...
            this->accel.x = sin_s(search_position_angleY(&this->actor->world.pos, &this->pos)) * (fqrand() + 1.0f);
            this->accel.z = cos_s(search_position_angleY(&this->actor->world.pos, &this->pos)) * (fqrand() + 1.0f);
            this->accel.y = -1.5f;
        ...
        if (this->life >= 9) {
            s16 rand = rnd_fx(65535.0f);

            this->accel.x = sin_s(rand) * (fqrand() + 1.0f);
            this->accel.z = cos_s(rand) * (fqrand() + 1.0f);
            this->life = 8;
            this->accel.y = -1.5f;
            this->velocity.y = 5.0f;
        }
