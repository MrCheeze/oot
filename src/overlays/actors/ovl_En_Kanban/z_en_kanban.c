...
void En_Kanban_Actor_ct(Actor* thisx, PlayState* play);
void En_Kanban_Actor_dt(Actor* thisx, PlayState* play);
void En_Kanban_Actor_move(Actor* thisx, PlayState* play2);
void En_Kanban_Actor_draw(Actor* thisx, PlayState* play);
...
ActorProfile En_Kanban_Profile = {
    ...
    /**/ En_Kanban_Actor_ct,
    /**/ En_Kanban_Actor_dt,
    /**/ En_Kanban_Actor_move,
    /**/ En_Kanban_Actor_draw,
};

static ColliderCylinderInit KanbanPipeInfoData = {
    {
        COL_MATERIAL_NONE,
        AT_ON | AT_TYPE_ENEMY,
...
static u8 cut_type_data[PLAYER_MWA_MAX] = {
...
static void ground_angle_calc(EnKanban* this) {
    f32 nx;
    f32 ny;
    f32 nz;
    ...
    if (this->actor.floorPoly != NULL) {
        ...
        this->floorRot.x = -fatan2(-nz * ny, 1.0f);
        this->floorRot.z = fatan2(-nx * ny, 1.0f);
    }
}
...
void En_Kanban_Actor_ct(Actor* thisx, PlayState* play) {
    ...
    if (this->actor.params != ENKANBAN_PIECE) {
        ...
        ClObjPipe_set5(play, &this->collider, &this->actor, &KanbanPipeInfoData);
        PRINTF("KANBAN ARG    %x\n", this->actor.params);
        ...
        Actor_BGcheck2(play, &this->actor, 10.0f, 10.0f, 50.0f, UPDBGCHECKINFO_FLAG_2);
...
void kn_message_check(EnKanban* this, PlayState* play) {
    if (!this->msgFlag) {
        if (this->msgTimer == 0) {
            if (ABS((s16)(this->actor.yawTowardsPlayer - this->actor.shape.rot.y)) < 0x2800) {
                if (Actor_talk_check(&this->actor, play)) {
                    ...
                    Actor_talk_request2(&this->actor, play, 68.0f);
                }
            }
        } else {
            this->msgTimer--;
    ...
    switch (this->actionState) {
        case ENKANBAN_SIGN:
            ...
            if (this->partFlags == 0xFFFF) {
                kn_message_check(this, play);
            }
            if ((this->invincibilityTimer == 0) && (this->collider.base.acFlags & AC_HIT)) {
                this->collider.base.acFlags &= ~AC_HIT;
                this->invincibilityTimer = 6;
                piece = (EnKanban*)Actor_info_make_child_actor(&play->actorCtx, &this->actor, play, ACTOR_EN_KANBAN,
                ...
                if (piece != NULL) {
                    ...
                    if (acHitElem->atDmgInfo.dmgFlags & DMG_SLASH) {
                        this->cutType = cut_type_data[player->meleeWeaponAnimation];
                    } else {
                    ...
                    piece->partFlags = cut_paturn_bit[this->cutType] & this->partFlags;
                    if (piece->partFlags == 0) {
                        Actor_delete(&piece->actor);
                        return;
                    }
                    piece->partCount = 0;
                    for (i = 0; i < ARRAY_COUNT(kanban_check_bit); i++) {
                        if (kanban_check_bit[i] & piece->partFlags) {
                            piece->partCount++;
                        }
                    }
                    this->partFlags &= ~cut_paturn_bit[this->cutType];
                    ...
                    Matrix_Position(&cut_offs[piece->pieceType], &offset);
                    piece->actor.world.pos.x += offset.x;
                    piece->actor.world.pos.y += offset.y;
                    piece->actor.world.pos.z += offset.z;
                    piece->offset.x = -cut_offs[piece->pieceType].x / this->actor.scale.x;
                    piece->offset.y = -cut_offs[piece->pieceType].y / this->actor.scale.x;
                    piece->offset.z = -cut_offs[piece->pieceType].z / this->actor.scale.x;
                    piece->pieceWidth = bound_offs[piece->pieceType].x;
                    piece->pieceHeight = bound_offs[piece->pieceType].y;
                    piece->actor.gravity = -1.0f;
                    piece->actionState = ENKANBAN_AIR;
                    piece->actor.world.rot.y = (s16)rnd_fx(0x3000) + this->actor.yawTowardsPlayer + 0x8000;
                    piece->actor.velocity.y = rnd_f(2.0f) + 3.0f;
                    piece->actor.speed = rnd_f(2.0f) + 3.0f;
                    if (piece->partCount >= 4) {
                        piece->bounceX = (s16)rnd_f(10.0f) + 6;
                        piece->bounceZ = (s16)rnd_f(10.0f) + 6;
                    } else {
                        piece->bounceX = (s16)rnd_f(7.0f) + 3;
                        piece->bounceZ = (s16)rnd_f(7.0f) + 3;
                    }
                    piece->spinVel.y = rnd_fx(0x1800);
                    if (fqrand() < 0.5f) {
                        piece->direction = 1;
                    } else {
                        piece->direction = -1;
                    ...
                    this->cutMarkTimer = 5;
                    Actor_SE_set(&this->actor, NA_SE_IT_SWORD_STRIKE);
                }
            }
        ...
        case ENKANBAN_UNUSED: {
            ...
            Actor_BGcheck2(play, &this->actor, 30.0f, 30.0f, 50.0f,
            ...
            if (1) {
                ...
                if (this->actor.bgCheckFlags & BGCHECKFLAG_WALL) {
                    ...
                    Actor_SE_set(&this->actor, NA_SE_EV_WOODPLATE_BOUND);
                }
                if (this->actor.bgCheckFlags & BGCHECKFLAG_WATER_TOUCH) {
                    ...
                    Effect_SS_G_Splash_sc_cl_ct(play, &this->actor.world.pos, NULL, NULL, 0, (this->partCount * 20) + 300);
                    Effect_SS_G_Ripple_ct2(play, &this->actor.world.pos, 150, 650, 0);
                    Effect_SS_G_Ripple_ct2(play, &this->actor.world.pos, 300, 800, 5);
                ...
                if (onGround) {
                    if (this->bounceCount <= 0) {
                        ...
                        this->actor.world.rot.y += (s16)rnd_fx(16384.0f);
                    } else {
                        this->actor.velocity.y = 0.0f;
                    }
                    ...
                    adds(&this->actor.shape.rot.x, this->direction * 0x4000, 1, 0x2000);
                } else {
            ...
            if (bounced) {
                ...
                for (j = 0; j < dustCount + 3; j++) {
                    pos.x = this->actor.world.pos.x + rnd_fx((this->partCount * 0.5f) + 20.0f);
                    pos.z = this->actor.world.pos.z + rnd_fx((this->partCount * 0.5f) + 20.0f);
                    Effect_SS_Dust_sc_li_ct(play, &pos, &velocity, &accel, 100, 5);
                }
            }
        ...
        case ENKANBAN_WATER:
            ...
            add_calc2(&this->actor.shape.yOffset, 100.0f, 1.0f, 5.0f);
            if (this->actionState == ENKANBAN_WATER) {
                ...
                if ((player->actor.speed > 0.0f) && (player->actor.world.pos.y < this->actor.world.pos.y) &&
                    ...
                    add_calc2(&this->actor.speed, player->actor.speed, 1.0f, 0.2f);
                    if (this->actor.speed > 1.0f) {
                        this->actor.speed = 1.0f;
                    }
                    if (add_calc_short_angle2(&this->actor.world.rot.y, this->actor.yawTowardsPlayer + 0x8000, 1, 0x1000,
                                           0) > 0) {
                ...
                if (this->actor.speed != 0.0f) {
                    Actor_BGcheck2(play, &this->actor, 10.0f, 10.0f, 50.0f,
                                            UPDBGCHECKINFO_FLAG_0 | UPDBGCHECKINFO_FLAG_2);
                    if (this->actor.bgCheckFlags & BGCHECKFLAG_WALL) {
                        this->actor.speed *= -0.5f;
                        if (this->spinVel.y > 0) {
                    ...
                    add_calc0(&this->actor.speed, 1.0f, 0.15f);
                }
                this->actor.shape.rot.y += this->spinVel.y;
                adds(&this->spinVel.y, 0, 1, 0x3A);
                adds(&this->actor.shape.rot.x, this->direction * 0x4000, 2, 0x1000);
                adds(&this->spinRot.x, sin_s(2500 * this->frameCount) * 500.0f, 2, 0x1000);
                adds(&this->spinRot.z, cos_s(3000 * this->frameCount) * 500.0f, 2, 0x1000);
                add_calc0(&this->floorRot.x, 0.5f, 0.2f);
                add_calc0(&this->floorRot.z, 0.5f, 0.2f);
                ...
                if (!(this->frameCount & rippleDelay)) {
                    ...
                    Effect_SS_G_Ripple_ct2(play, &this->actor.world.pos, rippleScale, rippleScale + 500, 0);
                }
                ...
                this->actor.world.rot.y = rnd_fx(0x10000);
                if (this->partCount >= 4) {
                    this->bounceX = (s16)rnd_f(10.0f) + 6;
                    this->bounceZ = (s16)rnd_f(10.0f) + 6;
                    this->actor.velocity.y = 2.0f + hammerStrength;
                    this->actor.speed = rnd_f(1.0f);
                } else {
                    this->bounceX = (s16)rnd_f(7.0f) + 3;
                    this->bounceZ = (s16)rnd_f(7.0f) + 3;
                    this->actor.velocity.y = 3.0f + hammerStrength;
                    this->actor.speed = rnd_f(1.5f);
                }
                this->spinVel.y = rnd_fx(0x1800);
                if (fqrand() < 0.5f) {
                    this->direction = 1;
                } else {
                    this->direction = -1;
            ...
            if (this->bounceX == 0) {
                ...
                while (bomb != NULL) {
                    ...
                    if (sqrtf(SQ(dx) + SQ(dy) + SQ(dz)) < 100.0f) {
                        ...
                        this->actionState = ENKANBAN_AIR;
                        this->actor.gravity = -1.0f;
                        this->actor.world.rot.y = RAD_TO_BINANG(fatan2(dx, dz));
                        if (this->partCount >= 4) {
                            this->bounceX = (s16)rnd_f(10.0f) + 6;
                            this->bounceZ = (s16)rnd_f(10.0f) + 6;
        ...
        case ENKANBAN_REPAIR: {
            ...
            Matrix_Position(&cut_offs[this->pieceType], &offset);
            distX =
                add_calc(&this->actor.world.pos.x, signpost->actor.world.pos.x + offset.x, 1.0f, 3.0f, 0.0f);
            distY =
                add_calc(&this->actor.world.pos.y, signpost->actor.world.pos.y + offset.y, 1.0f, 3.0f, 0.0f);
            distZ =
                add_calc(&this->actor.world.pos.z, signpost->actor.world.pos.z + offset.z, 1.0f, 3.0f, 0.0f);
            pDiff = add_calc_short_angle2(&this->actor.shape.rot.x, signpost->actor.shape.rot.x, 1, 0x200, 0);
            yDiff = add_calc_short_angle2(&this->actor.shape.rot.y, signpost->actor.shape.rot.y, 1, 0x200, 0);
            rDiff = add_calc_short_angle2(&this->actor.shape.rot.z, signpost->actor.shape.rot.z, 1, 0x200, 0);
            adds(&this->spinRot.x, 0, 1, 0x200);
            adds(&this->spinRot.z, 0, 1, 0x200);
            add_calc0(&this->floorRot.x, 1.0f, 0.05f);
            add_calc0(&this->floorRot.z, 1.0f, 0.05f);
            add_calc0(&this->actor.shape.yOffset, 1.0f, 2.0f);
...
void En_Kanban_Actor_draw(Actor* thisx, PlayState* play) {
    ...
    if (this->actionState != ENKANBAN_SIGN) {
        ...
        zShift = fabsf(sin_s(this->spinRot.x) * this->pieceHeight);
        zShift2 = fabsf(sin_s(this->spinRot.z) * this->pieceWidth);
        ...
        for (i = 0; i < ARRAY_COUNT(kanban_check_bit); i++) {
            if (kanban_check_bit[i] & this->partFlags) {
                gSPDisplayList(POLY_OPA_DISP++, kanban_break_name[i]);
            }
        }
    } else {
        ...
        if (this->cutMarkAlpha != 0) {
            ...
            Matrix_rotateZ(cut_eff_angle[this->cutType], MTXMODE_APPLY);
    ...
    if ((this->actor.projectedPos.z <= 400.0f) && (this->actor.projectedPos.z > 0.0f) &&
        ...
        if ((this->bounceX != 0) || (this->bounceZ != 0)) {
            u16 dayTime = z_common_data.save.dayTime;
            f32 shadowAlpha;
            ...
            for (i = 0; i < 0x400; i++) {
                if (kanban_shadow_txt_data[i] & this->partFlags) {
                    shadowTex[i] = 0xFF;
                } else {
                    shadowTex[i] = 0;
                }
            ...
            gSPDisplayList(POLY_XLU_DISP++, kanban_shadow_model);
        }
    }
