...
void Arrow_Light_Actor_ct(Actor* thisx, PlayState* play);
void Arrow_Light_Actor_dt(Actor* thisx, PlayState* play);
void Arrow_Light_Actor_move(Actor* thisx, PlayState* play);
void Arrow_Light_Actor_draw(Actor* thisx, PlayState* play);

static void move_wait(ArrowLight* this, PlayState* play);
static void move_start(ArrowLight* this, PlayState* play);
static void move_stick(ArrowLight* this, PlayState* play);

#include "assets/overlays/ovl_Arrow_Light/z_arrow_light.c"
...
ActorProfile Arrow_Light_Profile = {
    ...
    /**/ sizeof(ArrowLight),
    /**/ Arrow_Light_Actor_ct,
    /**/ Arrow_Light_Actor_dt,
    /**/ Arrow_Light_Actor_move,
    /**/ Arrow_Light_Actor_draw,
};
...
void Arrow_Light_actor_set_process(ArrowLight* this, ArrowLightActionFunc actionFunc) {
...
void Arrow_Light_Actor_ct(Actor* thisx, PlayState* play) {
    ...
    this->radius = 0;
    this->unk_160 = 1.0f;
    Arrow_Light_actor_set_process(this, move_wait);
...
static void move_wait(ArrowLight* this, PlayState* play) {
    ...
    Actor_level_SE_set(&this->actor, NA_SE_PL_ARROW_CHARGE_LIGHT - SFX_FLAG);

    // if arrow has no parent, player has fired the arrow
    if (arrow->actor.parent == NULL) {
        this->unkPos = this->actor.world.pos;
        this->radius = 10;
        Arrow_Light_actor_set_process(this, move_start);
        this->alpha = 255;
    }
}

static void add_calc_xyz_local(Vec3f* unkPos, Vec3f* lightPos, f32 scale) {
...
static void move_start(ArrowLight* this, PlayState* play) {
    ...
    this->actor.world.pos = arrow->actor.world.pos;
    this->actor.shape.rot = arrow->actor.shape.rot;
    distanceScaled = search_position_distance(&this->unkPos, &this->actor.world.pos) * (1.0f / 24.0f);
    this->unk_160 = distanceScaled;
    if (distanceScaled < 1.0f) {
        this->unk_160 = 1.0f;
    }
    add_calc_xyz_local(&this->unkPos, &this->actor.world.pos, 0.05f);

    if (arrow->hitFlags & 1) {
        Actor_SE_set(&this->actor, NA_SE_IT_EXPLOSION_LIGHT);
        Arrow_Light_actor_set_process(this, move_stick);
        this->timer = 32;
        this->alpha = 255;
...
void Arrow_Light_Actor_draw(Actor* thisx, PlayState* play) {
    ...
    Matrix_rotateXYZ(0x4000, 0x0, 0x0, MTXMODE_APPLY);
    if (this->timer != 0) {
    ...
    gSPDisplayList(POLY_XLU_DISP++, m_arrow_modelT0);
    gSPDisplayList(POLY_XLU_DISP++,
                   two_tex_scroll(play->state.gfxCtx, G_TX_RENDERTILE, 511 - (stateFrames * 5) % 512, 0, 4, 32, 1,
                                    511 - (stateFrames * 10) % 512, 511 - (stateFrames * 30) % 512, 8, 16));
    gSPDisplayList(POLY_XLU_DISP++, m_arrow_modelT1);
