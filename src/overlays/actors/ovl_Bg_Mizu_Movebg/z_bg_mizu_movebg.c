...
void Bg_Mizu_Movebg_actor_ct(Actor* thisx, PlayState* play);
void Bg_Mizu_Movebg_actor_dt(Actor* thisx, PlayState* play);
void Bg_Mizu_Movebg_actor_move(Actor* thisx, PlayState* play);
void Bg_Mizu_Movebg_actor_draw(Actor* thisx, PlayState* play2);

void mode_floater(BgMizuMovebg* this, PlayState* play);
void mode_path_floater(BgMizuMovebg* this, PlayState* play);
static s32 func_get_rail_pos(Path* pathList, Vec3f* pos, s32 pathId, s32 pointId);
...
ActorProfile Bg_Mizu_Movebg_Profile = {
    ...
    /**/ OBJECT_MIZU_OBJECTS,
    /**/ sizeof(BgMizuMovebg),
    /**/ Bg_Mizu_Movebg_actor_ct,
    /**/ Bg_Mizu_Movebg_actor_dt,
    /**/ Bg_Mizu_Movebg_actor_move,
    /**/ Bg_Mizu_Movebg_actor_draw,
};

static f32 mato00dr_00_offset[] = { -115.200005f, -115.200005f, -115.200005f, 0.0f };
...
static u8 LG_snd_flag; // unknown flags

static s32 sw_check(PlayState* play) {
    s32 result;

    if (Actor_Environment_sw_Check(play, WATER_TEMPLE_WATER_F1_FLAG)) {
        result = 1;
    } else if (Actor_Environment_sw_Check(play, WATER_TEMPLE_WATER_F2_FLAG)) {
        result = 2;
    } else if (Actor_Environment_sw_Check(play, WATER_TEMPLE_WATER_F3_FLAG)) {
        result = 3;
    } else {
        result = 1;
    }
...
void Bg_Mizu_Movebg_actor_ct(Actor* thisx, PlayState* play) {
    static Vec3f mtx_src = { 0.0f, 80.0f, 23.0f };
    ...
    ((BgMizuMovebg*)thisx)->dList = shape_model[MOVEBG_TYPE(thisx->params)];
    MoveBG_ct(&((BgMizuMovebg*)thisx)->dyna, DYNA_TRANSFORM_POS);
    DynaPolyUty_bgdi_SG2KSG(pbgdata_from_data[MOVEBG_TYPE(thisx->params)], &colHeader);
    ((BgMizuMovebg*)thisx)->dyna.bgId = DynaPolyInfo_setActor(play, &play->colCtx.dyna, thisx, colHeader);
    ...
    switch (type) {
        case MIZUMOVEBG_TYPE_FLOATING_PLATFORM_OUTSIDE_CENTER_PILLAR:
            ...
            ((BgMizuMovebg*)thisx)->actionFunc = mode_floater;
            break;
        ...
        case MIZUMOVEBG_TYPE_DRAGON_STATUE_BOSS_ROOM:
            thisx->world.pos.y = ((BgMizuMovebg*)thisx)->homeY +
                                 mato00dr_00_offset[sw_check(play)];
            ((BgMizuMovebg*)thisx)->actionFunc = mode_floater;
            break;
        ...
        case MIZUMOVEBG_TYPE_DRAGON_STATUE_4:
            if (Actor_Environment_sw_Check(play, MOVEBG_FLAGS(thisx->params))) {
        ...
        case MIZUMOVEBG_TYPE_HOOKSHOT_PLATFORM:
            ...
            func_get_rail_pos(play->pathList, &thisx->world.pos, MOVEBG_PATH_ID(thisx->params), waypointId);
            ((BgMizuMovebg*)thisx)->actionFunc = mode_path_floater;
            break;
    }
    ...
    switch (type) {
        ...
        case MIZUMOVEBG_TYPE_DRAGON_STATUE_4:
            ...
            Matrix_Position(&mtx_src, &offsetPos);

            if (Actor_info_make_child_actor(&play->actorCtx, thisx, play, ACTOR_OBJ_HSBLOCK, thisx->world.pos.x + offsetPos.x,
...
static void func_two_tile_control(BgMizuMovebg* this, PlayState* play) {
...
void mode_floater(BgMizuMovebg* this, PlayState* play) {
    ...
    switch (type) {
        ...
        case MIZUMOVEBG_TYPE_DRAGON_STATUE_BOSS_ROOM:
            targetPosY =
                this->homeY + mato00dr_00_offset[sw_check(play)];
            if (!chase_f(&this->dyna.actor.world.pos.y, targetPosY, 1.0f)) {
                if (!(LG_snd_flag & 2) && MOVEBG_SPEED(this->dyna.actor.params) != 0) {
                    LG_snd_flag |= 2;
                    this->sfxFlags |= 2;
                }
                if (this->sfxFlags & 2) {
        ...
        case MIZUMOVEBG_TYPE_DRAGON_STATUE_3:
        case MIZUMOVEBG_TYPE_DRAGON_STATUE_4:
            if (Actor_Environment_sw_Check(play, MOVEBG_FLAGS(this->dyna.actor.params))) {
...
void mode_path_floater(BgMizuMovebg* this, PlayState* play) {
    ...
    func_get_rail_pos(play->pathList, &waypoint, MOVEBG_PATH_ID(this->dyna.actor.params), this->waypointId);
    dist = Actor_search_position_distance(&this->dyna.actor, &waypoint);
    if (dist < this->dyna.actor.speed) {
        this->dyna.actor.speed = dist;
    }
    search_position_angleXY(&this->dyna.actor.world.pos, &waypoint, &this->dyna.actor.world.rot, 1);
    Actor_position_moveF_XY(&this->dyna.actor);
    ...
    if (fabsf(dx) < 2.0f && fabsf(dy) < 2.0f && fabsf(dz) < 2.0f) {
        ...
        if (this->waypointId >= play->pathList[MOVEBG_PATH_ID(this->dyna.actor.params)].count) {
            this->waypointId = 0;
            func_get_rail_pos(play->pathList, &this->dyna.actor.world.pos,
                                        MOVEBG_PATH_ID(this->dyna.actor.params), 0);
        }
    }
    if (!(LG_snd_flag & 1) && MOVEBG_SPEED(this->dyna.actor.params) != 0) {
        LG_snd_flag |= 1;
        this->sfxFlags |= 1;
    }
    if (this->sfxFlags & 1) {
...
void Bg_Mizu_Movebg_actor_draw(Actor* thisx, PlayState* play2) {
    ...
    gSPSegment(POLY_OPA_DISP++, 0x08,
               two_tex_scroll_env(play->state.gfxCtx, G_TX_RENDERTILE, frames * 1, 0, 32, 32, 1, 0, 0, 32, 32, 0,
    ...
    gSPSegment(POLY_OPA_DISP++, 0x0B,
               two_tex_scroll_env(play->state.gfxCtx, G_TX_RENDERTILE, frames * 3, 0, 32, 32, 1, 0, 0, 32, 32, 0,
