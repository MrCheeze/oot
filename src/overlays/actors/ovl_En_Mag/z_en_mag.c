...
void En_Mag_Actor_ct(Actor* thisx, PlayState* play);
void En_Mag_Actor_dt(Actor* thisx, PlayState* play);
void En_Mag_Actor_move(Actor* thisx, PlayState* play);
void En_Mag_Actor_draw(Actor* thisx, PlayState* play);
...
void sram_manual_initialize(void) {
    static u8 read_buff[0x2000];

    PRINTF(T("\n\n\n＝＝＝＝＝＝＝＝＝＝\n  ＳＲＡＭ初期化\n\n＝＝＝＝＝＝＝＝＝＝\n\n\n",
             "\n\n\n==========\n  SRAM initialization\n\n==========\n\n\n"));

    bzero(read_buff, 0x800);
    ssSRAMReadWrite(OS_K1_TO_PHYSICAL(0xA8000000), read_buff, 0x800, 1);
    ssSRAMReadWrite(OS_K1_TO_PHYSICAL(0xA8000800), read_buff, 0x800, 1);
    ssSRAMReadWrite(OS_K1_TO_PHYSICAL(0xA8001000), read_buff, 0x800, 1);
    ssSRAMReadWrite(OS_K1_TO_PHYSICAL(0xA8001800), read_buff, 0x800, 1);
    ssSRAMReadWrite(OS_K1_TO_PHYSICAL(0xA8002000), read_buff, 0x800, 1);
    ssSRAMReadWrite(OS_K1_TO_PHYSICAL(0xA8002800), read_buff, 0x800, 1);
    ssSRAMReadWrite(OS_K1_TO_PHYSICAL(0xA8003000), read_buff, 0x800, 1);
    ssSRAMReadWrite(OS_K1_TO_PHYSICAL(0xA8003800), read_buff, 0x800, 1);
    ssSRAMReadWrite(OS_K1_TO_PHYSICAL(0xA8004000), read_buff, 0x800, 1);
    ssSRAMReadWrite(OS_K1_TO_PHYSICAL(0xA8004800), read_buff, 0x800, 1);
    ssSRAMReadWrite(OS_K1_TO_PHYSICAL(0xA8005000), read_buff, 0x800, 1);
    ssSRAMReadWrite(OS_K1_TO_PHYSICAL(0xA8005800), read_buff, 0x800, 1);
    ssSRAMReadWrite(OS_K1_TO_PHYSICAL(0xA8006000), read_buff, 0x800, 1);
    ssSRAMReadWrite(OS_K1_TO_PHYSICAL(0xA8006800), read_buff, 0x800, 1);
    ssSRAMReadWrite(OS_K1_TO_PHYSICAL(0xA8007000), read_buff, 0x800, 1);
    ssSRAMReadWrite(OS_K1_TO_PHYSICAL(0xA8007800), read_buff, 0x800, 1);

    z_common_data.audioSetting = z_common_data.zTargetSetting = 0;
...
void En_Mag_Actor_ct(Actor* thisx, PlayState* play) {
    ...
    if (z_common_data.forceRisingButtonAlphas) {
        ...
        check_time = 20;
        z_common_data.transFadeDuration = 1;
        z_common_data.transWipeSpeed = 255;
    }
...
#if OOT_VERSION < GC_US || PLATFORM_IQUE || OOT_VERSION == HIRATSU3
void Sram_manual_reset(PlayState* play, EnMag* this) {
    static s32 key_input_data[] = {
    ...
    if (this->unk_E316 < 4) {
        if (var_v1 & key_input_data[this->unk_E316]) {
            this->unk_E316++;
        } else if (var_v1 != 0) {
            this->unk_E316 = 0;
        }
    } else {
        if (CHECK_BTN_ALL(input->press.button, key_input_data[this->unk_E316])) {
            this->unk_E316++;
        } else if (var_v1 != 0) {
            this->unk_E316 = 0;
        }
    ...
    if (this->unk_E316 == ARRAY_COUNT(key_input_data)) {
...
void En_Mag_Actor_move(Actor* thisx, PlayState* play) {
...
#if OOT_VERSION < GC_US || PLATFORM_IQUE || OOT_VERSION == HIRATSU3
    Sram_manual_reset(play, this);
#endif

    if (z_common_data.fileNum != 0xFEDC) {
            ...
            if (check_time == 0) {
                if (CHECK_BTN_ALL(input->press.button, BTN_START) || CHECK_BTN_ALL(input->press.button, BTN_A) ||
                    ...
                    if (play->transitionTrigger != TRANS_TRIGGER_START) {
                        ...
                        z_common_data.gameMode = GAMEMODE_FILE_SELECT;
    ...
    if (this->globalState == MAG_STATE_INITIAL) {
        if (eventbit_check(play, 3)) {
            this->effectFadeInTimer = 40;
            this->globalState = MAG_STATE_FADE_IN;
        }
    } else if (this->globalState == MAG_STATE_DISPLAY) {
        if (eventbit_check(play, 4)) {
            this->globalState = MAG_STATE_FADE_OUT;
        }
    }
}

static BAD_RETURN(s32) texture_rectangleI8(Gfx** gfxP, void* texture, s16 texWidth, s16 texHeight, s16 rectLeft, s16 rectTop,
...
BAD_RETURN(s32) texture_multi_rectangle(Gfx** gfxP, void* maskTex, void* effectTex, s16 maskWidth, s16 maskHeight,
...
void display_rgba32b(Gfx** gfxP, s16 centerX, s16 centerY, u8* source, u32 width, u32 height) {
    Gfx* gfx = *gfxP;
    u8* curTexture;
    s32 textureCount;
    u32 rectLeft;
...
void moji_display(Gfx** gfxP, u8* texture, s32 rectLeft, s32 rectTop) {
    Gfx* gfx = *gfxP;
...
void En_Mag_Actor_draw_Sub(Actor* thisx, PlayState* play, Gfx** gfxP) {
    ...
    static void* g_title_efc_txt_data[] = {
    ...
    if ((s16)this->effectPrimLodFrac != 0) {
        for (k = 0, i = 0, rectTop = 0; i < 3; i++) {
            for (j = 0, rectLeft = 64 + LOGO_X_SHIFT; j < 3; j++, k++, rectLeft += 64) {
                texture_multi_rectangle(&gfx, g_title_efc_txt_data[k], gTitleFlameEffectTex, 64, 64, 32, 32, rectLeft,
    ...
    if ((s16)this->mainAlpha != 0) {
        display_rgba32b(&gfx, 160 + LOGO_X_SHIFT, 100, (u8*)gTitleZeldaShieldLogoTex, 160, 160);
#if PLATFORM_N64
    ...
    if ((s16)this->mainAlpha != 0) {
        texture_rectangleI8(&gfx, gTitleTheLegendOfTextTex, 72, 8, 154 + LOGO_X_SHIFT, 73, 72, 8, 1024, 1024);
        texture_rectangleI8(&gfx, gTitleOcarinaOfTimeTMTextTex, 96, 8, 152 + LOGO_X_SHIFT, 127, 96, 8, 1024, 1024);
        ...
        texture_rectangleI8(&gfx, gTitleTheLegendOfTextTex, 72, 8, 153 + LOGO_X_SHIFT, 72, 72, 8, 1024, 1024);
        texture_rectangleI8(&gfx, gTitleOcarinaOfTimeTMTextTex, 96, 8, 151 + LOGO_X_SHIFT, 126, 96, 8, 1024, 1024);

#if OOT_MQ
        gDPPipeSync(gfx++);
        gDPSetPrimColor(gfx++, 0, 0, 255, 255, 255, (s16)this->subAlpha);
#if OOT_VERSION == GC_JP_MQ
        display_rgba32b(&gfx, 235, 149, (u8*)gTitleUraLogoTex, 40, 40);
#elif OOT_VERSION == GC_US_MQ
        if (z_common_data.language == LANGUAGE_JPN) {
            display_rgba32b(&gfx, 235, 149, (u8*)gTitleUraLogoTex, 40, 40);
        } else {
            display_rgba32b(&gfx, 174, 145, (u8*)gTitleMasterQuestSubtitleTex, 128, 32);
        }
#else
        display_rgba32b(&gfx, 174, 145, (u8*)gTitleMasterQuestSubtitleTex, 128, 32);
#endif
    ...
    if (z_common_data.fileNum == 0xFEDC) {
        ...
        gDPSetCombineLERP(gfx++, 0, 0, 0, PRIMITIVE, TEXEL0, 0, PRIMITIVE, 0, 0, 0, 0, PRIMITIVE, TEXEL0, 0, PRIMITIVE,
                          0);
        gDPSetPrimColor(gfx++, 0, 0, 0, 0, 0, no_alpha);

        rectLeft = VREG(19) + 1;
        for (i = 0; i < ARRAY_COUNT(no_controller); i++) {
            moji_display(&gfx, font->fontBuf + no_controller[i] * FONT_CHAR_TEX_SIZE, rectLeft,
                                  YREG(10) + 172);
            rectLeft += VREG(21);
            if (i == 1) {
                rectLeft += VREG(23);
        ...
        // Actual Text
        gDPPipeSync(gfx++);
        gDPSetPrimColor(gfx++, 0, 0, 100, 255, 255, no_alpha);
        ...
        rectLeft = YREG(7) + 1;
        for (i = 0; i < ARRAY_COUNT(press_start); i++) {
            moji_display(&gfx, font->fontBuf + press_start[i] * FONT_CHAR_TEX_SIZE, rectLeft,
                                  YREG(10) + 172);
            rectLeft += YREG(8);
            if (i == 4) {
                rectLeft += YREG(9);
        ...
        // Actual Text
        gDPPipeSync(gfx++);
        gDPSetPrimColor(gfx++, 0, 0, YREG(4), YREG(5), YREG(6), no_alpha);
        ...
        rectLeft += YREG(8) * 2;
        moji_display(&gfx, (u8*)gTitlePressCHN, rectLeft, YREG(10) + 172);
        ...
        rectLeft += YREG(8) * 2;
        moji_display(&gfx, (u8*)gTitlePressCHN, rectLeft, YREG(10) + 171);
        rectLeft += YREG(9) + YREG(8);
...
void En_Mag_Actor_draw(Actor* thisx, PlayState* play) {
    ...
    En_Mag_Actor_draw_Sub(thisx, play, &gfx);
