...
void Arrow_Fire_Actor_ct(Actor* thisx, PlayState* play);
void Arrow_Fire_Actor_dt(Actor* thisx, PlayState* play);
void Arrow_Fire_Actor_move(Actor* thisx, PlayState* play);
void Arrow_Fire_Actor_draw(Actor* thisx, PlayState* play2);

static void move_wait(ArrowFire* this, PlayState* play);
static void move_start(ArrowFire* this, PlayState* play);
static void move_stick(ArrowFire* this, PlayState* play);
...
ActorProfile Arrow_Fire_Profile = {
    ...
    /**/ Arrow_Fire_Actor_ct,
    /**/ Arrow_Fire_Actor_dt,
    /**/ Arrow_Fire_Actor_move,
    /**/ Arrow_Fire_Actor_draw,
};
...
void Arrow_Fire_actor_set_process(ArrowFire* this, ArrowFireActionFunc actionFunc) {
...
void Arrow_Fire_Actor_ct(Actor* thisx, PlayState* play) {
    ...
    this->radius = 0;
    this->unk_158 = 1.0f;
    Arrow_Fire_actor_set_process(this, move_wait);
...
static void move_wait(ArrowFire* this, PlayState* play) {
    ...
    Actor_level_SE_set(&this->actor, NA_SE_PL_ARROW_CHARGE_FIRE - SFX_FLAG);

    // if arrow has no parent, player has fired the arrow
    if (arrow->actor.parent == NULL) {
        this->unkPos = this->actor.world.pos;
        this->radius = 10;
        Arrow_Fire_actor_set_process(this, move_start);
        this->alpha = 255;
    }
}

static void add_calc_xyz_local(Vec3f* unkPos, Vec3f* firePos, f32 scale) {
...
static void move_start(ArrowFire* this, PlayState* play) {
    ...
    if (arrow->hitFlags & 1) {
        Actor_SE_set(&this->actor, NA_SE_IT_EXPLOSION_FRAME);
        Arrow_Fire_actor_set_process(this, move_stick);
        this->timer = 32;
        this->alpha = 255;
...
void Arrow_Fire_Actor_draw(Actor* thisx, PlayState* play2) {
    ...
    gSPDisplayList(POLY_XLU_DISP++,
                   two_tex_scroll(play->state.gfxCtx, G_TX_RENDERTILE, 255 - (stateFrames * 2) % 256, 0, 64, 32, 1,
