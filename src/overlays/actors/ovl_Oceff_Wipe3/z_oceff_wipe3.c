...
void Oceff_Wipe3_Actor_ct(Actor* thisx, PlayState* play);
void Oceff_Wipe3_Actor_dt(Actor* thisx, PlayState* play);
void Oceff_Wipe3_Actor_move(Actor* thisx, PlayState* play);
void Oceff_Wipe3_Actor_draw(Actor* thisx, PlayState* play);
...
ActorProfile Oceff_Wipe3_Profile = {
    ...
    /**/ sizeof(OceffWipe3),
    /**/ Oceff_Wipe3_Actor_ct,
    /**/ Oceff_Wipe3_Actor_dt,
    /**/ Oceff_Wipe3_Actor_move,
    /**/ Oceff_Wipe3_Actor_draw,
};
...
void Oceff_Wipe3_Actor_draw(Actor* thisx, PlayState* play) {
    ...
    if (this->counter < 32) {
        z = sin_s(this->counter << 9) * 1330;
    } else {
        z = 1330;
    }

    vtxPtr = efc_ocarina_42_v;
    if (this->counter >= 80) {
    ...
    gSPDisplayList(POLY_XLU_DISP++, efc_ocarina_42_modelT);
    gSPDisplayList(POLY_XLU_DISP++, two_tex_scroll(play->state.gfxCtx, G_TX_RENDERTILE, scroll * 12, scroll * (-12),
                                                     64, 64, 1, scroll * 8, scroll * (-8), 64, 64));
    gSPDisplayList(POLY_XLU_DISP++, efc_ocarina_42_modelT2);
