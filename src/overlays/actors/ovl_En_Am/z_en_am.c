...
void En_Am_actor_ct(Actor* thisx, PlayState* play);
void En_Am_actor_dt(Actor* thisx, PlayState* play);
void En_Am_actor_move(Actor* thisx, PlayState* play);
void En_Am_actor_draw(Actor* thisx, PlayState* play);

static void mode_wait2_init(EnAm* this);
static void mode_wait_init(EnAm* this);
static void mode_wait2(EnAm* this, PlayState* play);
static void mode_wait(EnAm* this, PlayState* play);
static void mode_walk(EnAm* this, PlayState* play);
void mode_go_home_wait(EnAm* this, PlayState* play);
void mode_go_home(EnAm* this, PlayState* play);
void mode_set_home(EnAm* this, PlayState* play);
void set_move_angle(EnAm* this, PlayState* play);
static void mode_hirumu(EnAm* this, PlayState* play);
void En_am_Actor_mode_damage2(EnAm* this, PlayState* play);
static void mode_dead(EnAm* this, PlayState* play);
...
void En_am_actor_set_process(EnAm* this, EnAmActionFunc actionFunc) {
 ...
 */
s32 BG_point_check2(EnAm* this, PlayState* play, f32 distance, s16 yaw) {
    s16 ret;
    s16 curBgCheckFlags;
    f32 sin;
    f32 cos;
    ...
    sin = sin_s(yaw) * distance;
    cos = cos_s(yaw) * distance;
...
void En_Am_actor_ct(Actor* thisx, PlayState* play) {
    ...
    Shape_Info_init(&this->dyna.actor.shape, 0.0f, Actor_shadow_circle, 48.0f);
    Skeleton_Info2_M_ct(play, &this->skelAnime, &gArmosSkel, &gArmosRicochetAnim, this->jointTable, this->morphTable, 14);
    Actor_set_scale(&this->dyna.actor, 0.01f);
    MoveBG_ct(&this->dyna, 0);
    ClObjPipe_ct(play, &this->hurtCollider);
    ClObjPipe_ct(play, &this->blockCollider);
    ClObjPipe_set5(play, &this->hurtCollider, &this->dyna.actor, &OcInfoData);

    if (this->dyna.actor.params == ARMOS_STATUE) {
        this->dyna.actor.colChkInfo.health = 1;
        ClObjPipe_set5(play, &this->blockCollider, &this->dyna.actor, &OcInfoData);
        ...
        DynaPolyUty_bgdi_SG2KSG(&gArmosCol, &colHeader);
        this->dyna.bgId = DynaPolyInfo_setActor(play, &play->colCtx.dyna, &this->dyna.actor, colHeader);
        Actor_info_part_chg(play, &play->actorCtx, &this->dyna.actor, ACTORCAT_BG);
        mode_wait2_init(this);
    } else {
        ClObjPipe_set5(play, &this->blockCollider, &this->dyna.actor, &AcInfoData);
        ClObjSwrd_ct(play, &this->hitCollider);
        ClObjSwrd_set5(play, &this->hitCollider, &this->dyna.actor, &AtInfoData);
        this->dyna.actor.colChkInfo.health = 1;
        this->dyna.actor.colChkInfo.damageTable = &btl_data;
        mode_wait_init(this);
        this->unk_258 = 0;
    }
...
void En_Am_actor_dt(Actor* thisx, PlayState* play) {
    ...
    ClObjPipe_dt(play, &this->hurtCollider);
    ClObjPipe_dt(play, &this->blockCollider);
    //! @bug Quad collider is not destroyed (though destroy doesn't really do anything anyway)
}

static void eff_set(EnAm* this, PlayState* play) {
    ...
    for (i = 4; i > 0; i--) {
        pos.x = this->dyna.actor.world.pos.x + ((fqrand() - 0.5f) * 65.0f);
        pos.y = (this->dyna.actor.world.pos.y + 40.0f) + ((fqrand() - 0.5f) * 10.0f);
        pos.z = this->dyna.actor.world.pos.z + ((fqrand() - 0.5f) * 65.0f);

        Effect_SS_KiraKira_ct(play, &pos, &kirakira_vec, &kirakira_acc, &primColor, &envColor);
    }

    Actor_SE_set(&this->dyna.actor, NA_SE_EN_AMOS_WALK);
    _dust_ground_set(play, &this->dyna.actor, &this->dyna.actor.world.pos, 4.0f, 3, 8.0f, 300, 15, false);
}

static void mode_wait_init(EnAm* this) {
    f32 lastFrame = Si2_anime_end_frame(&gArmosRicochetAnim);

    Skeleton_Info2_init(&this->skelAnime, &gArmosRicochetAnim, 0.0f, lastFrame, lastFrame, ANIMMODE_LOOP, 0.0f);
    this->behavior = AM_BEHAVIOR_DO_NOTHING;
    this->dyna.actor.speed = 0.0f;
    this->unk_258 = (this->textureBlend == 255) ? 0 : 1;
    En_am_actor_set_process(this, mode_wait);
}
...
static void mode_wait2_init(EnAm* this) {
    ...
    this->behavior = AM_BEHAVIOR_DO_NOTHING;
    this->dyna.actor.speed = 0.0f;
    En_am_actor_set_process(this, mode_wait2);
}

static void mode_walk_init(EnAm* this) {
    Skeleton_Info2_init_standard_speedset_repeat(&this->skelAnime, &gArmosHopAnim, 4.0f);
    this->unk_258 = 3;
    this->behavior = AM_BEHAVIOR_AGGRO;
    this->dyna.actor.speed = 0.0f;
    this->dyna.actor.world.rot.y = this->dyna.actor.shape.rot.y;
    En_am_actor_set_process(this, mode_walk);
}

void set_move_angle_init(EnAm* this) {
    Skeleton_Info2_init_standard_speedset_repeat(&this->skelAnime, &gArmosHopAnim, 4.0f);
    this->unk_258 = 3;
    ...
    this->dyna.actor.speed = 0.0f;
    this->dyna.actor.world.rot.y = this->dyna.actor.shape.rot.y;
    En_am_actor_set_process(this, set_move_angle);
}

void mode_go_home_init(EnAm* this) {
    Skeleton_Info2_init_standard_speedset_repeat(&this->skelAnime, &gArmosHopAnim, 4.0f);
    this->behavior = AM_BEHAVIOR_GO_HOME;
    this->unk_258 = 1;
    this->dyna.actor.speed = 0.0f;
    En_am_actor_set_process(this, mode_go_home);
}

void mode_set_home_init(EnAm* this) {
    Skeleton_Info2_init_standard_speedset_repeat(&this->skelAnime, &gArmosHopAnim, 4.0f);
    this->behavior = AM_BEHAVIOR_GO_HOME;
    this->unk_258 = 1;
    this->dyna.actor.speed = 0.0f;
    En_am_actor_set_process(this, mode_set_home);
}

void mode_go_home_wait_init(EnAm* this) {
    Skeleton_Info2_init_standard_speedset_repeat(&this->skelAnime, &gArmosHopAnim, 4.0f);
    this->behavior = AM_BEHAVIOR_GO_HOME;
    this->dyna.actor.speed = 0.0f;
    this->dyna.actor.world.rot.y = this->dyna.actor.shape.rot.y;
    En_am_actor_set_process(this, mode_go_home_wait);
}

static void mode_dead_init(EnAm* this, PlayState* play) {
    Skeleton_Info2_init(&this->skelAnime, &gArmosDamagedAnim, 1.0f, 4.0f,
                     Si2_anime_end_frame(&gArmosDamagedAnim) - 6.0f, ANIMMODE_ONCE, 0.0f);
    this->behavior = AM_BEHAVIOR_DAMAGED;
    this->dyna.actor.world.rot.y = this->dyna.actor.yawTowardsPlayer;
    Actor_SE_set(&this->dyna.actor, NA_SE_EN_AMOS_DAMAGE);

    if (BG_point_check2(this, play, -6.0f, this->dyna.actor.world.rot.y)) {
        this->dyna.actor.speed = -6.0f;
    }

    this->dyna.actor.colorFilterTimer = 0;
    Actor_info_finish(play, &this->dyna.actor);
    En_am_actor_set_process(this, mode_dead);
}

static void mode_hirumu_init(EnAm* this, PlayState* play) {
    Skeleton_Info2_init(&this->skelAnime, &gArmosRicochetAnim, 1.0f, 0.0f, 8.0f, ANIMMODE_ONCE, 0.0f);
    this->dyna.actor.world.rot.y = this->dyna.actor.yawTowardsPlayer;
    ...
    En_am_actor_set_process(this, mode_hirumu);
}
...
static void mode_wait(EnAm* this, PlayState* play) {
    ...
    if ((this->unk_258 != 0) ||
        ...
        if (this->textureBlend == 0) {
            Actor_SE_set(&this->dyna.actor, NA_SE_EN_AMOS_WAVE);
            Actor_SE_set(&this->dyna.actor, NA_SE_EN_AMOS_VOICE);
            Set_Fog(&this->dyna.actor, COLORFILTER_COLORFLAG_RED, 255, COLORFILTER_BUFFLAG_OPA, 8);
        }
        ...
        if (this->textureBlend >= 240) {
            ...
            rand = (fqrand() - 0.5f) * 10.0f;

            cos = cos_s(this->dyna.actor.shape.rot.y) * rand;
            sin = sin_s(this->dyna.actor.shape.rot.y) * rand;
 ...
 */
void mode_go_home_wait(EnAm* this, PlayState* play) {
    s16 yawToHome = search_position_angleY(&this->dyna.actor.world.pos, &this->dyna.actor.home.pos);

    if (this->skelAnime.curFrame == 8.0f) {
        add_calc_short_angle2(&this->dyna.actor.world.rot.y, yawToHome, 1, 0x1F40, 0);
...
void mode_set_home(EnAm* this, PlayState* play) {
    if (this->skelAnime.curFrame == 8.0f) {
        if ((this->dyna.actor.world.pos.x == this->dyna.actor.home.pos.x) &&
            ...
            add_calc_short_angle2(&this->dyna.actor.world.rot.y, this->dyna.actor.home.rot.y, 1, 0x1F40, 0);
        }
        this->unk_258 = 2;
        this->dyna.actor.velocity.y = 12.0f;
    } else if (this->skelAnime.curFrame > 11.0f) {
    ...
    if (this->unk_258 == 2) {
        add_calc(&this->dyna.actor.world.pos.x, this->dyna.actor.home.pos.x, 1.0f, 8.0f, 0.0f);
        add_calc(&this->dyna.actor.world.pos.z, this->dyna.actor.home.pos.z, 1.0f, 8.0f, 0.0f);
    }
...
void mode_go_home(EnAm* this, PlayState* play) {
    ...
    if (this->skelAnime.curFrame == 8.0f) {
        ...
        if (!(this->dyna.actor.bgCheckFlags & BGCHECKFLAG_GROUND)) {
            ...
            add_calc_short_angle2(&this->dyna.actor.world.rot.y, yawToHome, 1, 0xBB8, 0);
            ...
            if (Actor_search_position_distance(&this->dyna.actor, &this->dyna.actor.home.pos) < 80.0f) {
                mode_set_home_init(this);
            }
        }
    }
...
static void mode_dead(EnAm* this, PlayState* play) {
    ...
    if ((this->dyna.actor.velocity.y <= 0.0f) && !BG_point_check2(this, play, -8.0f, this->dyna.actor.world.rot.y)) {
        this->dyna.actor.speed = 0.0f;
    }
...
void set_move_angle(EnAm* this, PlayState* play) {
    ...
    if (this->cooldownTimer != 0) {
        ...
        if (this->skelAnime.curFrame == 8.0f) {
            add_calc_short_angle2(&this->dyna.actor.world.rot.y, this->dyna.actor.yawTowardsPlayer, 1, 0x1F40, 0);
            this->dyna.actor.velocity.y = 12.0f;
        } else if (this->skelAnime.curFrame > 11.0f) {
            if (!(this->dyna.actor.bgCheckFlags & BGCHECKFLAG_GROUND)) {
                this->skelAnime.curFrame = 11;
...
static void mode_walk(EnAm* this, PlayState* play) {
    if (this->deathTimer < 52) {
        if (this->skelAnime.curFrame == 8.0f) {
            ...
            if (BG_point_check2(this, play, 80.0f, this->dyna.actor.world.rot.y)) {
                this->dyna.actor.speed = 6.0f;
            } else {
                this->dyna.actor.speed = 0.0f;
            }
            ...
            if (!(this->dyna.actor.bgCheckFlags & BGCHECKFLAG_GROUND)) {
                ...
                add_calc_short_angle2(&this->dyna.actor.world.rot.y, this->dyna.actor.yawTowardsPlayer, 1, 0x1770, 0);
                ...
                if (((Actor_search_position_distanceXZ(&this->dyna.actor, &this->dyna.actor.home.pos) > 240.0f) ||
                     (this->attackTimer == 0)) &&
                    (this->deathTimer == 0)) {
                    mode_go_home_wait_init(this);
                }
            }
        }
...
static void mode_wait2(EnAm* this, PlayState* play) {
    ...
    if (this->unk_258 == 0) {
        ...
        if (this->hurtCollider.base.ocFlags1 & OC1_HIT) {
            moveDir = search_position_angleY(&this->dyna.actor.world.pos, &this->hurtCollider.base.oc->world.pos) - temp158f;
        }

        if ((this->dyna.unk_150 == 0.0f) || (this->unk_258 == 0) ||
            !(this->dyna.actor.bgCheckFlags & BGCHECKFLAG_GROUND) ||
            !MoveBG_checkMoveArea(play, &this->dyna, 0x14, (sin_s(this->unk_258) * (this->dyna.unk_150 * 0.5f)) + 40.0f,
                           0xA) ||
        ...
        this->dyna.actor.world.rot.y = this->dyna.unk_158;
        this->dyna.actor.speed = sin_s(this->unk_258) * (this->dyna.unk_150 * 0.5f);
    }
...
void En_am_Actor_mode_damage2_init(EnAm* this, PlayState* play) {
    // animation is set but Skeleton_Info2_anime_play is not called in the action
    // likely copy pasted from mode_dead_init
    Skeleton_Info2_init(&this->skelAnime, &gArmosDamagedAnim, 1.0f, 0.0f, Si2_anime_end_frame(&gArmosDamagedAnim),
                     ANIMMODE_ONCE, 0.0f);
    ...
    Set_Fog(&this->dyna.actor, COLORFILTER_COLORFLAG_BLUE, 120, COLORFILTER_BUFFLAG_OPA, 100);
    ...
    En_am_actor_set_process(this, En_am_Actor_mode_damage2);
}

void En_am_Actor_mode_damage2(EnAm* this, PlayState* play) {
    add_calc_short_angle2(&this->dyna.actor.shape.rot.y, this->dyna.actor.world.rot.y, 1, 0xFA0, 0);

    if (this->dyna.actor.speed < 0.0f) {
        this->dyna.actor.speed += 0.5f;
    }

    if ((this->dyna.actor.velocity.y <= 0.0f) && !BG_point_check2(this, play, -9.0f, this->dyna.actor.world.rot.y)) {
        this->dyna.actor.speed = 0.0f;
    }
...
static void mode_hirumu(EnAm* this, PlayState* play) {
    ...
    if ((this->dyna.actor.velocity.y <= 0.0f) &&
        !BG_point_check2(this, play, this->dyna.actor.speed * 1.5f, this->dyna.actor.world.rot.y)) {
        this->dyna.actor.speed = 0.0f;
    }
...
void en_am_attack_proc(Actor* thisx, PlayState* play) {
    static Vec3f sword_top = { 2500.0f, 7000.0f, 0.0f };
    static Vec3f sword_root = { -2500.0f, 0.0f, 0.0f };
    static Vec3f sword_top2 = { 2500.0f, 7000.0f, 4000.0f };
    static Vec3f sword_root2 = { -2500.0f, 0.0f, 4000.0f };
    EnAm* this = (EnAm*)thisx;

    Matrix_Position(&sword_top, &this->hitCollider.dim.quad[1]);
    Matrix_Position(&sword_root, &this->hitCollider.dim.quad[0]);
    Matrix_Position(&sword_top2, &this->hitCollider.dim.quad[3]);
    Matrix_Position(&sword_root2, &this->hitCollider.dim.quad[2]);

    CollisionCheck_Uty_setSword4Pos(&this->hitCollider, &this->hitCollider.dim.quad[0], &this->hitCollider.dim.quad[1],
                             &this->hitCollider.dim.quad[2], &this->hitCollider.dim.quad[3]);
}

void En_am_damage_proc(EnAm* this, PlayState* play) {
    s32 pad;
    ...
    if (this->deathTimer == 0) {
        if (this->blockCollider.base.acFlags & AC_BOUNCED) {
            ...
            if (this->dyna.actor.colChkInfo.damageEffect != AM_DMGEFF_MAGIC_FIRE_LIGHT) {
                ...
                Hit_bit_set(&this->dyna.actor, &this->hurtCollider.elem, false);
                ...
                if ((this->dyna.actor.colChkInfo.damageEffect == AM_DMGEFF_NUT) ||
                    ...
                    if (this->behavior != AM_BEHAVIOR_STUNNED) {
                        En_am_Actor_mode_damage2_init(this, play);
                        ...
                        sparkPos.y += 50.0f;
                        CollisionCheckSetSparkFlashBlue(play, &sparkPos);
                    }
...
void En_Am_actor_move(Actor* thisx, PlayState* play) {
    ...
    static Color_RGBA8 prim = { 150, 150, 150, 255 };
    static Color_RGBA8 env = { 100, 100, 100, 150 };
    s32 pad;
    EnAm* this = (EnAm*)thisx;
    EnBom* bomb;
    Vec3f dustPos;
    ...
    if (this->dyna.actor.colChkInfo.damageEffect != AM_DMGEFF_MAGIC_FIRE_LIGHT) {
        ...
        if (this->deathTimer != 0) {
            ...
            if (this->deathTimer == 0) {
                ...
                eff_set(this, play);
                bomb =
                    (EnBom*)Actor_info_make_actor(&play->actorCtx, play, ACTOR_EN_BOM, this->dyna.actor.world.pos.x,
                                        this->dyna.actor.world.pos.y, this->dyna.actor.world.pos.z, 0, 0, 2, BOMB_BODY);
                if (bomb != NULL) {
                    bomb->timer = 0;
                }

                Actor_SE_set(&this->dyna.actor, NA_SE_EN_AMOS_DEAD);
                Item_Set_Std(play, &this->dyna.actor, &this->dyna.actor.world.pos, 0xA0);

                for (i = 9; i >= 0; i--) {
                    dustPos.x = (sinf(dustPosScale) * 7.0f) + this->dyna.actor.world.pos.x;
                    dustPos.y = (rnd_fx(10.0f) * 6.0f) + (this->dyna.actor.world.pos.y + 40.0f);
                    dustPos.z = (cosf(dustPosScale) * 7.0f) + this->dyna.actor.world.pos.z;

                    Effect_SS_Dust_sc_cl_co_ct(play, &dustPos, &dam, &dam, &prim, &env, 200, 45, 12);
                    dustPosScale += 60.0f;
                }
            ...
            if ((this->deathTimer % 4) == 0) {
                Set_Fog(&this->dyna.actor, COLORFILTER_COLORFLAG_RED, 255, COLORFILTER_BUFFLAG_OPA, 4);
            }
        }

        Actor_position_moveF(&this->dyna.actor);
        Actor_BGcheck2(play, &this->dyna.actor, 20.0f, 28.0f, 80.0f,
                                UPDBGCHECKINFO_FLAG_0 | UPDBGCHECKINFO_FLAG_2 | UPDBGCHECKINFO_FLAG_3 |
                                    UPDBGCHECKINFO_FLAG_4);
    }

    CollisionCheck_Uty_ActorWorldPosSetPipeC(&this->dyna.actor, &this->hurtCollider);
    CollisionCheck_Uty_ActorWorldPosSetPipeC(&this->dyna.actor, &this->blockCollider);
    CollisionCheck_setOC(play, &play->colChkCtx, &this->hurtCollider.base);

    if (this->dyna.actor.params != ARMOS_STATUE) {
        Actor_world_to_eye(&this->dyna.actor, this->dyna.actor.scale.x * 4500.0f);
...
void en_am_display2(PlayState* play, s32 limbIndex, Gfx** dList, Vec3s* rot, void* thisx) {
    EnAm* this = (EnAm*)thisx;

    if ((limbIndex == 1) && (this->unk_264 != 0)) {
        en_am_attack_proc(&this->dyna.actor, play);
    }
}
...
void En_Am_actor_draw(Actor* thisx, PlayState* play) {
    ...
    Si2_draw(play, this->skelAnime.skeleton, this->skelAnime.jointTable, NULL, en_am_display2, this);
    ...
    if (this->iceTimer != 0) {
        ...
        if ((this->iceTimer % 4) == 0) {
            ...
            sp68.x = this->dyna.actor.world.pos.x + ice_pos[index].x;
            sp68.y = this->dyna.actor.world.pos.y + ice_pos[index].y;
            sp68.z = this->dyna.actor.world.pos.z + ice_pos[index].z;

            Effect_En_Ice_ct0(play, &this->dyna.actor, &sp68, 150, 150, 150, 250, 235, 245, 255, 1.4f);
        }
    }
