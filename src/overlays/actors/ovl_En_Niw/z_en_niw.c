/*
 * File: z_en_niw.c
 * Overlay: ovl_En_Niw
 * Description: Cucco
 */

#include "z_en_niw.h"
#include "assets/objects/object_niw/object_niw.h"
#include "overlays/actors/ovl_En_Attack_Niw/z_en_attack_niw.h"
#include "terminal.h"
#include "versions.h"

#define FLAGS (ACTOR_FLAG_UPDATE_CULLING_DISABLED | ACTOR_FLAG_THROW_ONLY)

void En_Niw_actor_ct(Actor* thisx, PlayState* play);
void En_Niw_actor_dt(Actor* thisx, PlayState* play);
void En_Niw_actor_move(Actor* thisx, PlayState* play);
void En_Niw_actor_draw(Actor* thisx, PlayState* play);

void mode_initial(EnNiw* this, PlayState* play);
static void mode_wait(EnNiw* this, PlayState* play);
static void mode_jump(EnNiw* this, PlayState* play);
void mode_ninja_wait(EnNiw* this, PlayState* play);
static void mode_walk(EnNiw* this, PlayState* play);
void mode_tonzura(EnNiw* this, PlayState* play);
void mode_get_up(EnNiw* this, PlayState* play);
void mode_get_throw_wait(EnNiw* this, PlayState* play);
void mode_fly_out(EnNiw* this, PlayState* play);
void mode_attack_camera_demo(EnNiw* this, PlayState* play);
void mode_summon_cry(EnNiw* this, PlayState* play);
void mode_fainal_attack(EnNiw* this, PlayState* play);
static void mode_runaway_init(EnNiw* this, PlayState* play);
static void mode_runaway(EnNiw* this, PlayState* play);
void niw_eff_set(EnNiw* this, Vec3f* pos, Vec3f* vel, Vec3f* accel, f32 scale);
void niw_eff_move(EnNiw* this, PlayState* play);
void niw_eff_disp(EnNiw* this, PlayState* play);

static s16 Niw_Attack_DEMO_START = 0;

ActorProfile En_Niw_Profile = {
    /**/ ACTOR_EN_NIW,
    /**/ ACTORCAT_PROP,
    /**/ FLAGS,
    /**/ OBJECT_NIW,
    /**/ sizeof(EnNiw),
    /**/ En_Niw_actor_ct,
    /**/ En_Niw_actor_dt,
    /**/ En_Niw_actor_move,
    /**/ En_Niw_actor_draw,
};

static f32 head_angle_data[] = {
    5000.0f,
    -5000.0f,
};

static f32 hurahura_data[] = {
    5000.0f,
    3000.0f,
    4000.0f,
};

static Vec3f niw_lady_birth_pos_check_data[] = {
    { -1697.0f, 80.0f, 870.0f }, { 57.0f, 320.0f, -673.0f }, { 796.0f, 80.0f, 1639.0f }, { 1417.0f, 465.0f, 169.0f },
    { -60.0f, 0.0f, -46.0f },    { -247.0f, 80.0f, 854.0f }, { 1079.0f, 80.0f, -47.0f },
};

static s16 niw_lady_birth_bit_check_data[] = {
    INFTABLE_MASK(INFTABLE_199), INFTABLE_MASK(INFTABLE_19A), INFTABLE_MASK(INFTABLE_19B), INFTABLE_MASK(INFTABLE_19C),
    INFTABLE_MASK(INFTABLE_19D), INFTABLE_MASK(INFTABLE_19E), INFTABLE_MASK(INFTABLE_19F),
};

static u8 room1_niw = false;

static u8 room2_niw = false;

static ColliderCylinderInit OcInfoData = {
    {
        COL_MATERIAL_HIT5,
        AT_NONE,
        AC_ON | AC_TYPE_PLAYER,
        OC1_ON,
        OC2_TYPE_2,
        COLSHAPE_CYLINDER,
    },
    {
        ELEM_MATERIAL_UNK0,
        { 0x00000000, 0x00, 0x00 },
        { 0xFFCFFFFF, 0x00, 0x00 },
        ATELEM_NONE,
        ACELEM_ON,
        OCELEM_ON,
    },
    { 15, 25, 4, { 0, 0, 0 } },
};

static ColliderCylinderInit No_weapon_OcInfoData = {
    {
        COL_MATERIAL_NONE,
        AT_NONE,
        AC_NONE,
        OC1_ON | OC1_TYPE_ALL,
        OC2_TYPE_2,
        COLSHAPE_CYLINDER,
    },
    {
        ELEM_MATERIAL_UNK0,
        { 0x00000000, 0x00, 0x00 },
        { 0x00000000, 0x00, 0x00 },
        ATELEM_NONE,
        ACELEM_NONE,
        OCELEM_ON,
    },
    { 15, 25, 4, { 0, 0, 0 } },
};

static InitChainEntry value_init[] = {
    ICHAIN_U8(attentionRangeType, ATTENTION_RANGE_6, ICHAIN_CONTINUE),
    ICHAIN_F32_DIV1000(gravity, -2000, ICHAIN_CONTINUE),
    ICHAIN_F32(lockOnArrowOffset, 0, ICHAIN_STOP),
};

void En_Niw_actor_ct(Actor* thisx, PlayState* play) {
    EnNiw* this = (EnNiw*)thisx;
    s32 pad;
    s32 i;

    if (this->actor.params < 0) {
        this->actor.params = 0;
    }

    // Cucco at at the very beginning of Zora's River
    if (this->actor.params == 0xB) {
        if (room1_niw) {
            Actor_delete(&this->actor);
            PRINTF(VT_FGCOL(YELLOW) "☆☆☆☆☆ もういてる原 Ver.1 ☆☆☆☆☆ \n" VT_RST);
            return;
        }
        room1_niw = true;
        this->actor.room = -1;
    }

    // Cucco at tall platform in the first room of Zora's River
    if (this->actor.params == 0xC) {
        if (room2_niw) {
            Actor_delete(&this->actor);
            PRINTF(VT_FGCOL(YELLOW) "☆☆☆☆☆ もういてる原 Ver.2 ☆☆☆☆☆ \n" VT_RST);
            return;
        }
        room2_niw = true;
        this->actor.room = -1;
    }

    ValueSet_process(&this->actor, value_init);
    this->actor.flags |= ACTOR_FLAG_ATTENTION_ENABLED;
    Shape_Info_init(&this->actor.shape, 0.0f, Actor_shadow_circle, 25.0f);
    Skeleton_Info2_SV_M_ct(play, &this->skelAnime, &gCuccoSkel, &gCuccoAnim, this->jointTable, this->morphTable, 16);

    if (play->sceneId == SCENE_KAKARIKO_VILLAGE) {
        for (i = 0; i < ARRAY_COUNT(niw_lady_birth_pos_check_data); i++) {
            if (fabsf(this->actor.world.pos.x - niw_lady_birth_pos_check_data[i].x) < 40.0f &&
                fabsf(this->actor.world.pos.z - niw_lady_birth_pos_check_data[i].z) < 40.0f) {
                this->unk_2AA = i;
                PRINTF(VT_FGCOL(YELLOW) " 通常鶏index %d\n" VT_RST, this->unk_2AA);
                if (z_common_data.save.info.infTable[INFTABLE_INDEX_199_19A_19B_19C_19D_19E_19F] &
                    niw_lady_birth_bit_check_data[i]) {
                    this->actor.world.pos.x = 300.0f;
                    this->actor.world.pos.y = 100.0f;
                    this->actor.world.pos.z = 1530.0f;
                    this->actor.params = 0;
                }
            }
        }
    }

    xyz_t_move(&this->unk_2AC, &this->actor.world.pos);
    xyz_t_move(&this->unk_2B8, &this->actor.world.pos);
    this->unk_304 = 10.0f;
    Actor_set_scale(&this->actor, 0.01f);
    this->unk_2A4 = (s16)rnd_f(3.99f) + 5;

    if (this->unk_2A4 < 0) {
        this->unk_2A4 = 1;
    }

    switch (this->actor.params) {
        case 2:
            if (IS_DAY) {
                Actor_delete(&this->actor);
            }
            break;
        case 1:
            if (GET_EVENTCHKINF(EVENTCHKINF_TALON_RETURNED_FROM_CASTLE)) {
                Actor_delete(&this->actor);
            }
            break;
        case 3:
            if (!GET_EVENTCHKINF(EVENTCHKINF_TALON_RETURNED_FROM_CASTLE)) {
                Actor_delete(&this->actor);
            }
            break;
        case 5:
            if (GET_EVENTCHKINF(EVENTCHKINF_EPONA_OBTAINED)) {
                Actor_delete(&this->actor);
            }
            break;
        case 7:
            if (!GET_EVENTCHKINF(EVENTCHKINF_EPONA_OBTAINED)) {
                Actor_delete(&this->actor);
            }
            break;
        case 0xD:
            this->actor.gravity = 0.0f;
            FALLTHROUGH;
        case 0xE:
            this->actor.colChkInfo.mass = 0;
            this->actor.flags &= ~ACTOR_FLAG_ATTENTION_ENABLED;
            break;
        case 4:
            this->actor.gravity = 0.0f;
            break;
    }

    ClObjPipe_ct(play, &this->collider);

    switch (this->actor.params) {
        case 0xA:
            this->actor.colChkInfo.mass = MASS_IMMOVABLE;
            FALLTHROUGH;
        case 0xD:
        case 0xE:
            ClObjPipe_set5(play, &this->collider, &this->actor, &No_weapon_OcInfoData);
            if (play->sceneId == SCENE_LINKS_HOUSE && !GET_EVENTCHKINF(EVENTCHKINF_HORSE_RACE_COW_UNLOCK)) {
                Actor_delete(&this->actor);
            }
            break;
        default:
            ClObjPipe_set5(play, &this->collider, &this->actor, &OcInfoData);
            break;
    }

    PRINTF(VT_FGCOL(YELLOW) "☆☆☆☆☆ どんな奴？ ☆☆☆☆☆ %d\n" VT_RST, this->actor.params);
    PRINTF("\n\n");
    this->actionFunc = mode_initial;
}

void En_Niw_actor_dt(Actor* thisx, PlayState* play) {
    EnNiw* this = (EnNiw*)thisx;

    ClObjPipe_dt(play, &this->collider);
}

static void move_parts_change(EnNiw* this, PlayState* play, s16 arg2) {
    f32 factor = 1.0f;

    if (this->actor.params == 0xD) {
        factor = 2.0f;
    }
    if (this->timer1 == 0) {
        if (arg2 == 0) {
            this->unk_26C[0] = 0.0f;
        } else {
            this->unk_26C[0] = -10000.0f * factor;
        }
        this->unk_298++;
        this->timer1 = 3;
        if (!(this->unk_298 & 1)) {
            this->unk_26C[0] = 0.0f;

            if (arg2 == 0) {
                this->timer1 = rnd_f(30.0f);
            }
        }
    }
    if (this->timer2 == 0) {
        this->unk_29C++;
        this->unk_29C &= 1;

        switch (arg2) {
            case 0:
                this->unk_26C[1] = this->unk_26C[2] = 0.0f;
                break;
            case 1:
                this->timer2 = 3;
                this->unk_26C[1] = this->unk_26C[2] = 7000.0f * factor;

                if (this->unk_29C == 0) {
                    this->unk_26C[1] = this->unk_26C[2] = 0.0f;
                }
                break;
            case 2:
                this->timer2 = 2;
                this->unk_26C[1] = this->unk_26C[2] = -10000.0f;
                this->unk_26C[7] = this->unk_26C[5] = 25000.0f;
                this->unk_26C[8] = this->unk_26C[6] = 6000.0f;

                if (this->unk_29C == 0) {
                    this->unk_26C[5] = this->unk_26C[7] = 8000.0f;
                }
                break;
            case 3:
                this->timer2 = 2;
                this->unk_26C[7] = this->unk_26C[5] = 10000.0f;

                if (this->unk_29C == 0) {
                    this->unk_26C[7] = this->unk_26C[5] = 3000.0f;
                }
                break;
            case 4:
                this->timer1 = 5;
                break;
            case 5:
                this->timer2 = 5;
                this->unk_26C[7] = this->unk_26C[5] = 14000.0f;
                if (this->unk_29C == 0) {
                    this->unk_26C[7] = this->unk_26C[5] = 10000.0f;
                }
                break;
        }
    }
    if (this->unk_2E0 != this->unk_26C[9]) {
        add_calc2(&this->unk_2E0, this->unk_26C[9], 0.5f, 4000.0f);
    }
    if (this->unk_2DC != this->unk_26C[0]) {
        add_calc2(&this->unk_2DC, this->unk_26C[0], 0.5f, 4000.0f);
    }
    if (this->unk_2C4 != this->unk_26C[2]) {
        add_calc2(&this->unk_2C4, this->unk_26C[2], 0.8f, 7000.0f);
    }
    if (this->unk_2C8 != this->unk_26C[7]) {
        add_calc2(&this->unk_2C8, this->unk_26C[7], 0.8f, 7000.0f);
    }
    if (this->unk_2CC != this->unk_26C[8]) {
        add_calc2(&this->unk_2CC, this->unk_26C[8], 0.8f, 7000.0f);
    }
    if (this->unk_2D0 != this->unk_26C[1]) {
        add_calc2(&this->unk_2D0, this->unk_26C[1], 0.8f, 7000.0f);
    }
    if (this->unk_2D4 != this->unk_26C[5]) {
        add_calc2(&this->unk_2D4, this->unk_26C[5], 0.8f, 7000.0f);
    }
    if (this->unk_2D8 != this->unk_26C[6]) {
        add_calc2(&this->unk_2D8, this->unk_26C[6], 0.8f, 7000.0f);
    }
}

void enemy_birth_rtn(EnNiw* this, PlayState* play) {
    f32 viewX;
    f32 viewY;
    f32 viewZ;
    Vec3f attackCuccoPos;
    Actor* attackCucco;

    if ((this->timer5 == 0) && (this->unk_296 < 7)) {
        viewX = play->view.at.x - play->view.eye.x;
        viewY = play->view.at.y - play->view.eye.y;
        viewZ = play->view.at.z - play->view.eye.z;
        attackCuccoPos.x = ((fqrand() - 0.5f) * viewX) + play->view.eye.x;
        attackCuccoPos.y = rnd_fx(0.3f) + ((play->view.eye.y + 50.0f) + (viewY * 0.5f));
        attackCuccoPos.z = ((fqrand() - 0.5f) * viewZ) + play->view.eye.z;
        attackCucco = Actor_info_make_child_actor(&play->actorCtx, &this->actor, play, ACTOR_EN_ATTACK_NIW, attackCuccoPos.x,
                                         attackCuccoPos.y, attackCuccoPos.z, 0, 0, 0, 0);

        if (attackCucco != NULL) {
            this->unk_296++;
            this->timer5 = 10;
        } else {
            PRINTF("\n\n");
            PRINTF(VT_FGCOL(GREEN) " ☆☆☆☆☆ 発生できず  ☆☆☆☆☆ \n" VT_RST);
        }
    }
}

void yororehi_move(EnNiw* this, PlayState* play, s32 arg2) {
    f32 factor;
    f32 targetRotY;

    if (this->timer4 == 0) {
        this->timer4 = 3;

        if (this->actor.bgCheckFlags & BGCHECKFLAG_GROUND) {
            this->actor.velocity.y = 3.5f;
        }
    }
    if (this->timer3 == 0) {
        this->unk_2A0++;
        this->unk_2A0 &= 1;
        this->timer3 = 5;
    }
    if (this->unk_2A0 == 0) {
        factor = hurahura_data[arg2];
    } else {
        factor = -hurahura_data[arg2];
    }
    if (arg2 == 1) {
        if (this->timer6 == 0 || this->actor.bgCheckFlags & BGCHECKFLAG_WALL) {
            this->timer6 = 150;
            if (this->timer8 == 0) {
                this->timer8 = 70;
                this->unk_2E4 = this->actor.yawTowardsPlayer;
            }
        }
    }
    targetRotY = this->unk_2E4 + factor;
    add_calc_short_angle2(&this->actor.world.rot.y, targetRotY, 3, this->unk_2FC, 0);
    add_calc2(&this->unk_2FC, 3000.0f, 1.0f, 500.0f);
    move_parts_change(this, play, 5);
}

void mode_initial(EnNiw* this, PlayState* play) {
    Skeleton_Info2_init(&this->skelAnime, &gCuccoAnim, 1.0f, 0.0f, Si2_anime_end_frame(&gCuccoAnim), ANIMMODE_LOOP,
                     -10.0f);

    switch (this->actor.params) {
        case 4:
            this->actionFunc = mode_ninja_wait;
            break;
        case 0xD:
            this->actionFunc = mode_wait;
            break;
        default:
            this->actionFunc = mode_walk;
            break;
    }
}

static void mode_wait(EnNiw* this, PlayState* play) {
    if (this->unk_308 != 0) {
        this->actor.velocity.y = rnd_f(2.0f) + 4.0f;
        this->actor.speed = rnd_f(2.0f) + 3.0f;
        this->actionFunc = mode_jump;
    }
    move_parts_change(this, play, 1);
}

static void mode_jump(EnNiw* this, PlayState* play) {
    if ((this->actor.bgCheckFlags & BGCHECKFLAG_GROUND) && this->actor.velocity.y < 0.0f) {
        this->unk_2AC.x = this->unk_2B8.x = this->actor.world.pos.x;
        this->unk_2AC.y = this->unk_2B8.y = this->actor.world.pos.y;
        this->unk_2AC.z = this->unk_2B8.z = this->actor.world.pos.z;
        this->timer5 = this->timer4 = this->unk_29E = 0;

        this->unk_26C[7] = this->unk_26C[5] = this->unk_26C[6] = this->unk_26C[8] = this->actor.speed = this->unk_2FC =
            this->unk_300 = 0.0f;

        this->actionFunc = mode_walk;
    } else {
        move_parts_change(this, play, 2);
    }
}

void mode_ninja_wait(EnNiw* this, PlayState* play) {
    Player* player = GET_PLAYER(play);

    if (this->actor.xzDistToPlayer < 30.0f && fabsf(this->actor.world.pos.y - player->actor.world.pos.y) < 5.0f) {
        this->timer6 = 100;
        this->actor.gravity = -2.0f;
        this->actionFunc = mode_runaway_init;
    } else if (Actor_carry_check(&this->actor, play)) {
        this->actor.gravity = -2.0f;
        Actor_SE_set(&this->actor, NA_SE_EV_CHICKEN_CRY_M);
        this->sfxTimer1 = 30;
        this->path = 0;
        this->timer4 = 30;
        this->actor.flags &= ~ACTOR_FLAG_ATTENTION_ENABLED;
        this->actor.speed = 0.0f;
        this->actionFunc = mode_get_up;
    } else {
        // GI_NONE in this case allows the player to lift the actor
        Actor_carry_request_set2(&this->actor, play, GI_NONE, 25.0f, 10.0f);
        move_parts_change(this, play, 1);
    }
}

static void mode_walk(EnNiw* this, PlayState* play) {
    s32 pad[2];
    f32 posY = rnd_fx(100.0f);
    f32 posZ = rnd_fx(100.0f);
    s16 tmp;

    if (this->actor.params != 0xA) {
        if (Actor_carry_check(&this->actor, play)) {
            Actor_SE_set(&this->actor, NA_SE_EV_CHICKEN_CRY_M);
            this->sfxTimer1 = 30;
            this->path = 0;
            this->timer4 = 30;
            this->actor.flags &= ~ACTOR_FLAG_ATTENTION_ENABLED;
            this->actor.speed = 0.0f;
            this->actionFunc = mode_get_up;
            return;
        }
        Actor_carry_request(&this->actor, play);
    } else {
        if (this->path != 0) {
            this->unk_2A6 = 1;
            if (this->sfxTimer3 == 0) {
                Actor_SE_set(&this->actor, NA_SE_EV_CHICKEN_CRY_M);
                this->sfxTimer3 = 100;
            }
            this->unk_2A0 = rnd_f(1.99f);
            this->actor.speed = 4.0f;
            this->unk_300 = 0.0f;
            this->unk_2FC = 0.0f;
            this->actionFunc = mode_tonzura;
            return;
        }
    }

    tmp = 0;

    if (this->timer5 != 0) {
        if (rnd_f(3.99f) < 1.0f) {
            this->unk_2E6++;
            this->unk_2E6 &= 1;
        }
        add_calc2(&this->unk_26C[9], head_angle_data[this->unk_2E6], 0.5f, 4000.0f);
    }

    if (this->timer5 == 0 && this->timer4 == 0) {
        this->unk_29E++;

        if (this->unk_29E >= 8) {
            this->timer5 = rnd_f(30.0f);
            this->unk_29E = rnd_f(3.99f);
            if (this->actor.params != 0xA && this->actor.params != 8) {
                if (posY < 0.0f) {
                    posY -= 100.0f;
                } else {
                    posY += 100.0f;
                }
                if (posZ < 0.0f) {
                    posZ -= 100.0f;
                } else {
                    posZ += 100.0f;
                }
            } else {
                posY = rnd_fx(30.0f);
                posZ = rnd_fx(30.0f);
                if (posY < 0.0f) {
                    posY -= 20.0f;
                } else {
                    posY += 20.0f;
                }
                if (posZ < 0.0f) {
                    if (1) {} // Required to match
                    if (1) {}
                    if (1) {}
                    if (1) {}
                    posZ -= 20.0f;
                } else {
                    posZ += 20.0f;
                }
            }
            this->unk_2B8.x = this->unk_2AC.x + posY;
            this->unk_2B8.z = this->unk_2AC.z + posZ;
        } else {
            this->timer4 = 4;
            if (this->actor.bgCheckFlags & BGCHECKFLAG_GROUND) {
                this->actor.speed = 0.0f;
                this->actor.velocity.y = 3.5f;
            }
        }
    }

    if (this->timer4 != 0) {
        add_calc0(&this->unk_26C[9], 0.5f, 4000.0f);
        tmp = 1;
        add_calc2(&this->actor.world.pos.x, this->unk_2B8.x, 1.0f, this->unk_2FC);
        add_calc2(&this->actor.world.pos.z, this->unk_2B8.z, 1.0f, this->unk_2FC);
        add_calc2(&this->unk_2FC, 3.0f, 1.0f, 0.3f);
        posY = this->unk_2B8.x - this->actor.world.pos.x;
        posZ = this->unk_2B8.z - this->actor.world.pos.z;

        if (fabsf(posY) < 10.0f) {
            posY = 0.0;
        }
        if (fabsf(posZ) < 10.0f) {
            posZ = 0.0;
        }
        if (posY == 0.0f && posZ == 0.0f) {
            this->timer4 = 0;
            this->unk_29E = 7;
        }

        add_calc_short_angle2(&this->actor.world.rot.y, RAD_TO_BINANG(fatan2(posY, posZ)), 3, this->unk_300, 0);
        add_calc2(&this->unk_300, 10000.0f, 1.0f, 1000.0f);
    }

    move_parts_change(this, play, tmp);
}

void mode_tonzura(EnNiw* this, PlayState* play) {
    Path* path;
    Vec3s* pointPos;
    f32 pathDiffX;
    f32 pathDiffZ;
    s16 pathIndex = this->path - 1;

    if (this->path == 0) {
        this->unk_2AC.x = this->unk_2B8.x = this->actor.world.pos.x;
        this->unk_2AC.y = this->unk_2B8.y = this->actor.world.pos.y;
        this->unk_2AC.z = this->unk_2B8.z = this->actor.world.pos.z;
        this->timer5 = this->timer4 = this->unk_29E = 0;
        this->unk_26C[7] = this->unk_26C[5] = this->unk_26C[6] = this->unk_26C[8] = this->actor.speed = this->unk_2FC =
            this->unk_300 = 0.0f;
        this->actionFunc = mode_initial;
    } else {
        path = &play->pathList[pathIndex];
        pointPos = SEGMENTED_TO_VIRTUAL(path->points);
        pointPos += this->waypoint;
        pathDiffX = pointPos->x - this->actor.world.pos.x;
        pathDiffZ = pointPos->z - this->actor.world.pos.z;
        this->unk_2E4 = RAD_TO_BINANG(fatan2(pathDiffX, pathDiffZ));
        yororehi_move(this, play, 2);

        if (fabsf(pathDiffX) < 30.0f && fabsf(pathDiffZ) < 30.0f) {
            this->waypoint++;
            if (this->waypoint >= this->unk_2EC) {
                this->waypoint = 0;
            }
        }

        move_parts_change(this, play, 2);
    }
}

void mode_get_up(EnNiw* this, PlayState* play) {
    if (this->timer4 == 0) {
        this->unk_2A6 = 2;
        this->timer4 = 10;
    }

    this->actor.shape.rot.x = rnd_fx(5000.0f);
    this->actor.shape.rot.y = rnd_fx(5000.0f);
    this->actor.shape.rot.z = rnd_fx(5000.0f);

    if (Actor_carry_end_check(&this->actor, play)) {
        if (this->actor.params == 0xD) {
            this->sfxTimer1 = 0;
            this->unk_2A6 = 1;
            this->actionFunc = mode_fly_out;
            this->actor.velocity.y = 4.0f;
            return;
        }
        this->actor.shape.rot.z = 0;
        this->actor.shape.rot.y = this->actor.shape.rot.z;
        this->actor.shape.rot.x = this->actor.shape.rot.z;
        this->actor.flags |= ACTOR_FLAG_ATTENTION_ENABLED;
        this->actionFunc = mode_get_throw_wait;
    }
    move_parts_change(this, play, 2);
}

void mode_get_throw_wait(EnNiw* this, PlayState* play) {
    if (this->path == 0) {
        if (!(this->actor.bgCheckFlags & BGCHECKFLAG_GROUND)) {
            return;
        }
        if (this->actor.params == 0xE) {
            this->unk_2AC.x = this->unk_2B8.x = this->actor.world.pos.x;
            this->unk_2AC.y = this->unk_2B8.y = this->actor.world.pos.y;
            this->unk_2AC.z = this->unk_2B8.z = this->actor.world.pos.z;
            this->timer5 = this->timer4 = this->unk_29E = 0;

            this->unk_26C[7] = this->unk_26C[5] = this->unk_26C[6] = this->unk_26C[8] = this->actor.speed =
                this->unk_2FC = this->unk_300 = 0.0f;

            this->actionFunc = mode_initial;
            return;
        }

        this->path = 1;
        this->timer5 = 80;
        this->actor.speed = 0.0f;
        this->actor.velocity.y = 4.0f;
    } else {
        if (this->actor.bgCheckFlags & BGCHECKFLAG_GROUND) {
            this->sfxTimer1 = 0;
            this->actor.velocity.y = 4.0f;
            this->unk_2A6 = 1;
        }
        if (this->timer5 == 0) {
            this->timer6 = 100;
            this->timer4 = 0;
            this->path = 0;
            this->actionFunc = mode_runaway_init;
            return;
        }
    }

    if (Actor_carry_check(&this->actor, play)) {
        Actor_SE_set(&this->actor, NA_SE_EV_CHICKEN_CRY_M);
        this->sfxTimer1 = 30;
        this->path = 0;
        this->timer4 = 30;
        this->actor.flags &= ~ACTOR_FLAG_ATTENTION_ENABLED;
        this->actor.speed = 0.0f;
        this->actionFunc = mode_get_up;
    } else {
        if (this->timer5 >= 6) {
            Actor_carry_request(&this->actor, play);
        }
        move_parts_change(this, play, 2);
    }
}

void mode_fly_out(EnNiw* this, PlayState* play) {
    if (this->actor.world.pos.y > 400.0f) {
        Actor_delete(&this->actor);
    }

    move_parts_change(this, play, 2);
}

static void mode_water_move(EnNiw* this, PlayState* play) {
    Vec3f pos;

    if (this->unk_2A8 != 0) {
        enemy_birth_rtn(this, play);
    }

    this->actor.speed = 2.0f;

    if (this->actor.bgCheckFlags & BGCHECKFLAG_WATER) {
        this->actor.gravity = 0.0f;

        if (this->actor.depthInWater > 15.0f) {
            this->actor.world.pos.y += 2.0f;
        }
        if (this->timer4 == 0) {
            this->timer4 = 30;
            xyz_t_move(&pos, &this->actor.world.pos);
            pos.y += this->actor.depthInWater;
            Effect_SS_G_Ripple_ct2(play, &pos, 100, 500, 30);
        }
        if (this->actor.bgCheckFlags & BGCHECKFLAG_WALL) {
            this->actor.velocity.y = 10.0f;
            this->actor.speed = 1.0f;
        }
    } else {
        this->actor.gravity = -2.0f;

        if (this->actor.bgCheckFlags & BGCHECKFLAG_WALL) {
            this->actor.velocity.y = 10.0f;
            this->actor.speed = 1.0f;
            this->actor.gravity = 0.0f;
        } else {
            this->actor.speed = 4.0f;
        }
        if (this->actor.bgCheckFlags & BGCHECKFLAG_GROUND) {
            this->actor.gravity = -2.0f;
            this->timer6 = 100;
            this->timer4 = 0;
            this->actor.velocity.y = 0.0f;
            if (this->unk_2A8 == 0) {
                this->actionFunc = mode_runaway_init;
            } else {
                this->actionFunc = mode_fainal_attack;
            }
        }
    }

    move_parts_change(this, play, 2);
}

void mode_attack_camera_demo_init(EnNiw* this, PlayState* play) {
    makeOnepointDemo(play, 2290, -99, &this->actor, CAM_ID_MAIN);
    this->timer5 = 100;
    this->unk_2A2 = 1;
    this->actionFunc = mode_attack_camera_demo;
}

void mode_attack_camera_demo(EnNiw* this, PlayState* play) {
    this->sfxTimer1 = 100;

    if (this->timer5 == 0) {
        this->timer5 = 60;
        this->timer1 = 10;
        this->unk_2A2 = 4;
        this->actionFunc = mode_summon_cry;
    }

    move_parts_change(this, play, this->unk_2A2);
}

void mode_summon_cry(EnNiw* this, PlayState* play) {
    this->sfxTimer1 = 100;

    if (this->timer5 == 40) {
        this->unk_26C[0] = 10000.0f;
        this->unk_26C[7] = 14000.0f;
        this->unk_26C[5] = 14000.0f;
        this->unk_26C[6] = 0.0f;
        this->unk_26C[8] = 0.0f;
        this->unk_26C[1] = 0.0f;
        this->unk_26C[2] = 0.0f;
        this->timer1 = 10;
        Actor_SE_set(&this->actor, NA_SE_EV_CHICKEN_CRY_M);
    }
    if (this->timer5 == 0) {
        this->timer7 = 10;
        this->unk_2E4 = this->actor.yawTowardsPlayer;
        this->actor.flags &= ~ACTOR_FLAG_ATTENTION_ENABLED;
        this->actionFunc = mode_fainal_attack;
    }

    move_parts_change(this, play, this->unk_2A2);
}

void mode_fainal_attack(EnNiw* this, PlayState* play) {
    enemy_birth_rtn(this, play);

    if (this->timer7 < 2) {
        if (this->timer7 == 1) {
            this->actor.speed = 3.0f;
            this->unk_2A0 = rnd_f(1.99f);
            this->timer1 = this->timer2 = this->timer3 = this->timer4 = 0;
        } else {
            yororehi_move(this, play, 1);
        }
    }
}

static void mode_runaway_init(EnNiw* this, PlayState* play) {
    Skeleton_Info2_init(&this->skelAnime, &gCuccoAnim, 1.0f, 0.0f, Si2_anime_end_frame(&gCuccoAnim), ANIMMODE_LOOP,
                     -10.0f);
    this->unk_2A0 = rnd_f(1.99f);
    this->actor.speed = 4.0f;
    this->actionFunc = mode_runaway;
}

static void mode_runaway(EnNiw* this, PlayState* play) {
    Player* player = GET_PLAYER(play);

    if (this->timer6 == 0) {
        this->unk_2AC.x = this->unk_2B8.x = this->actor.world.pos.x;
        this->unk_2AC.y = this->unk_2B8.y = this->actor.world.pos.y;
        this->unk_2AC.z = this->unk_2B8.z = this->actor.world.pos.z;
        this->timer5 = this->timer4 = this->unk_29E = 0;
        this->unk_26C[7] = this->unk_26C[5] = this->unk_26C[6] = this->unk_26C[8] = this->actor.speed = this->unk_2FC =
            this->unk_300 = 0.0f;
        if (this->actor.params == 4) {
            this->actor.params = 0;
        }
        this->actionFunc = mode_initial;
    } else {
        this->unk_2E4 = RAD_TO_BINANG(fatan2(this->actor.world.pos.x - player->actor.world.pos.x,
                                                   this->actor.world.pos.z - player->actor.world.pos.z));
        yororehi_move(this, play, 0);
        move_parts_change(this, play, 2);
    }
}

void mode_drop_out_return(EnNiw* this, PlayState* play) {
    if (this->actor.bgCheckFlags & BGCHECKFLAG_GROUND) {
        this->unk_2A4 = (s16)rnd_f(3.99f) + 5;
        this->actionFunc = mode_initial;
    }
}

void En_Niw_damage_proc(EnNiw* this, PlayState* play) {
    if (this->unk_2A8 == 0 && this->actor.params != 0xA && this->actionFunc != mode_ninja_wait &&
        this->collider.base.acFlags & AC_HIT) {
        this->collider.base.acFlags &= ~AC_HIT;
        this->sfxTimer1 = 30;
        if (this->unk_2A4 > 0 && Niw_Attack_DEMO_START == 0) {
            this->unk_2A4--;
        }
        this->unk_2A6 = 1;
        Actor_SE_set(&this->actor, NA_SE_EV_CHICKEN_CRY_M);
        this->timer6 = 100;
        this->path = 0;
        this->actionFunc = mode_runaway_init;
    }
}

void En_Niw_actor_move(Actor* thisx, PlayState* play) {
    s32 pad1;
    EnNiw* this = (EnNiw*)thisx;
    Player* player = GET_PLAYER(play);
    s16 i;
    s16 featherCount;
    Vec3f zeroVec1 = { 0.0f, 0.0f, 0.0f };
    Vec3f zeroVec2 = { 0.0f, 0.0f, 0.0f };
    Vec3f pos;
    Vec3f vel;
    Vec3f accel;
    s32 pad2;
    f32 scale;
    f32 dist;

    if (1) {} // Required to match

    this->unk_294++;

    if (this->actionFunc != mode_walk) {
        this->unk_26C[9] = 0.0f;
    }
    if (this->unk_2A6) {
        featherCount = 20;

        if (this->unk_2A6 == 2) {
            featherCount = 4;
        }

        for (i = 0; i < featherCount; i++) {
            pos.x = rnd_fx(10.0f) + thisx->world.pos.x;
            pos.y = rnd_fx(10.0f) + (thisx->world.pos.y + this->unk_304);
            pos.z = rnd_fx(10.0f) + thisx->world.pos.z;
            scale = rnd_f(6.0f) + 6.0f;

            if (this->unk_2A6 == 2 && this->unk_304 != 0) {
                pos.y += 10.0f;
            }
            if (this->unk_304 == 0) {
                scale = rnd_f(2.0f) + 2.0f;
            }

            vel.x = rnd_fx(3.0f);
            vel.y = (rnd_f(2.0f) * 0.5f) + 2.0f;
            vel.z = rnd_fx(3.0f);
            accel.x = 0.0f;
            accel.y = -0.15f;
            accel.z = 0.0f;
            niw_eff_set(this, &pos, &vel, &accel, scale);
        }

        this->unk_2A6 = 0;
    }

    niw_eff_move(this, play);
    DECR(this->timer1);
    DECR(this->timer2);
    DECR(this->timer3);
    DECR(this->timer4);
    DECR(this->timer5);
    DECR(this->timer7);
    DECR(this->timer6);
    DECR(this->sfxTimer1);
    DECR(this->sfxTimer2);
    DECR(this->sfxTimer3);
    DECR(this->timer8);
    DECR(this->timer9);

    thisx->shape.rot = thisx->world.rot;
    thisx->shape.shadowScale = 15.0f;
    this->actionFunc(this, play);
    Actor_world_to_eye(&this->actor, this->unk_304);
    Actor_position_moveF(&this->actor);

    if (this->actionFunc != mode_fly_out && this->actionFunc != mode_ninja_wait && play->sceneId != SCENE_ZORAS_RIVER) {
        Actor_BGcheck2(play, &this->actor, 20.0f, 20.0f, 60.0f,
                                UPDBGCHECKINFO_FLAG_0 | UPDBGCHECKINFO_FLAG_1 | UPDBGCHECKINFO_FLAG_2 |
                                    UPDBGCHECKINFO_FLAG_3 | UPDBGCHECKINFO_FLAG_4);
    }
    if (play->sceneId == SCENE_ZORAS_RIVER) {
        Actor_BGcheck2(play, &this->actor, 20.0f, 20.0f, 60.0f,
                                UPDBGCHECKINFO_FLAG_0 | UPDBGCHECKINFO_FLAG_2 | UPDBGCHECKINFO_FLAG_3 |
                                    UPDBGCHECKINFO_FLAG_4);
    }
    if (thisx->floorHeight <= BGCHECK_Y_MIN || thisx->floorHeight >= 32000.0f) {
        Vec3f cam;
        f32 camResult;
        s32 pad3[10];

        PRINTF(VT_FGCOL(GREEN) "☆☆☆☆☆ 上下？ ☆☆☆☆☆ %f\n" VT_RST, thisx->floorHeight);
        cam.x = play->view.at.x - play->view.eye.x;
        cam.y = play->view.at.y - play->view.eye.y;
        cam.z = play->view.at.z - play->view.eye.z;
        camResult = cam.y / sqrtf(SQ(cam.x) + SQ(cam.y) + SQ(cam.z));
        PRINTF(VT_FGCOL(RED) "☆☆☆☆☆ 範囲外Ｘ！ ☆☆☆☆☆ %f\n" VT_RST, thisx->world.pos.x);
        PRINTF(VT_FGCOL(RED) "☆☆☆☆☆ 範囲外Ｙ！ ☆☆☆☆☆ %f\n" VT_RST, thisx->world.pos.y);
        PRINTF(VT_FGCOL(RED) "☆☆☆☆☆ 範囲外Ｚ！ ☆☆☆☆☆ %f\n" VT_RST, thisx->world.pos.z);
        PRINTF(VT_FGCOL(GREEN) "☆☆☆☆☆ セットＸ！ ☆☆☆☆☆ %f\n" VT_RST, thisx->home.pos.x);
        PRINTF(VT_FGCOL(GREEN) "☆☆☆☆☆ セットＹ！ ☆☆☆☆☆ %f\n" VT_RST, thisx->home.pos.y);
        PRINTF(VT_FGCOL(GREEN) "☆☆☆☆☆ セットＺ！ ☆☆☆☆☆ %f\n" VT_RST, thisx->home.pos.z);
        thisx->world.pos.x = thisx->home.pos.x;
        thisx->world.pos.z = thisx->home.pos.z;
        thisx->world.pos.y = ((thisx->home.pos.y + play->view.eye.y) + (camResult * 160.0f));

        if (thisx->world.pos.y < thisx->home.pos.y) {
            thisx->world.pos.y = thisx->home.pos.y + 300.0f;
        }

        PRINTF(VT_FGCOL(YELLOW) "☆☆☆☆☆ 修整後Ｘ！ ☆☆☆☆☆ %f\n" VT_RST, thisx->world.pos.x);
        PRINTF(VT_FGCOL(YELLOW) "☆☆☆☆☆ 修整後Ｙ！ ☆☆☆☆☆ %f\n" VT_RST, thisx->world.pos.y);
        PRINTF(VT_FGCOL(YELLOW) "☆☆☆☆☆ 修整後Ｚ！ ☆☆☆☆☆ %f\n" VT_RST, thisx->world.pos.z);
        PRINTF("\n\n");
        thisx->speed = 0.0f;
        thisx->gravity = -2.0f;
        xyz_t_move(&this->unk_2AC, &thisx->home.pos);
        xyz_t_move(&this->unk_2B8, &thisx->home.pos);
        this->unk_300 = 0.0f;
        this->unk_2FC = 0.0f;
        this->unk_2F0.z = 0.0f;
        this->unk_2F0.y = 0.0f;
        this->unk_2F0.x = 0.0f;
        this->unk_2D8 = 0.0f;
        this->unk_2D4 = 0.0f;
        this->unk_2D0 = 0.0f;
        this->unk_2CC = 0.0f;
        this->unk_2C8 = 0.0f;
        this->unk_2C4 = 0.0f;
        this->unk_2DC = 0.0f;
        this->unk_2E0 = 0.0f;
        this->unk_2A8 = this->unk_294 = this->unk_298 = this->unk_2A6 = this->unk_29E = this->unk_2A0 = this->unk_2A2 =
            0;

        for (i = 0; i < ARRAY_COUNT(this->unk_26C); i++) {
            this->unk_26C[i] = 0;
        }

        this->unk_2A8 = 0;
        this->actionFunc = mode_drop_out_return;
        return;
    }

    if ((thisx->bgCheckFlags & BGCHECKFLAG_WATER) && thisx->depthInWater > 15.0f && this->actionFunc != mode_water_move &&
        thisx->params != 0xD && thisx->params != 0xE && thisx->params != 0xA) {
        thisx->velocity.y = 0.0f;
        thisx->gravity = 0.0f;
        xyz_t_move(&pos, &thisx->world.pos);
        pos.y += thisx->depthInWater;
        this->timer4 = 30;
        Effect_SS_G_Splash_sc_cl_ct(play, &pos, NULL, NULL, 0, 400);
        this->timer5 = 0;
        PRINTF("\n\n");
        PRINTF(VT_FGCOL(YELLOW) "☆☆☆☆☆ ぶくぶく ☆☆☆☆☆ \n" VT_RST);
        PRINTF("\n\n");
        this->actionFunc = mode_water_move;
        return;
    }

    if (Niw_Attack_DEMO_START == 0 && this->unk_2A4 <= 0 && thisx->params != 0xD && thisx->params != 0xE && thisx->params != 0xA) {
        this->timer6 = 100;

        if (thisx->xzDistToPlayer > 10.0f) {
            Niw_Attack_DEMO_START = 1;
            this->timer5 = this->timer4 = this->unk_29E = 0;
            thisx->speed = 0.0f;
            this->unk_2FC = 0.0f;
            this->unk_300 = 0.0f;
            this->unk_26C[7] = 0.0f;
            this->unk_26C[5] = 0.0f;
            this->unk_26C[6] = 0.0f;
            this->unk_26C[8] = 0.0f;
            this->sfxTimer1 = 10000;
            this->unk_2A8 = 1;
            this->unk_2AC.x = this->unk_2B8.x = thisx->world.pos.x;
            this->unk_2AC.y = this->unk_2B8.y = thisx->world.pos.y;
            this->unk_2AC.z = this->unk_2B8.z = thisx->world.pos.z;
            this->actionFunc = mode_attack_camera_demo_init;
            return;
        }
    }

    dist = 20.0f;

#if OOT_VERSION < NTSC_1_1
    if (this->unk_2A8 != 0 && thisx->xyzDistToPlayerSq < SQ(dist) && !(player->stateFlags1 & PLAYER_STATE1_26))
#else
    if (this->unk_2A8 != 0 && thisx->xyzDistToPlayerSq < SQ(dist) && player->invincibilityTimer == 0)
#endif
    {
        Actor_player_power_damage_AT_set(play, &this->actor, 2.0f, thisx->world.rot.y, 0.0f, 0x10);
    }

    En_Niw_damage_proc(this, play);

    if (this->sfxTimer2 == 0 && this->actionFunc == mode_get_up) {
        this->sfxTimer2 = 7;
        Actor_SE_set(&this->actor, NA_SE_EN_DEKU_WAKEUP);
    }
    if (this->sfxTimer1 == 0) {
        if (this->actionFunc != mode_walk) {
            this->sfxTimer1 = 30;
            Actor_SE_set(&this->actor, NA_SE_EV_CHICKEN_CRY_A);
        } else {
            this->sfxTimer1 = 300;
            Actor_SE_set(&this->actor, NA_SE_EV_CHICKEN_CRY_N);
        }
    }
    if (this->unk_2A8 == 0) {
        CollisionCheck_Uty_ActorWorldPosSetPipeC(&this->actor, &this->collider);

        if (thisx->params != 0xA && thisx->params != 0xD && thisx->params != 0xE && thisx->params != 4) {
            CollisionCheck_setAC(play, &play->colChkCtx, &this->collider.base);
        }
        if (this->actionFunc != mode_get_up && this->actionFunc != mode_get_throw_wait &&
            this->actionFunc != mode_wait && this->actionFunc != mode_jump &&
            this->actionFunc != mode_ninja_wait) {
            CollisionCheck_setOC(play, &play->colChkCtx, &this->collider.base);
        }
    }
}

s32 En_Niw_draw_sub(PlayState* play, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx) {
    EnNiw* this = (EnNiw*)thisx;
    Vec3f zeroVec = { 0.0f, 0.0f, 0.0f };

    if (limbIndex == 13) {
        rot->y += (s16)this->unk_2DC;
    }
    if (limbIndex == 15) {
        rot->y += (s16)this->unk_2E0;
    }
    if (limbIndex == 11) {
        rot->x += (s16)this->unk_2D8;
        rot->y += (s16)this->unk_2D4;
        rot->z += (s16)this->unk_2D0;
    }
    if (limbIndex == 7) {
        rot->x += (s16)this->unk_2CC;
        rot->y += (s16)this->unk_2C8;
        rot->z += (s16)this->unk_2C4;
    }

    return false;
}

void En_Niw_actor_draw(Actor* thisx, PlayState* play) {
    EnNiw* this = (EnNiw*)thisx;
    Vec3f scale = { 0.15f, 0.15f, 0.15f };
    GraphicsContext* gfxCtx = play->state.gfxCtx;

    _texture_z_light_fog_prim(play->state.gfxCtx);
    Si2_draw_SV(play, this->skelAnime.skeleton, this->skelAnime.jointTable, this->skelAnime.dListCount,
                          En_Niw_draw_sub, NULL, this);

    if (this->actionFunc == mode_ninja_wait) {
        Shadow_draw(&this->actor.world.pos, &scale, 255, play);
    }

    niw_eff_disp(this, play);
}

void niw_eff_set(EnNiw* this, Vec3f* pos, Vec3f* vel, Vec3f* accel, f32 scale) {
    s16 i;
    EnNiwEffect* effect = this->effects;

    for (i = 0; i < EN_NIW_EFFECT_COUNT; i++, effect++) {
        if (effect->type == 0) {
            effect->type = 1;
            effect->pos = *pos;
            effect->vel = *vel;
            effect->accel = *accel;
            effect->timer = 0;
            effect->scale = scale / 1000.0f;
            effect->life = (s16)rnd_f(20.0f) + 40;
            effect->unk_2A = rnd_f(1000.0f);
            break;
        }
    }
}

void niw_eff_move(EnNiw* this, PlayState* play) {
    s16 i;
    EnNiwEffect* effect = this->effects;

    for (i = 0; i < EN_NIW_EFFECT_COUNT; i++, effect++) {
        if (effect->type != 0) {
            effect->timer++;
            effect->pos.x += effect->vel.x;
            effect->pos.y += effect->vel.y;
            effect->pos.z += effect->vel.z;
            effect->vel.x += effect->accel.x;
            effect->vel.y += effect->accel.y;
            effect->vel.z += effect->accel.z;
            if (effect->type == 1) {
                effect->unk_2A++;
                add_calc2(&effect->vel.x, 0.0f, 1.0f, 0.05f);
                add_calc2(&effect->vel.z, 0.0f, 1.0f, 0.05f);
                if (effect->vel.y < -0.5f) {
                    effect->vel.y = -0.5f;
                }

                effect->unk_30 = sin_s(effect->unk_2A * 0xBB8) * M_PI * 0.2f;

                if (effect->life < effect->timer) {
                    effect->type = 0;
                }
            }
        }
    }
}

void niw_eff_disp(EnNiw* this, PlayState* play) {
    u8 materialFlag = 0;
    s16 i;
    s32 pad;
    GraphicsContext* gfxCtx = play->state.gfxCtx;
    EnNiwEffect* effect = &this->effects[0];

    OPEN_DISPS(gfxCtx, "../z_en_niw.c", 1897);

    _texture_z_light_fog_prim_xlu(play->state.gfxCtx);

    for (i = 0; i < EN_NIW_EFFECT_COUNT; i++, effect++) {
        if (effect->type == 1) {
            if (materialFlag == 0) {
                gSPDisplayList(POLY_XLU_DISP++, gCuccoEffectFeatherMaterialDL);
                materialFlag++;
            }
            Matrix_translate(effect->pos.x, effect->pos.y, effect->pos.z, MTXMODE_NEW);
            Matrix_rotate_scale_exchange(&play->billboardMtxF);
            Matrix_scale(effect->scale, effect->scale, 1.0f, MTXMODE_APPLY);
            Matrix_rotateZ(effect->unk_30, MTXMODE_APPLY);
            Matrix_translate(0.0f, -1000.0f, 0.0f, MTXMODE_APPLY);
            MATRIX_FINALIZE_AND_LOAD(POLY_XLU_DISP++, gfxCtx, "../z_en_niw.c", 1913);
            gSPDisplayList(POLY_XLU_DISP++, gCuccoEffectFeatherModelDL);
        }
    }

    CLOSE_DISPS(gfxCtx, "../z_en_niw.c", 1919);
}
