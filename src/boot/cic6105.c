...
OSTask D_800067C0_unknown = {
    4, 0, rspbootTextStart, 0x3E8, cic6105TextStart, 0x20, (u64*)__Creator__, 8, NULL, 0, NULL, 0, NULL, 0, NULL, 0,
};
...
void CIC6105_FaultClient(void) {
    ...
    faultprint_Locate(48, 200);
    if (spStatus & SP_STATUS_SIG7) {
        faultprint_Printf("OCARINA %08x %08x", B_80008EF8, B_80008EFC);
    } else {
        faultprint_Printf("LEGEND %08x %08x", B_80008EF8, B_80008EFC);
    }
    faultprint_Locate(40, 184);
    faultprint_Printf("ROM_F");
    faultprint_Printf(" [Creator:%s]", __Creator__);
    faultprint_Locate(56, 192);
    faultprint_Printf("[Date:%s]", __DateTime__);
#if OOT_VERSION < PAL_1_0
    faultprint_Locate(128, 24);
#else
    faultprint_Locate(96, 32);
#endif
    faultprint_Printf("I LOVE YOU %08x", func_80001714());
}
...
void func_80001640(void) {
    ...
    osSendMesg(&_sched.cmdQueue, &sp38, OS_MESG_BLOCK);
