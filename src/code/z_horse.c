 ...
 */
s32 Horse_checkOKSpot(s32 sceneId) {
 ...
 */
void Horse_goto_Default(PlayState* play) {
 ...
 */
void Horse_escape_Spot06Water(PlayState* play) {
    if (z_common_data.save.info.horseData.sceneId == SCENE_LAKE_HYLIA) {
        z_common_data.save.info.horseData.sceneId = SCENE_LAKE_HYLIA;
        z_common_data.save.info.horseData.pos.x = -2065;
        z_common_data.save.info.horseData.pos.y = -863;
        z_common_data.save.info.horseData.pos.z = 1839;
        z_common_data.save.info.horseData.angle = 0;
    }
}
...
void Horse_SetNormal(PlayState* play, Player* player) {
    ...
    if (R_EXITED_SCENE_RIDING_HORSE &&
        ...
        player->rideActor =
            Actor_info_make_actor(&play->actorCtx, play, ACTOR_EN_HORSE, player->actor.world.pos.x, player->actor.world.pos.y,
        ...
        player_ride_horse_init(play, player, player->rideActor);
        player_make_horse_camera(play, player);
        z_common_data.save.info.horseData.sceneId = play->sceneId;
        if (play->sceneId == SCENE_GERUDOS_FORTRESS) {
            player->rideActor->room = -1;
        }
    } else if ((play->sceneId == SCENE_GERUDOS_FORTRESS) && (z_common_data.minigameState == 3)) {
        // Completed Horseback Archery Minigame
        Actor* horseActor;
        z_common_data.minigameState = 0;
        horseActor =
            Actor_info_make_actor(&play->actorCtx, play, ACTOR_EN_HORSE, 3586.0f, 1413.0f, -402.0f, 0, 0x4000, 0, HORSE_PTYPE_1);
        horseActor->room = -1;
    } else if ((z_common_data.save.entranceIndex == ENTR_LON_LON_RANCH_7) &&
               GET_EVENTCHKINF(EVENTCHKINF_EPONA_OBTAINED)) {
        // Completed Horse Race
        Actor* horseActor =
            Actor_info_make_actor(&play->actorCtx, play, ACTOR_EN_HORSE, -25.0f, 0.0f, -1600.0f, 0, -0x4000, 0, HORSE_PTYPE_1);
        ASSERT(horseActor != NULL, "horse_actor != NULL", "../z_horse.c", 389);
    } else if ((play->sceneId == z_common_data.save.info.horseData.sceneId) &&
               (event_check(EVENTCHKINF_EPONA_OBTAINED) || R_DEBUG_FORCE_EPONA_OBTAINED)) {
        // Player enters a scene where the horse was left previously
        PRINTF(T("馬存在によるセット %d %d %d\n", "Set by existence of horse %d %d %d\n"),
               z_common_data.save.info.horseData.sceneId, event_check(EVENTCHKINF_EPONA_OBTAINED),
               R_DEBUG_FORCE_EPONA_OBTAINED);

        if (Horse_checkOKSpot(z_common_data.save.info.horseData.sceneId)) {
            Actor* horseActor =
                Actor_info_make_actor(&play->actorCtx, play, ACTOR_EN_HORSE, z_common_data.save.info.horseData.pos.x,
                            z_common_data.save.info.horseData.pos.y, z_common_data.save.info.horseData.pos.z, 0,
                            z_common_data.save.info.horseData.angle, 0, HORSE_PTYPE_1);
            ...
            PRINTF(
                ...
                z_common_data.save.info.horseData.sceneId);
            PRINTF_RST();
        ...
        Actor* horseActor =
            Actor_info_make_actor(&play->actorCtx, play, ACTOR_EN_HORSE, 0.0f, 0.0f, -500.0f, 0, 0, 0, HORSE_PTYPE_1);
        ...
        for (i = 0; i < ARRAY_COUNT(horse_set_pos); i++) {
            HorseSpawn* horseSpawn = &horse_set_pos[i];
            if (horseSpawn->sceneId == play->sceneId) {
                Actor* horseActor =
                    Actor_info_make_actor(&play->actorCtx, play, ACTOR_EN_HORSE, horseSpawn->pos.x, horseSpawn->pos.y,
        ...
        Actor_info_make_actor(&play->actorCtx, play, ACTOR_EN_HORSE, 0.0f, 0.0f, -60.0f, 0, 0x7360, 0, HORSE_PTYPE_1);
    }
}
...
void Horse_SetDemo(PlayState* play, Player* player) {
    static HorseCutsceneSpawn horse_set_pos[] = {
    ...
    if ((z_common_data.save.entranceIndex == ENTR_HYRULE_FIELD_11 ||
        ...
        } else if (z_common_data.save.entranceIndex == ENTR_HYRULE_FIELD_12) {
            spawnPos = spawnPositions[1];
            // East of Lon Lon player spawn
        } else if (z_common_data.save.entranceIndex == ENTR_HYRULE_FIELD_13) {
        ...
        player->rideActor = Actor_info_make_actor(&play->actorCtx, play, ACTOR_EN_HORSE, spawnPos.x, spawnPos.y, spawnPos.z, 0,
        ...
        z_common_data.save.info.horseData.sceneId = play->sceneId;
               ...
               !(event_check(EVENTCHKINF_EPONA_OBTAINED) || R_DEBUG_FORCE_EPONA_OBTAINED)) {
        player->rideActor =
            Actor_info_make_actor(&play->actorCtx, play, ACTOR_EN_HORSE, 894.0f, 0.0f, -2084.0f, 0, -0x7FFF, 0, HORSE_PTYPE_5);
        ...
        for (i = 0; i < ARRAY_COUNT(horse_set_pos); i++) {
            if ((play->sceneId == horse_set_pos[i].sceneId) &&
                (((void)0, z_common_data.save.cutsceneIndex) == horse_set_pos[i].cutsceneIndex)) {
                if (horse_set_pos[i].type == HORSE_PTYPE_7) {
                    if ((play->sceneId == SCENE_LON_LON_RANCH) &&
                        (((void)0, z_common_data.save.cutsceneIndex) == 0xFFF1)) {
                        horse_set_pos[i].pos.x = player->actor.world.pos.x;
                        horse_set_pos[i].pos.y = player->actor.world.pos.y;
                        horse_set_pos[i].pos.z = player->actor.world.pos.z;
                    }

                    player->rideActor =
                        Actor_info_make_actor(&play->actorCtx, play, ACTOR_EN_HORSE, horse_set_pos[i].pos.x, horse_set_pos[i].pos.y,
                                    horse_set_pos[i].pos.z, 0, player->actor.world.rot.y, 0, horse_set_pos[i].type);
                    ASSERT(player->rideActor != NULL, "player->ride.actor != NULL", "../z_horse.c", 628);

                    player_ride_horse_init(play, player, player->rideActor);
                    player_make_horse_camera(play, player);
                } else if ((horse_set_pos[i].type == HORSE_PTYPE_5) || (horse_set_pos[i].type == HORSE_PTYPE_6) ||
                           (horse_set_pos[i].type == HORSE_PTYPE_HORSEBACK_ARCHERY)) {
                    Vec3f eye;
                    s32 paramFlag = 0;

                    if (GET_EVENTINF_INGO_RACE_HORSETYPE() != HORSE_EPONA && horse_set_pos[i].type == HORSE_PTYPE_6) {
                        // HORSE_HNI
                        paramFlag = 0x8000;
                    }
                    ...
                    player->rideActor =
                        Actor_info_make_actor(&play->actorCtx, play, ACTOR_EN_HORSE, horse_set_pos[i].pos.x, horse_set_pos[i].pos.y,
                                    horse_set_pos[i].pos.z, 0, horse_set_pos[i].angle, 0, horse_set_pos[i].type | paramFlag);
                    ASSERT(player->rideActor != NULL, "player->ride.actor != NULL", "../z_horse.c", 667);

                    player->actor.world.pos.x = horse_set_pos[i].pos.x;
                    player->actor.world.pos.y = horse_set_pos[i].pos.y;
                    player->actor.world.pos.z = horse_set_pos[i].pos.z;
                    player->actor.shape.rot.x = player->actor.shape.rot.z = 0;
                    player->actor.shape.rot.y = horse_set_pos[i].angle;
                    ...
                    Gama_play_camera_setting(play, play->activeCamId, &player->actor.world.pos, &eye);
                } else {
                    Actor_info_make_actor(&play->actorCtx, play, ACTOR_EN_HORSE, horse_set_pos[i].pos.x, horse_set_pos[i].pos.y,
                                horse_set_pos[i].pos.z, 0, horse_set_pos[i].angle, 0, horse_set_pos[i].type);
                }
                break;
            }
        }
 ...
 */
void Horse_Set_Check(PlayState* play, Player* player) {
    if (LINK_IS_ADULT) {
        if (!Horse_checkOKSpot(z_common_data.save.info.horseData.sceneId)) {
            PRINTF_COLOR_ERROR();
            PRINTF(
        ...
        if (Horse_checkOKSpot(play->sceneId)) {
            if (IS_CUTSCENE_LAYER ||
                // has hopped over one of the Lon-Lon Ranch fences
                ((z_common_data.save.entranceIndex == ENTR_HYRULE_FIELD_11 ||
                  z_common_data.save.entranceIndex == ENTR_HYRULE_FIELD_12 ||
                  z_common_data.save.entranceIndex == ENTR_HYRULE_FIELD_13 ||
                  z_common_data.save.entranceIndex == ENTR_HYRULE_FIELD_15) &&
                 (z_common_data.respawnFlag == 0)) ||
...
void horse_rot_trace_pos(Actor* actor, Vec3f* pos, s16 turnAmount) {
    s16 x = search_position_angleY(&actor->world.pos, pos) - actor->world.rot.y;
