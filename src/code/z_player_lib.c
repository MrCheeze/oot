...
void func_8008F470(GlobalContext* globalCtx, Skeleton* skeleton, Vec3s* limbDrawTable, s32 dListCount, s32 dListIndex,
    ...
    Graph_OpenDisps(dispRefs, globalCtx->state.gfxCtx, "../z_player_lib.c", 1721);

    if (sp80 < 0) {
        sp80 = D_80125FB8[arg7][0];
    }

    gSPSegment(gfxCtx->polyOpa.p++, 0x08, SEGMENTED_TO_VIRTUAL(D_80125FD8[sp80]));

    if (sp7C < 0) {
        sp7C = D_80125FB8[arg7][1];
    }

    gSPSegment(gfxCtx->polyOpa.p++, 0x09, SEGMENTED_TO_VIRTUAL(D_80125FF8[sp7C]));

    color = &D_80126008[arg5];
    gDPSetEnvColor(gfxCtx->polyOpa.p++, color->r, color->g, color->b, 0x00);
    ...
    if ((overrideLimbDraw != func_800902F0) && (overrideLimbDraw != func_80090440) && (gSaveContext.gameMode != 3)) {
        if (LINK_IS_ADULT) {
            ...
            if (strengthUpg >= 2) {
                ...
                gSPDisplayList(gfxCtx->polyOpa.p++, D_06025218);
                gSPDisplayList(gfxCtx->polyOpa.p++, D_06025598);
                gSPDisplayList(gfxCtx->polyOpa.p++, (D_80160014 == 0) ? D_060252D8 : D_06025438);
                gSPDisplayList(gfxCtx->polyOpa.p++, (D_80160018 == 8) ? D_06025658 : D_060257B8);
            }

            if (arg6 != 0) {
                Gfx** ptr = D_8012601C[arg6 - 1];
                gSPDisplayList(gfxCtx->polyOpa.p++, ptr[0]);
                gSPDisplayList(gfxCtx->polyOpa.p++, ptr[1]);
            }
        } else {
            if (func_8008F034() > 0) {
                gSPDisplayList(gfxCtx->polyOpa.p++, D_06016118);
            }
        }
    }

    Graph_CloseDisps(dispRefs, globalCtx->state.gfxCtx, "../z_player_lib.c", 1803);
}
...
void func_800907E4(GlobalContext* globalCtx, Player* player, Vec3f* arg2, s32 arg3) {
    ...
    Graph_OpenDisps(dispRefs, globalCtx->state.gfxCtx, "../z_player_lib.c", 2401);

    gSegments[6] = PHYSICAL_TO_VIRTUAL(player->giObjectSegment);

    gSPSegment(gfxCtx->polyOpa.p++, 0x06, player->giObjectSegment);
    gSPSegment(gfxCtx->polyXlu.p++, 0x06, player->giObjectSegment);
    ...
    Graph_CloseDisps(dispRefs, globalCtx->state.gfxCtx, "../z_player_lib.c", 2421);
}
...
void func_80090AFC(GlobalContext* globalCtx, Player* player, f32 arg2) {
    ...
    if (func_8003E188(&globalCtx->colCtx, &sp8C, &sp80, &sp74, &sp9C, 1, 1, 1, 1, &sp98) != 0) {
        ...
        Graph_OpenDisps(dispRefs, globalCtx->state.gfxCtx, "../z_player_lib.c", 2572);
        ...
        gSPMatrix(gfxCtx->overlay.p++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_player_lib.c", 2587),
        ...
        Graph_CloseDisps(dispRefs, globalCtx->state.gfxCtx, "../z_player_lib.c", 2592);
    }
}
...
void func_80090D20(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3s* rot, Actor* actor) {
    ...
    if (limbIndex == 16) {
        ...
        if (player->unk_154 == 6) {
            ...
            Graph_OpenDisps(dispRefs, globalCtx->state.gfxCtx, "../z_player_lib.c", 2633);
            ...
            gSPMatrix(gfxCtx->polyOpa.p++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_player_lib.c", 2653),
                      G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
            gSPDisplayList(gfxCtx->polyOpa.p++, D_06006CC0);

            Graph_CloseDisps(dispRefs, globalCtx->state.gfxCtx, "../z_player_lib.c", 2656);
            ...
            Graph_OpenDisps(dispRefs, globalCtx->state.gfxCtx, "../z_player_lib.c", 2710);

            gSPMatrix(gfxCtx->polyXlu.p++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_player_lib.c", 2712),
                      G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
            gDPSetEnvColor(gfxCtx->polyXlu.p++, spE0->r, spE0->g, spE0->b, 0x00);
            gSPDisplayList(gfxCtx->polyXlu.p++, D_801260F8[gSaveContext.linkAge]);

            Graph_CloseDisps(dispRefs, globalCtx->state.gfxCtx, "../z_player_lib.c", 2717);
        }
        ...
        if (player->unk_15D == 0xFF) {
            ...
            Graph_OpenDisps(dispRefs, globalCtx->state.gfxCtx, "../z_player_lib.c", 2783);
            ...
            gSPMatrix(gfxCtx->polyXlu.p++, Matrix_NewMtx(globalCtx->state.gfxCtx, "../z_player_lib.c", 2804),
                      G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
            gSPDisplayList(gfxCtx->polyXlu.p++, spB0->dList);

            Matrix_Pull();

            Graph_CloseDisps(dispRefs, globalCtx->state.gfxCtx, "../z_player_lib.c", 2809);
...
void func_80091A24(GlobalContext* globalCtx, void* seg04, void* seg06, struct_80091A24_arg3* arg3, Vec3f* pos,
    ...
    Graph_OpenDisps(dispRefs, globalCtx->state.gfxCtx, "../z_player_lib.c", 3129);

    opaRef = gfxCtx->polyOpa.p;
    gfxCtx->polyOpa.p++;

    xluRef = gfxCtx->polyXlu.p;
    gfxCtx->polyXlu.p++;

    gSPDisplayList(gfxCtx->work.p++, gfxCtx->polyOpa.p);
    gSPDisplayList(gfxCtx->work.p++, gfxCtx->polyXlu.p);

    gSPSegment(gfxCtx->polyOpa.p++, 0x00, NULL);

    gDPPipeSync(gfxCtx->polyOpa.p++);

    gSPLoadGeometryMode(gfxCtx->polyOpa.p++, 0);
    gSPTexture(gfxCtx->polyOpa.p++, 0xFFFF, 0xFFFF, 0, G_TX_RENDERTILE, G_OFF);
    gDPSetCombineMode(gfxCtx->polyOpa.p++, G_CC_SHADE, G_CC_SHADE);
    gDPSetOtherMode(gfxCtx->polyOpa.p++,
                    G_AD_DISABLE | G_CD_MAGICSQ | G_CK_NONE | G_TC_FILT | G_TF_BILERP | G_TT_NONE | G_TL_TILE |
                        G_TD_CLAMP | G_TP_PERSP | G_CYC_FILL | G_PM_NPRIMITIVE,
                    G_AC_NONE | G_ZS_PIXEL | G_RM_NOOP | G_RM_NOOP2);
    gSPLoadGeometryMode(gfxCtx->polyOpa.p++, G_ZBUFFER | G_SHADE | G_CULL_BACK | G_LIGHTING | G_SHADING_SMOOTH);
    gDPSetScissorFrac(gfxCtx->polyOpa.p++, G_SC_NON_INTERLACE, 0, 0, width * 4.0f, height * 4.0f);
    gSPClipRatio(gfxCtx->polyOpa.p++, FRUSTRATIO_1);

    gDPSetColorImage(gfxCtx->polyOpa.p++, G_IM_FMT_RGBA, G_IM_SIZ_16b, width, img2);
    ...
    gDPFillRectangle(gfxCtx->polyOpa.p++, 0, 0, width - 1, height - 1);

    gDPPipeSync(gfxCtx->polyOpa.p++);

    gDPSetColorImage(gfxCtx->polyOpa.p++, G_IM_FMT_RGBA, G_IM_SIZ_16b, width, img1);
    gDPSetCycleType(gfxCtx->polyOpa.p++, G_CYC_FILL);
    gDPSetRenderMode(gfxCtx->polyOpa.p++, G_RM_NOOP, G_RM_NOOP2);
    gDPSetFillColor(gfxCtx->polyOpa.p++,
    ...
    gDPSetDepthImage(gfxCtx->polyOpa.p++, img2);

    D_80126200.vp.vscale[0] = D_80126200.vp.vtrans[0] = width * 2;
    D_80126200.vp.vscale[1] = D_80126200.vp.vtrans[1] = height * 2;
    gSPViewport(gfxCtx->polyOpa.p++, &D_80126200);

    guPerspective(perspMtx, &perspNorm, fovy, (f32)width / (f32)height, 10.0f, 4000.0f, 1.0f);

    gSPPerspNormalize(gfxCtx->polyOpa.p++, perspNorm);
    gSPMatrix(gfxCtx->polyOpa.p++, perspMtx, G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_PROJECTION);

    guLookAt(lookAtMtx, eye->x, eye->y, eye->z, at->x, at->y, at->z, 0.0f, 1.0f, 0.0f);

    gSPMatrix(gfxCtx->polyOpa.p++, lookAtMtx, G_MTX_NOPUSH | G_MTX_MUL | G_MTX_PROJECTION);
    ...
    gSPSegment(gfxCtx->polyOpa.p++, 0x04, seg04);
    gSPSegment(gfxCtx->polyOpa.p++, 0x06, seg06);

    gSPSetLights1(gfxCtx->polyOpa.p++, D_80126210);

    func_80093C80(globalCtx);

    gfxCtx->polyOpa.p = Gfx_SetFog2(gfxCtx->polyOpa.p++, 0, 0, 0, 0, 997, 1000);
    ...
    gSPBranchList(opaRef, gfxCtx->polyOpa.p);
    gSPBranchList(xluRef, gfxCtx->polyXlu.p);

    Graph_CloseDisps(dispRefs, globalCtx->state.gfxCtx, "../z_player_lib.c", 3288);
}
