...
u16 short_SLIST_TBL_allocIndex(DynaSSNodeList* nodeList);
void MeshTableCalcWorldPosToMeshPos(CollisionContext* colCtx, Vec3f* pos, Vec3i* sector);
s32 T_BGCheck_pos_check(CollisionContext* colCtx, Vec3f* pos);
s32 T_BGCheck_GeneralLineCheck(CollisionContext* colCtx, u16 xpFlags1, u16 xpFlags2, Vec3f* posA, Vec3f* posB,
                          Vec3f* posResult, CollisionPoly** outPoly, s32* outBgId, Actor* actor, f32 chkDist,
                          u32 bccFlags);
void T_BGCheckAlloc_ct(SSNodeList* this);
void T_BGCheckAlloc_Initialize(PlayState* play, SSNodeList* this, s32 tblMax, s32 numPolys);
u16 T_BGCheckAlloc_getShortSlistNode_index(SSNodeList* this);
void DynaPolyInfo_ct(PlayState* play, DynaCollisionContext* dyna);
void DynaPolyInfo_Init(PlayState* play, DynaCollisionContext* dyna);
f32 DynaPolyInfo_groundCheck(DynaRaycastDown* dynaRaycastDown);
s32 DynaPolyInfo_wallCheck(CollisionContext* colCtx, u16 xpFlags, f32* outX, f32* outZ, Vec3f* pos, f32 radius,
                          CollisionPoly** outPoly, s32* outBgId, Actor* actor);
s32 DynaPolyInfo_roofCheck(CollisionContext* colCtx, u16 xpFlags, f32* outY, Vec3f* pos, f32 chkDist,
                             CollisionPoly** outPoly, s32* outBgId, Actor* actor);
s32 DynaPolyInfo_lineCheck(CollisionContext* colCtx, u16 xpFlags, Vec3f* posA, Vec3f* posB, Vec3f* posResult,
                                 CollisionPoly** outPoly, f32* distSq, s32* outBgId, Actor* actor, f32 chkDist,
                                 s32 bccFlags);
s32 DynaPolyInfo_simpleCheck(CollisionContext* colCtx, u16 xpFlags, CollisionPoly** outPoly, s32* outBgId,
                               Vec3f* center, f32 radius, Actor* actor, u16 bciFlags);
void T_BGCheckAlloc_clearPolygonCheck(SSNodeList* nodeList, s32 numPolys);
...
s32 Bg_Wall_Code[WALL_TYPE_MAX] = {
    0,                         // WALL_TYPE_0
...
u16 Bg_PolygonBgSNDCode2SoundGroundLabel[SURFACE_MATERIAL_MAX] = {
 ...
 * Set SSNode
 */
void short_SLIST_NODE_ct(SSNode* node, s16* polyId, u16 next) {
    node->polyId = *polyId;
    node->next = next;
}
 ...
 */
void short_SLIST_ct(SSList* ssList) {
    ssList->head = SS_NULL;
}
 ...
 */
void short_SLIST_Prepend(SSNodeList* nodeList, SSList* ssList, s16* polyId) {
    u16 newNodeId = T_BGCheckAlloc_getShortSlistNode_index(nodeList);

    short_SLIST_NODE_ct(&nodeList->tbl[newNodeId], polyId, ssList->head);
 ...
 */
void short_SLIST_Prepend_sst(DynaSSNodeList* nodeList, SSList* ssList, s16* polyId) {
    u16 newNodeId = short_SLIST_TBL_allocIndex(nodeList);
 ...
 */
void short_SLIST_TBL_ct(PlayState* play, DynaSSNodeList* nodeList) {
    nodeList->tbl = NULL;
    nodeList->count = 0;
}
 ...
 */
void short_SLIST_TBL_Init(PlayState* play, DynaSSNodeList* nodeList, s32 max) {
    nodeList->tbl = THA_allocAlign(&play->state.tha, max * sizeof(SSNode), ALIGNOF_MASK(SSNode));
 ...
 */
void short_SLIST_TBL_cleanUp(DynaSSNodeList* nodeList) {
    nodeList->count = 0;
}
 ...
 */
void T_Vertex_ToXyzT(Vec3s* src, Vec3f* dst) {
    dst->x = src->x;
    dst->y = src->y;
    dst->z = src->z;
}
 ...
 */
void T_Vertex_FromXyzT(Vec3s* dst, Vec3f* src) {
    dst->x = src->x;
    dst->y = src->y;
    dst->z = src->z;
}
 ...
 */
s16 T_Polygon_getMinY(CollisionPoly* poly, Vec3s* vtxList) {
 ...
 */
void T_Polygon_getABC(CollisionPoly* poly, f32* nx, f32* ny, f32* nz) {
 ...
 */
void T_Polygon_Ground_Matrix(CollisionPoly* poly, f32 tx, f32 ty, f32 tz, MtxF* dest) {
    f32 nx;
    f32 ny;
    f32 nz;
    s32 pad;
    ...
    T_Polygon_getABC(poly, &nx, &ny, &nz);
...
/**
 * Calculate point distance from planef along normal
 */
f32 T_Polygon_PlaneFunc(CollisionPoly* poly, Vec3f* point) {
 ...
 * Get Poly Vertices
 */
void T_Polygon_GetVertex_vertex(CollisionPoly* poly, Vec3s* vtxList, Vec3f* dest) {
    T_Vertex_ToXyzT(&vtxList[COLPOLY_VTX_INDEX(poly->flags_vIA)], &dest[0]);
    T_Vertex_ToXyzT(&vtxList[COLPOLY_VTX_INDEX(poly->flags_vIB)], &dest[1]);
    T_Vertex_ToXyzT(&vtxList[poly->vIC], &dest[2]);
}
 ...
 */
void T_Polygon_GetVertex_bg_ai(CollisionPoly* poly, s32 bgId, CollisionContext* colCtx, Vec3f* dest) {
    Vec3s* vtxList;
    ...
    if (poly == NULL || bgId > BG_ACTOR_MAX || dest == NULL) {
        ...
        T_Polygon_GetVertex_vertex(poly, vtxList, dest);
    }
}
 ...
 */
s32 T_Polygon_CrossYLine_sukimaR(CollisionPoly* poly, Vec3s* vtxList, f32 x, f32 z, f32* yIntersect,
                                         f32 chkDist) {
    static Vec3f tri_pos[3];
    f32 nx;
    f32 ny;
    f32 nz;
    Vec3s* vA;
    ...
    xyz_t_move_s_xyz(&tri_pos[0], vA);
    vB = &vtxList[COLPOLY_VTX_INDEX(poly->flags_vIB)];
    xyz_t_move_s_xyz(&tri_pos[1], vB);
    vC = &vtxList[poly->vIC];
    xyz_t_move_s_xyz(&tri_pos[2], vC);

    nx = COLPOLY_GET_NORMAL(poly->normal.x);
    ny = COLPOLY_GET_NORMAL(poly->normal.y);
    nz = COLPOLY_GET_NORMAL(poly->normal.z);

    return Math3DTriangleCrossYLine_sukimaR(&tri_pos[0], &tri_pos[1], &tri_pos[2], nx, ny, nz, poly->dist, z,
 ...
 */
s32 T_Polygon_CrossYLine_sukimaR_VP0(CollisionPoly* poly, Vec3s* vtxList, f32 x, f32 z, f32* yIntersect, f32 chkDist) {
    static Vec3f tri_pos[3];
    f32 nx;
    f32 ny;
    f32 nz;

    T_Polygon_GetVertex_vertex(poly, vtxList, tri_pos);
    T_Polygon_getABC(poly, &nx, &ny, &nz);
    return Math3DTriangleCrossYLine_sukimaR_VP0(&tri_pos[0], &tri_pos[1], &tri_pos[2], nx, ny, nz,
 ...
 * Determinant max 300.0f
 */
s32 T_Polygon_CrossYLine(CollisionPoly* poly, Vec3s* vtxList, f32 x, f32 z, f32* yIntersect) {
    return T_Polygon_CrossYLine_sukimaR(poly, vtxList, x, z, yIntersect, 1.0f);
}
 ...
 * Determinant max 300.0f
 */
s32 T_Polygon_CrossXLine(CollisionPoly* poly, Vec3s* vtxList, f32 y, f32 z, f32* xIntersect) {
    static Vec3f tri_pos[3];
    f32 nx;
    f32 ny;
    f32 nz;
    ...
    T_Polygon_GetVertex_vertex(poly, vtxList, tri_pos);
    T_Polygon_getABC(poly, &nx, &ny, &nz);
    return Math3DTriangleCrossXLine(&tri_pos[0], &tri_pos[1], &tri_pos[2], nx, ny, nz, poly->dist, y, z,
                                            xIntersect);
}
 ...
 * Determinant max 300.0f
 */
s32 T_Polygon_CrossZLine(CollisionPoly* poly, Vec3s* vtxList, f32 x, f32 y, f32* zIntersect) {
    static Vec3f tri_pos[3];
    f32 nx;
    f32 ny;
    f32 nz;
    ...
    T_Polygon_GetVertex_vertex(poly, vtxList, tri_pos);
    T_Polygon_getABC(poly, &nx, &ny, &nz);
    return Math3DTriangleCrossZLine(&tri_pos[0], &tri_pos[1], &tri_pos[2], nx, ny, nz, poly->dist, x, y,
                                            zIntersect);
}
 ...
 * returns `planeIntersect`, which is the point at which the line from `posA` to `posB` crosses `poly`'s planef
 * if `chkOneFace` is true, return false (no intersection) when going through the poly from A to B is done in the
 * normal's direction
 */
s32 T_Polygon_CrossLine(CollisionPoly* poly, Vec3s* vtxList, Vec3f* posA, Vec3f* posB, Vec3f* planeIntersect,
    ...
    T_Polygon_getABC(poly, &planef.normal.x, &planef.normal.y, &planef.normal.z);
    T_Polygon_GetVertex_vertex(poly, vtxList, tri_pos);
    Math3DInDivPos2(posA, posB, planeDistA / planeDistDelta, planeIntersect);
    if ((fabsf(planef.normal.x) > 0.5f &&
         Math3DTrianglePFCrossXLine_noCross_sukimaR(&tri_pos[0], &tri_pos[1], &tri_pos[2], &planef, planeIntersect->y,
                                     planeIntersect->z, chkDist)) ||
        (fabsf(planef.normal.y) > 0.5f &&
         Math3DTrianglePFCrossYLine_noCross_sukimaR(&tri_pos[0], &tri_pos[1], &tri_pos[2], &planef, planeIntersect->z,
                                     planeIntersect->x, chkDist)) ||
        (fabsf(planef.normal.z) > 0.5f &&
         Math3DTrianglePFCrossZLine_noCross_sukimaR(&tri_pos[0], &tri_pos[1], &tri_pos[2], &planef, planeIntersect->x,
 ...
 */
s32 T_Polygon_simpleCheck(CollisionPoly* poly, Vec3s* vtxList, Vec3f* center, f32 radius) {
    static Sphere16 sphere;
    static TriNorm tri;
    Vec3f intersect;

    T_Polygon_GetVertex_vertex(poly, vtxList, tri.vtx);
    T_Polygon_getABC(poly, &tri.plane.normal.x, &tri.plane.normal.y, &tri.plane.normal.z);
    tri.plane.originDist = poly->dist;
    sphere.center.x = center->x;
    sphere.center.y = center->y;
    sphere.center.z = center->z;
    sphere.radius = radius;
    return Math3D_sphereCrossTriangle3_cp(&sphere, &tri, &intersect);
}
 ...
 */
void T_Mesh_sort_add_poly(CollisionContext* colCtx, SSList* ssList, CollisionPoly* polyList, Vec3s* vtxList,
                                  s16 polyId) {
    SSNode* curNode;
    SSNode* nextNode;
    s32 polyYMin;
    ...
    if (ssList->head == SS_NULL) {
        short_SLIST_Prepend(&colCtx->polyNodes, ssList, &polyId);
        return;
    }

    polyYMin = T_Polygon_getMinY(&polyList[polyId], vtxList);
    ...
    while (true) {
        ...
        if (curNode->next == SS_NULL) {
            ...
            newNodeId = T_BGCheckAlloc_getShortSlistNode_index(&colCtx->polyNodes);
            short_SLIST_NODE_ct(&colCtx->polyNodes.tbl[newNodeId], &polyId, SS_NULL);
        ...
        if (polyYMin < vtxList[COLPOLY_VTX_INDEX(polyList[curPolyId].flags_vIA)].y &&
            ...
            polyYMin < vtxList[polyList[curPolyId].vIC].y) {
            newNodeId = T_BGCheckAlloc_getShortSlistNode_index(&colCtx->polyNodes);
            short_SLIST_NODE_ct(&colCtx->polyNodes.tbl[newNodeId], &polyId, curNode->next);
            curNode->next = newNodeId;
            return;
        }
        curNode = nextNode;
 ...
 */
void T_Mesh_add_poly_index(StaticLookup* lookup, CollisionContext* colCtx, CollisionPoly* polyList, Vec3s* vtxList,
                          s16 index) {
    if (polyList[index].normal.y > COLPOLY_SNORMAL(0.5f)) {
        T_Mesh_sort_add_poly(colCtx, &lookup->floor, polyList, vtxList, index);
    } else if (polyList[index].normal.y < COLPOLY_SNORMAL(-0.8f)) {
        T_Mesh_sort_add_poly(colCtx, &lookup->ceiling, polyList, vtxList, index);
    } else {
        T_Mesh_sort_add_poly(colCtx, &lookup->wall, polyList, vtxList, index);
    }
}
 ...
 */
f32 T_Mesh_groundCheckList(CollisionContext* colCtx, u16 xpFlags, SSList* ssList, CollisionPoly** outPoly,
    ...
    while (true) {
        ...
        if (T_Polygon_CrossYLine_sukimaR_VP0(&colCtx->colHeader->polyList[polyId], colCtx->colHeader->vtxList, pos->x,
 ...
 * stores the pointer of the closest poly to `outPoly`
 */
f32 T_Mesh_ground_check(StaticLookup* lookup, CollisionContext* colCtx, u16 xpFlags, CollisionPoly** poly,
    ...
    if (downChkFlags & BGCHECK_RAYCAST_DOWN_CHECK_FLOORS) {
        yIntersect = T_Mesh_groundCheckList(colCtx, xpFlags, &lookup->floor, poly, pos, yIntersect, chkDist, 0);
    }
    ...
    if ((downChkFlags & BGCHECK_RAYCAST_DOWN_CHECK_WALLS) || (downChkFlags & BGCHECK_RAYCAST_DOWN_CHECK_WALLS_SIMPLE)) {
        ...
        yIntersect =
            T_Mesh_groundCheckList(colCtx, xpFlags, &lookup->wall, poly, pos, yIntersect, chkDist, groundChk);
    }
    ...
    if (downChkFlags & BGCHECK_RAYCAST_DOWN_CHECK_CEILINGS) {
        ...
        yIntersect =
            T_Mesh_groundCheckList(colCtx, xpFlags, &lookup->ceiling, poly, pos, yIntersect, chkDist, groundChk);
    }
 ...
 */
s32 T_BGCheck_positionWallCollection(CollisionContext* colCtx, CollisionPoly* poly, f32* posX, f32* posZ, f32 nx, f32 ny,
 ...
 */
s32 T_Mesh_wall_check(StaticLookup* lookup, CollisionContext* colCtx, u16 xpFlags, f32* outX, f32* outZ,
    ...
    while (true) {
        ...
        planeDist = Math3DSignedLengthPlaneAndPos(nx, ny, nz, curPoly->dist, &resultPos);
        ...
        if (T_Polygon_CrossZLine(curPoly, vtxList, resultPos.x, pos->y, &intersect)) {
            f32 zIntersectDist = intersect - resultPos.z;

            if (fabsf(zIntersectDist) <= radius / temp_f16) {
                if (zIntersectDist * nz <= 4.0f) {
                    T_BGCheck_positionWallCollection(colCtx, curPoly, &resultPos.x, &resultPos.z, nx, ny, nz,
    ...
    while (true) {
        ...
        if (T_Polygon_CrossXLine(curPoly, vtxList, pos->y, resultPos.z, &intersect)) {
 ...
 */
s32 T_Mesh_roof_check(StaticLookup* lookup, u16 xpFlags, CollisionContext* colCtx, f32* outY, Vec3f* pos,
    ...
    *outY = pos->y;
    ...
    while (true) {
        ...
        if (T_Polygon_CrossYLine(curPoly, vtxList, pos->x, pos->z, &ceilingY)) {
 ...
 */
s32 T_Mesh_line_check_proc(SSList* ssList, CollisionContext* colCtx, u16 xpFlags1, u16 xpFlags2, Vec3f* posA,
    ...
    while (true) {
        ...
        *checkedPoly = true;
        curPoly = &polyList[polyId];
        minY = T_Polygon_getMinY(curPoly, colCtx->colHeader->vtxList);
        if (posA->y < minY && posB->y < minY) {
            break;
        }
        if (T_Polygon_CrossLine(curPoly, colCtx->colHeader->vtxList, posA, posB, &polyIntersect,
                                     (bccFlags & BGCHECK_CHECK_ONE_FACE) != 0, chkDist)) {
            distSq = Math3DLengthSquare(posA, &polyIntersect);
            if (distSq < *outDistSq) {
 ...
 * `outDistSq` returns the squared distance from `posA` to the point of intersect
 */
s32 T_Mesh_line_check_poly_chgrp(StaticLookup* lookup, CollisionContext* colCtx, u16 xpFlags1, u16 xpFlags2,
    ...
    if ((bccFlags & BGCHECK_CHECK_FLOOR) && lookup->floor.head != SS_NULL) {
        if (T_Mesh_line_check_proc(&lookup->floor, colCtx, xpFlags1, xpFlags2, posA, posB, outPos, outPoly,
    ...
    if ((bccFlags & BGCHECK_CHECK_WALL) && lookup->wall.head != SS_NULL) {
        if (T_Mesh_line_check_proc(&lookup->wall, colCtx, xpFlags1, xpFlags2, posA, posB, outPos, outPoly,
                                           outDistSq, chkDist, bccFlags)) {
            result = true;
        }
    }
    ...
    if ((bccFlags & BGCHECK_CHECK_CEILING) && lookup->ceiling.head != SS_NULL) {
        if (T_Mesh_line_check_proc(&lookup->ceiling, colCtx, xpFlags1, xpFlags2, posA, posB, outPos, outPoly,
                                           outDistSq, chkDist, bccFlags)) {
            result = true;
        }
    }
 ...
 */
s32 T_Mesh_simple_check_proc(SSNode* node, u16 xpFlags, CollisionContext* colCtx, Vec3f* center, f32 radius,
    ...
    while (true) {
        ...
        if (T_Polygon_simpleCheck(curPoly, vtxList, center, radius)) {
            *outPoly = curPoly;
            return true;
        }
        if (node->next == SS_NULL) {
 ...
 */
s32 T_Mesh_simple_check(StaticLookup* lookup, u16 xpFlags, CollisionContext* colCtx, Vec3f* center, f32 radius,
                                 CollisionPoly** outPoly, u16 bciFlags) {
    if (lookup->floor.head != SS_NULL && !(bciFlags & BGCHECK_IGNORE_FLOOR) &&
        T_Mesh_simple_check_proc(&colCtx->polyNodes.tbl[lookup->floor.head], xpFlags, colCtx, center, radius,
                                         outPoly)) {
        return true;
    }

    if (lookup->wall.head != SS_NULL && !(bciFlags & BGCHECK_IGNORE_WALL) &&
        T_Mesh_simple_check_proc(&colCtx->polyNodes.tbl[lookup->wall.head], xpFlags, colCtx, center, radius,
                                         outPoly)) {
        return true;
    }

    if (lookup->ceiling.head != SS_NULL && !(bciFlags & BGCHECK_IGNORE_CEILING) &&
        T_Mesh_simple_check_proc(&colCtx->polyNodes.tbl[lookup->ceiling.head], xpFlags, colCtx, center, radius,
                                         outPoly)) {
        return true;
    }
 ...
 */
StaticLookup* MeshTableMeshInVertex(CollisionContext* colCtx, StaticLookup* lookupTbl, Vec3f* pos) {
    Vec3i sector;
    s32 subdivAmountX;

    MeshTableCalcWorldPosToMeshPos(colCtx, pos, &sector);
 ...
 */
StaticLookup* MeshTableMeshInVertex_nn(CollisionContext* colCtx, StaticLookup* lookupTbl, Vec3f* pos) {
    Vec3i sector;
    s32 subdivAmountX;

    if (!T_BGCheck_pos_check(colCtx, pos)) {
        return NULL;
    }
    MeshTableCalcWorldPosToMeshPos(colCtx, pos, &sector);
    subdivAmountX = colCtx->subdivAmount.x;
 ...
 */
void MeshTableCalcWorldPosToMeshPosMin(CollisionContext* colCtx, Vec3f* pos, s32* sx, s32* sy, s32* sz) {
 ...
 * `sx`, `sy`, `sz` returns the subdivision x, y, z indices
 */
void MeshTableCalcWorldPosToMeshPosMax(CollisionContext* colCtx, Vec3f* pos, s32* sx, s32* sy, s32* sz) {
    f32 dx = pos->x - colCtx->minBounds.x;
    f32 dy = pos->y - colCtx->minBounds.y;
    f32 dz = pos->z - colCtx->minBounds.z;
 ...
 */
void MeshTablePolygonMinMaxMesh(CollisionContext* colCtx, Vec3s* vtxList, CollisionPoly* polyList,
    ...
    xyz_t_move_s_xyz(&maxVtx, &vtxList[vtxId]);
    xyz_t_move(&minVtx, &maxVtx);
    ...
    MeshTableCalcWorldPosToMeshPosMin(colCtx, &minVtx, subdivMinX, subdivMinY, subdivMinZ);
    MeshTableCalcWorldPosToMeshPosMax(colCtx, &maxVtx, subdivMaxX, subdivMaxY, subdivMaxZ);
}
 ...
 */
s32 MeshTableCheckInsidePolygon(Vec3f* min, Vec3f* max, CollisionPoly* polyList, Vec3s* vtxList, s16 polyId) {
    f32 intersect;
    Vec3f va2;
    Vec3f vb2;
    Vec3f vc2;
    ...
    T_Vertex_ToXyzT(&vtxList[COLPOLY_VTX_INDEX(poly->flags_vIA)], &va);
    flags[0] = Math3DCheckBoxPos_CheckPosition_xyz_t(&va, min, max);
    if (flags[0] == 0) {
        return true;
    }

    T_Vertex_ToXyzT(&vtxList[COLPOLY_VTX_INDEX(poly->flags_vIB)], &vb);
    flags[1] = Math3DCheckBoxPos_CheckPosition_xyz_t(&vb, min, max);
    if (flags[1] == 0) {
        return true;
    }

    T_Vertex_ToXyzT(&vtxList[poly->vIC], &vc);
    flags[2] = Math3DCheckBoxPos_CheckPosition_xyz_t(&vc, min, max);
    if (flags[2] == 0) {
        return true;
    }
    ...
    flags[0] |= Math3DCheckBoxEdgePlane_Bevel2DCheck_xyz_t(&va, min, max) << 8;
    flags[1] |= Math3DCheckBoxEdgePlane_Bevel2DCheck_xyz_t(&vb, min, max) << 8;
    flags[2] |= Math3DCheckBoxEdgePlane_Bevel2DCheck_xyz_t(&vc, min, max) << 8;
    if (flags[0] & flags[1] & flags[2]) {
        return false;
    }

    flags[0] |= Math3DCheckBoxEdgePlane_Bevel3DCheck_xyz_t(&va, min, max) << 0x18;
    flags[1] |= Math3DCheckBoxEdgePlane_Bevel3DCheck_xyz_t(&vb, min, max) << 0x18;
    flags[2] |= Math3DCheckBoxEdgePlane_Bevel3DCheck_xyz_t(&vc, min, max) << 0x18;
    if (flags[0] & flags[1] & flags[2]) {
        return false;
    }
    ...
    if (Math3DTriangleCrossYLine_scope(&va, &vb, &vc, nx, ny, nz, dist, min->z, min->x, &intersect, min->y,
                                           max->y) ||
        Math3DTriangleCrossYLine_scope(&va, &vb, &vc, nx, ny, nz, dist, max->z, min->x, &intersect, min->y,
                                           max->y) ||
        Math3DTriangleCrossYLine_scope(&va, &vb, &vc, nx, ny, nz, dist, min->z, max->x, &intersect, min->y,
                                           max->y) ||
        Math3DTriangleCrossYLine_scope(&va, &vb, &vc, nx, ny, nz, dist, max->z, max->x, &intersect, min->y,
                                           max->y)) {
        return true;
    }
    if (Math3DTriangleCrossZLine_scope(&va, &vb, &vc, nx, ny, nz, dist, min->x, min->y, &intersect, min->z,
                                           max->z) ||
        Math3DTriangleCrossZLine_scope(&va, &vb, &vc, nx, ny, nz, dist, min->x, max->y, &intersect, min->z,
                                           max->z) ||
        Math3DTriangleCrossZLine_scope(&va, &vb, &vc, nx, ny, nz, dist, max->x, min->y, &intersect, min->z,
                                           max->z) ||
        Math3DTriangleCrossZLine_scope(&va, &vb, &vc, nx, ny, nz, dist, max->x, max->y, &intersect, min->z,
                                           max->z)) {
        return true;
    }
    if (Math3DTriangleCrossXLine_scope(&va, &vb, &vc, nx, ny, nz, dist, min->y, min->z, &intersect, min->x,
                                           max->x) ||
        Math3DTriangleCrossXLine_scope(&va, &vb, &vc, nx, ny, nz, dist, min->y, max->z, &intersect, min->x,
                                           max->x) ||
        Math3DTriangleCrossXLine_scope(&va, &vb, &vc, nx, ny, nz, dist, max->y, min->z, &intersect, min->x,
                                           max->x) ||
        Math3DTriangleCrossXLine_scope(&va, &vb, &vc, nx, ny, nz, dist, max->y, max->z, &intersect, min->x,
                                           max->x)) {
        return true;
    }

    T_Vertex_ToXyzT(&vtxList[COLPOLY_VTX_INDEX(poly->flags_vIA)], &va2);
    T_Vertex_ToXyzT(&vtxList[COLPOLY_VTX_INDEX(poly->flags_vIB)], &vb2);
    T_Vertex_ToXyzT(&vtxList[poly->vIC], &vc2);
    if (Math3DCheckBoxCrossLine_xyz_t(min, max, &va2, &vb2) || Math3DCheckBoxCrossLine_xyz_t(min, max, &vb2, &vc2) ||
        Math3DCheckBoxCrossLine_xyz_t(min, max, &vc2, &va2)) {
        return true;
    }
    return false;
}
 ...
 */
u32 MeshTableInit(CollisionContext* colCtx, PlayState* play, StaticLookup* lookupTbl) {
    Vec3s* vtxList;
    CollisionPoly* polyList;
    s32 polyMax;
    s32 polyIdx;
    ...
    for (polyIdx = 0; polyIdx < polyMax; polyIdx++) {
        MeshTablePolygonMinMaxMesh(colCtx, vtxList, polyList, &sxMin, &syMin, &szMin, &sxMax, &syMax, &szMax,
                                         polyIdx);
        ...
        for (sz = szMin; sz < szMax + 1; sz++) {
            ...
            for (sy = syMin; sy < syMax + 1; sy++) {
                ...
                for (sx = sxMin; sx < sxMax + 1; sx++) {
                    if (MeshTableCheckInsidePolygon(&curSubdivMin, &curSubdivMax, polyList, vtxList, polyIdx)) {
                        T_Mesh_add_poly_index(lookup, colCtx, polyList, vtxList, polyIdx);
                    }
...
s32 spot_check(PlayState* play) {
    ...
    for (i = spot_id; i < spot_id + ARRAY_COUNT(spot_id); i++) {
        if (play->sceneId == *i) {
            return true;
        }
    }
 ...
 */
s32 special_memory_size(s32 sceneId, u32* memSize) {
    static BgCheckSceneMemEntry bg_special_size[] = {
    ...
    for (i = 0; i < ARRAY_COUNT(bg_special_size); i++) {
        if (sceneId == bg_special_size[i].sceneId) {
            *memSize = bg_special_size[i].memSize;
            return true;
        }
    }
    return false;
 ...
 */
void T_BGCheck_mesh_calc_proc(f32 min, s32 subdivAmount, f32* max, f32* subdivLength, f32* subdivLengthInv) {
    f32 length = (*max - min);
 ...
 */
void T_BGCheck_ct(CollisionContext* colCtx, PlayState* play, CollisionHeader* colHeader) {
    static BgCheckSceneSubdivisionEntry bgcheck_mesh_num_tbl[] = {
    ...
    if ((R_SCENE_CAM_TYPE == SCENE_CAM_TYPE_FIXED_SHOP_VIEWPOINT) ||
        ...
        if (special_memory_size(play->sceneId, &customMemSize)) {
        ...
        for (i = 0; i < ARRAY_COUNT(bgcheck_mesh_num_tbl); i++) {
            if (play->sceneId == bgcheck_mesh_num_tbl[i].sceneId) {
                colCtx->subdivAmount.x = bgcheck_mesh_num_tbl[i].subdivAmount.x;
                colCtx->subdivAmount.y = bgcheck_mesh_num_tbl[i].subdivAmount.y;
                colCtx->subdivAmount.z = bgcheck_mesh_num_tbl[i].subdivAmount.z;
                useCustomSubdivisions = true;
                customNodeListMax = bgcheck_mesh_num_tbl[i].nodeListMax;
            }
        }
    ...
    T_BGCheck_mesh_calc_proc(colCtx->minBounds.x, colCtx->subdivAmount.x, &colCtx->maxBounds.x,
                                    &colCtx->subdivLength.x, &colCtx->subdivLengthInv.x);
    T_BGCheck_mesh_calc_proc(colCtx->minBounds.y, colCtx->subdivAmount.y, &colCtx->maxBounds.y,
                                    &colCtx->subdivLength.y, &colCtx->subdivLengthInv.y);
    T_BGCheck_mesh_calc_proc(colCtx->minBounds.z, colCtx->subdivAmount.z, &colCtx->maxBounds.z,
    ...
    if (customNodeListMax > 0) {
        ...
        // this is a non-issue as long as bgcheck_mesh_num_tbl.nodeListMax is -1
        tblMax = customNodeListMax;
    } else {
        if (colCtx->memSize < memSize) {
            _dbg_hungup("../z_bgcheck.c", LN1(4227, 4230));
        }
        tblMax = (colCtx->memSize - memSize) / sizeof(SSNode);
    }

    T_BGCheckAlloc_ct(&colCtx->polyNodes);
    T_BGCheckAlloc_Initialize(play, &colCtx->polyNodes, tblMax, colCtx->colHeader->numPolygons);

    lookupTblMemSize = MeshTableInit(colCtx, play, colCtx->lookupTbl);
    PRINTF_COLOR_GREEN();
    PRINTF(T("/*---結局 BG使用サイズ %dbyte---*/\n", "/*---BG size used in the end %dbyte---*/\n"),
           memSize + lookupTblMemSize);
    PRINTF_RST();

    DynaPolyInfo_ct(play, &colCtx->dyna);
    DynaPolyInfo_Init(play, &colCtx->dyna);
}
 ...
 */
CollisionHeader* T_BGCheck_getBGDataInfo(CollisionContext* colCtx, s32 bgId) {
    if (bgId == BGCHECK_SCENE) {
 ...
 */
f32 T_BGCheck_GeneralGroundCheck(PlayState* play, CollisionContext* colCtx, u16 xpFlags, CollisionPoly** outPoly,
    ...
    *outPoly = NULL;
    ...
    while (true) {
        ...
        lookup = MeshTableMeshInVertex_nn(colCtx, lookupTbl, &checkPos);
        if (lookup == NULL) {
            checkPos.y -= colCtx->subdivLength.y;
            continue;
        }
        yIntersect =
            T_Mesh_ground_check(lookup, colCtx, xpFlags, outPoly, pos, downChkFlags, chkDist, BGCHECK_Y_MIN);
    ...
    yIntersectDyna = DynaPolyInfo_groundCheck(&dynaRaycastDown);

    if (yIntersect < yIntersectDyna) {
        yIntersect = yIntersectDyna;
    }

    if (yIntersect != BGCHECK_Y_MIN && T_BGCheck_getGroundUpBit_ai(colCtx, *outPoly, *outBgId)) {
        yIntersect -= 1.0f;
    }
    return yIntersect;
}
 ...
 */
f32 T_BGCheck_CameraGroundCheck(CollisionContext* colCtx, CollisionPoly** outGroundPoly, Vec3f* pos) {
    s32 bgId;

    return T_BGCheck_GeneralGroundCheck(NULL, colCtx, COLPOLY_IGNORE_CAMERA, outGroundPoly, &bgId, pos, NULL,
 ...
 * returns yIntersect of the poly found, or BGCHECK_Y_MIN if no poly detected
 */
f32 T_BGCheck_ObjGroundCheck(CollisionContext* colCtx, CollisionPoly** outGroundPoly, Vec3f* pos) {
    s32 bgId;

    return T_BGCheck_GeneralGroundCheck(NULL, colCtx, COLPOLY_IGNORE_ENTITY, outGroundPoly, &bgId, pos, NULL,
                                   BGCHECK_RAYCAST_DOWN_CHECK_WALLS_SIMPLE | BGCHECK_RAYCAST_DOWN_CHECK_FLOORS |
                                       BGCHECK_RAYCAST_DOWN_CHECK_GROUND_ONLY,
                                   1.0f);
}
 ...
 * returns yIntersect of the poly found, or BGCHECK_Y_MIN if no poly detected
 */
f32 T_BGCheck_ObjGroundCheck2(PlayState* play, CollisionContext* colCtx, CollisionPoly** outGroundPoly, Vec3f* pos) {
    s32 bgId;

    return T_BGCheck_GeneralGroundCheck(play, colCtx, COLPOLY_IGNORE_ENTITY, outGroundPoly, &bgId, pos, NULL,
                                   BGCHECK_RAYCAST_DOWN_CHECK_WALLS_SIMPLE | BGCHECK_RAYCAST_DOWN_CHECK_FLOORS |
                                       BGCHECK_RAYCAST_DOWN_CHECK_GROUND_ONLY,
                                   1.0f);
}
 ...
 * returns yIntersect of the poly found, or BGCHECK_Y_MIN if no poly detected
 */
f32 T_BGCheck_ObjGroundCheck_ai(CollisionContext* colCtx, CollisionPoly** outGroundPoly, s32* bgId, Vec3f* pos) {
    return T_BGCheck_GeneralGroundCheck(NULL, colCtx, COLPOLY_IGNORE_ENTITY, outGroundPoly, bgId, pos, NULL,
                                   BGCHECK_RAYCAST_DOWN_CHECK_WALLS_SIMPLE | BGCHECK_RAYCAST_DOWN_CHECK_FLOORS |
                                       BGCHECK_RAYCAST_DOWN_CHECK_GROUND_ONLY,
                                   1.0f);
 ...
 * returns yIntersect of the poly found, or BGCHECK_Y_MIN if no poly detected
 */
f32 T_BGCheck_ObjGroundCheck_aiac(CollisionContext* colCtx, CollisionPoly** outGroundPoly, s32* bgId, Actor* actor,
                               Vec3f* pos) {
    return T_BGCheck_GeneralGroundCheck(NULL, colCtx, COLPOLY_IGNORE_ENTITY, outGroundPoly, bgId, pos, actor,
                                   BGCHECK_RAYCAST_DOWN_CHECK_WALLS_SIMPLE | BGCHECK_RAYCAST_DOWN_CHECK_FLOORS |
                                       BGCHECK_RAYCAST_DOWN_CHECK_GROUND_ONLY,
                                   1.0f);
}
 ...
 * returns yIntersect of the poly found, or BGCHECK_Y_MIN if no poly detected
 */
f32 T_BGCheck_ObjGroundCheck_aiac2(PlayState* play, CollisionContext* colCtx, CollisionPoly** outGroundPoly, s32* bgId,
                               Actor* actor, Vec3f* pos) {
    return T_BGCheck_GeneralGroundCheck(play, colCtx, COLPOLY_IGNORE_ENTITY, outGroundPoly, bgId, pos, actor,
                                   BGCHECK_RAYCAST_DOWN_CHECK_WALLS_SIMPLE | BGCHECK_RAYCAST_DOWN_CHECK_FLOORS |
                                       BGCHECK_RAYCAST_DOWN_CHECK_GROUND_ONLY,
                                   1.0f);
}
 ...
 * returns yIntersect of the poly found, or BGCHECK_Y_MIN if no poly detected
 */
f32 T_BGCheck_ObjGroundCheck_aiac_skp(CollisionContext* colCtx, CollisionPoly** outGroundPoly, s32* bgId, Actor* actor,
 ...
 * returns yIntersect of the poly found, or BGCHECK_Y_MIN if no poly detected
 */
f32 T_BGCheck_ObjGroundCheck_grpWG_aiac(CollisionContext* colCtx, CollisionPoly** outPoly, s32* bgId, Actor* actor, Vec3f* pos) {
    return T_BGCheck_GeneralGroundCheck(NULL, colCtx, COLPOLY_IGNORE_ENTITY, outPoly, bgId, pos, actor,
 ...
 * returns yIntersect of the poly found, or BGCHECK_Y_MIN if no poly detected
 */
f32 T_BGCheck_GroundCheck(CollisionContext* colCtx, CollisionPoly* outGroundPoly, Vec3f* pos) {
    CollisionPoly* checkResultPoly;
    f32 result;
    s32 bgId;

    result = T_BGCheck_GeneralGroundCheck(NULL, colCtx, COLPOLY_IGNORE_NONE, &checkResultPoly, &bgId, pos, NULL,
                                     BGCHECK_RAYCAST_DOWN_CHECK_WALLS_SIMPLE | BGCHECK_RAYCAST_DOWN_CHECK_FLOORS |
                                         BGCHECK_RAYCAST_DOWN_CHECK_GROUND_ONLY,
                                     1.0f);
 ...
 * returns yIntersect of the poly found, or BGCHECK_Y_MIN if no poly detected
 */
f32 T_BGCheck_GroundCheck_ai(CollisionContext* colCtx, CollisionPoly* outGroundPoly, s32* bgId, Vec3f* pos) {
    CollisionPoly* checkResultPoly;
    f32 result = T_BGCheck_GeneralGroundCheck(NULL, colCtx, COLPOLY_IGNORE_NONE, &checkResultPoly, bgId, pos, NULL,
                                         BGCHECK_RAYCAST_DOWN_CHECK_WALLS_SIMPLE | BGCHECK_RAYCAST_DOWN_CHECK_FLOORS |
                                             BGCHECK_RAYCAST_DOWN_CHECK_GROUND_ONLY,
                                         1.0f);
 ...
 * returns yIntersect of the poly found, or BGCHECK_Y_MIN if no poly detected
 */
f32 T_BGCheck_CameraGroundCheck_grpWG_ai(CollisionContext* colCtx, CollisionPoly** outPoly, s32* bgId, Vec3f* pos) {
    return T_BGCheck_GeneralGroundCheck(NULL, colCtx, COLPOLY_IGNORE_CAMERA, outPoly, bgId, pos, NULL,
                                   BGCHECK_RAYCAST_DOWN_CHECK_WALLS | BGCHECK_RAYCAST_DOWN_CHECK_FLOORS, 1.0f);
}
 ...
 * returns yIntersect of the poly found, or BGCHECK_Y_MIN if no poly detected
 */
f32 T_BGCheck_ObjGroundCheck_grpW_aiac(CollisionContext* colCtx, CollisionPoly** outPoly, s32* bgId, Actor* actor,
                                   Vec3f* pos) {
    return T_BGCheck_GeneralGroundCheck(NULL, colCtx, COLPOLY_IGNORE_ENTITY, outPoly, bgId, pos, actor,
 ...
 * returns yIntersect of the poly found, or BGCHECK_Y_MIN if no poly detected
 */
f32 T_BGCheck_ObjGroundCheck_grpWG_ai(CollisionContext* colCtx, CollisionPoly** outPoly, s32* bgId, Vec3f* pos) {
    return T_BGCheck_GeneralGroundCheck(NULL, colCtx, COLPOLY_IGNORE_ENTITY, outPoly, bgId, pos, NULL,
                                   BGCHECK_RAYCAST_DOWN_CHECK_WALLS | BGCHECK_RAYCAST_DOWN_CHECK_FLOORS, 1.0f);
}
 ...
 */
s32 T_BGCheck_GeneralWallCheck3(CollisionContext* colCtx, u16 xpFlags, Vec3f* posResult, Vec3f* posNext, Vec3f* posPrev,
    ...
    *posResult = *posNext;
    ...
    // if there's movement on the xz planef, and argA flag is 0,
    if ((dx != 0.0f || dz != 0.0f) && (argA & 1) == 0) {
        if ((checkHeight + dy) < 5.0f) {
            //! @bug checkHeight is not applied to posPrev/posNext
            result = T_BGCheck_GeneralLineCheck(colCtx, xpFlags, COLPOLY_IGNORE_NONE, posPrev, posNext, &posIntersect, &poly,
            ...
            // if the radius is less than the distance travelled on the xz planef, also test for floor collisions
            ...
            result = T_BGCheck_GeneralLineCheck(colCtx, xpFlags, COLPOLY_IGNORE_NONE, &checkLinePrev, &checkLineNext,
    ...
    if (DynaPolyInfo_wallCheck(colCtx, xpFlags, &posResult->x, &posResult->z, &sphCenter, radius, outPoly, outBgId,
                              actor)) {
        result = true;
    ...
    if (T_BGCheck_pos_check(colCtx, posNext) == true &&
        ...
        T_Mesh_wall_check(MeshTableMeshInVertex(colCtx, lookupTbl, posResult), colCtx, xpFlags,
    ...
    if (dynaPolyCollision == true || *outBgId != BGCHECK_SCENE) {
        if (T_BGCheck_GeneralLineCheck(colCtx, xpFlags, COLPOLY_IGNORE_NONE, posPrev, posResult, &posIntersect2, &poly,
 ...
 */
s32 T_BGCheck_ObjWallCheck2(CollisionContext* colCtx, Vec3f* posResult, Vec3f* posNext, Vec3f* posPrev, f32 radius,
                             CollisionPoly** outPoly, f32 checkHeight) {
    s32 bgId;

    return T_BGCheck_GeneralWallCheck3(colCtx, COLPOLY_IGNORE_ENTITY, posResult, posNext, posPrev, radius, outPoly, &bgId,
                                 NULL, checkHeight, 0);
}
 ...
 * `outPoly` returns the closest poly detected, while `outBgId` returns the poly owner
 */
s32 T_BGCheck_ObjWallCheck2_ai(CollisionContext* colCtx, Vec3f* posResult, Vec3f* posNext, Vec3f* posPrev, f32 radius,
                             CollisionPoly** outPoly, s32* outBgId, f32 checkHeight) {
    return T_BGCheck_GeneralWallCheck3(colCtx, COLPOLY_IGNORE_ENTITY, posResult, posNext, posPrev, radius, outPoly, outBgId,
                                 NULL, checkHeight, 0);
}
 ...
 * `outPoly` returns the closest poly detected, while `outBgId` returns the poly owner
 */
s32 T_BGCheck_ObjWallCheck2_aiac(CollisionContext* colCtx, Vec3f* posResult, Vec3f* posNext, Vec3f* posPrev, f32 radius,
...
/***
 * Public. Tests if moving from `posPrev` to `posNext` will collide with a "wall"
 * Skips a check that occurs only when moving on the xz planef
 * `radius` is used to form a sphere for collision detection purposes
 * `checkHeight` is the positive height above posNext to perform certain checks
 * `actor` is the actor performing the check, allowing it to be skipped
 * returns true if a collision is detected, else false
 * `outPoly` returns the closest poly detected, while `outBgId` returns the poly owner
 */
s32 T_BGCheck_ObjWallCheck2_aiac_nl(CollisionContext* colCtx, Vec3f* posResult, Vec3f* posNext, Vec3f* posPrev, f32 radius,
                             CollisionPoly** outPoly, s32* outBgId, Actor* actor, f32 checkHeight) {
    return T_BGCheck_GeneralWallCheck3(colCtx, COLPOLY_IGNORE_ENTITY, posResult, posNext, posPrev, radius, outPoly, outBgId,
                                 actor, checkHeight, 1);
}
 ...
 */
s32 T_BGCheck_GeneralRoofCheck(CollisionContext* colCtx, u16 xpFlags, f32* outY, Vec3f* pos, f32 checkHeight,
                             CollisionPoly** outPoly, s32* outBgId, Actor* actor) {
    StaticLookup* lookupTbl;
    StaticLookup* lookup;
    s32 result;
    ...
    *outY = pos->y;
    ...
    lookup = MeshTableMeshInVertex(colCtx, lookupTbl, pos);
    result = T_Mesh_roof_check(lookup, xpFlags, colCtx, outY, pos, checkHeight, outPoly);

    posTemp = *pos;
    posTemp.y = *outY;
    tempY = *outY;

    if (DynaPolyInfo_roofCheck(colCtx, xpFlags, &tempY, &posTemp, checkHeight, outPoly, outBgId, actor)) {
        *outY = tempY;
        result = true;
    }
    return result;
 ...
 */
s32 T_BGCheck_RoofCheck(CollisionContext* colCtx, f32* outY, Vec3f* pos, f32 checkHeight) {
    CollisionPoly* poly;
    s32 bgId;

    return T_BGCheck_GeneralRoofCheck(colCtx, COLPOLY_IGNORE_NONE, outY, pos, checkHeight, &poly, &bgId, NULL);
}
 ...
 * `outY` returns the displaced y coordinate needed to not collide with the poly
 */
s32 T_BGCheck_ObjRoofCheck_aiac(CollisionContext* colCtx, f32* outY, Vec3f* pos, f32 checkHeight,
                               CollisionPoly** outPoly, s32* outBgId, Actor* actor) {
    return T_BGCheck_GeneralRoofCheck(colCtx, COLPOLY_IGNORE_ENTITY, outY, pos, checkHeight, outPoly, outBgId, actor);
}
...
s32 T_BGCheck_GeneralLineCheck(CollisionContext* colCtx, u16 xpFlags1, u16 xpFlags2, Vec3f* posA, Vec3f* posB,
    ...
    *outBgId = BGCHECK_SCENE;
    ...
    T_BGCheckAlloc_clearPolygonCheck(&colCtx->polyNodes, colCtx->colHeader->numPolygons);
    MeshTableCalcWorldPosToMeshPos(colCtx, posA, (Vec3i*)&subdivMin);
    MeshTableCalcWorldPosToMeshPos(colCtx, &posBTemp, (Vec3i*)&subdivMax);
    *posResult = *posB;
    result = false;
    distSq = 1.0e38f;
    *outPoly = NULL;
        ...
        for (i = subdivMin[2]; i < subdivMax[2] + 1; i++) {
            ...
            for (j = subdivMin[1]; j < subdivMax[1] + 1; j++) {
                ...
                for (k = subdivMin[0]; k < subdivMax[0] + 1; k++) {
                    if (Math3DCheckBoxCrossLine_xyz_t(&sectorMin, &sectorMax, posA, &posBTemp) == true &&
                        T_Mesh_line_check_poly_chgrp(lookup, colCtx, xpFlags1, xpFlags2, posA, &posBTemp, posResult,
    ...
    } else if (!T_BGCheck_pos_check(colCtx, posA)) {
        return false;
    } else {
        result =
            T_Mesh_line_check_poly_chgrp(MeshTableMeshInVertex(colCtx, lookupTbl, posA), colCtx, xpFlags1,
                                           xpFlags2, posA, &posBTemp, posResult, outPoly, chkDist, &distSq, bccFlags);
        if (result == true) {
            distSq = Math3DLengthSquare(posResult, posA);
        }
    }
    if ((bccFlags & BGCHECK_CHECK_DYNA) &&
        DynaPolyInfo_lineCheck(colCtx, xpFlags1, posA, &posBTemp, posResult, outPoly, &distSq, outBgId, actor,
                                     chkDist, bccFlags)) {
        result = true;
    }
    return result;
 ...
 * Get bccFlags
 */
u32 T_BGCheck_MakeGLProcMask(s32 chkWall, s32 chkFloor, s32 chkCeil, s32 chkOneFace, s32 chkDyna) {
    u32 result = 0;
 ...
 * returns true if it does, else false
 */
s32 T_BGCheck_CameraLineCheck_poly_chgrp_ai(CollisionContext* colCtx, Vec3f* posA, Vec3f* posB, Vec3f* posResult,
                            CollisionPoly** outPoly, s32 chkWall, s32 chkFloor, s32 chkCeil, s32 chkOneFace,
                            s32* bgId) {
    return T_BGCheck_GeneralLineCheck(colCtx, COLPOLY_IGNORE_CAMERA, COLPOLY_IGNORE_NONE, posA, posB, posResult, outPoly,
                                 bgId, NULL, 1.0f, T_BGCheck_MakeGLProcMask(chkWall, chkFloor, chkCeil, chkOneFace, true));
}
 ...
 * returns true if it does, else false
 */
s32 T_BGCheck_CameraLineCheck_polyThrough_chgrp_ai(CollisionContext* colCtx, Vec3f* posA, Vec3f* posB, Vec3f* posResult,
                            CollisionPoly** outPoly, s32 chkWall, s32 chkFloor, s32 chkCeil, s32 chkOneFace,
                            s32* bgId) {
    return T_BGCheck_GeneralLineCheck(colCtx, COLPOLY_IGNORE_NONE, COLPOLY_IGNORE_CAMERA, posA, posB, posResult, outPoly,
                                 bgId, NULL, 1.0f, T_BGCheck_MakeGLProcMask(chkWall, chkFloor, chkCeil, chkOneFace, true));
}
 ...
 * returns true if it does, else false
 */
s32 T_BGCheck_ObjLineCheck_poly_chgrp_ai(CollisionContext* colCtx, Vec3f* posA, Vec3f* posB, Vec3f* posResult,
                            CollisionPoly** outPoly, s32 chkWall, s32 chkFloor, s32 chkCeil, s32 chkOneFace,
                            s32* bgId) {
    return T_BGCheck_GeneralLineCheck(colCtx, COLPOLY_IGNORE_ENTITY, COLPOLY_IGNORE_NONE, posA, posB, posResult, outPoly,
                                 bgId, NULL, 1.0f, T_BGCheck_MakeGLProcMask(chkWall, chkFloor, chkCeil, chkOneFace, true));
}
 ...
 * returns true if it does, else false
 */
s32 T_BGCheck_ObjLineCheck_poly_chgrp_aiac(CollisionContext* colCtx, Vec3f* posA, Vec3f* posB, Vec3f* posResult,
                            CollisionPoly** outPoly, s32 chkWall, s32 chkFloor, s32 chkCeil, s32 chkOneFace, s32* bgId,
                            Actor* actor) {
    return T_BGCheck_GeneralLineCheck(colCtx, COLPOLY_IGNORE_ENTITY, COLPOLY_IGNORE_NONE, posA, posB, posResult, outPoly,
                                 bgId, actor, 1.0f, T_BGCheck_MakeGLProcMask(chkWall, chkFloor, chkCeil, chkOneFace, true));
}
 ...
 * returns true if it does, else false
 */
s32 T_BGCheck_ObjLineCheck_poly_chgrp_aiac_skp(CollisionContext* colCtx, Vec3f* posA, Vec3f* posB, Vec3f* posResult,
                            CollisionPoly** outPoly, s32 chkWall, s32 chkFloor, s32 chkCeil, s32 chkOneFace, s32* bgId,
                            Actor* actor, f32 chkDist) {
    return T_BGCheck_GeneralLineCheck(colCtx, COLPOLY_IGNORE_ENTITY, COLPOLY_IGNORE_NONE, posA, posB, posResult, outPoly,
                                 bgId, actor, chkDist,
                                 T_BGCheck_MakeGLProcMask(chkWall, chkFloor, chkCeil, chkOneFace, true));
}
 ...
 * returns true if it does, else false
 */
s32 T_BGCheck_ArrowLineCheck_poly_chgrp_ai(CollisionContext* colCtx, Vec3f* posA, Vec3f* posB, Vec3f* posResult,
                               CollisionPoly** outPoly, s32 chkWall, s32 chkFloor, s32 chkCeil, s32 chkOneFace,
                               s32* bgId) {
    return T_BGCheck_GeneralLineCheck(colCtx, COLPOLY_IGNORE_PROJECTILES, COLPOLY_IGNORE_NONE, posA, posB, posResult,
 ...
 * returns true if it does, else false
 */
s32 T_BGCheck_LineCheck_poly(CollisionContext* colCtx, Vec3f* posA, Vec3f* posB, Vec3f* posResult, CollisionPoly** outPoly,
                         s32 chkOneFace) {
    return T_BGCheck_LineCheck_poly_chgrp(colCtx, posA, posB, posResult, outPoly, true, true, true, chkOneFace);
}
 ...
 * returns true if it does, else false
 */
s32 T_BGCheck_LineCheck_poly_chgrp(CollisionContext* colCtx, Vec3f* posA, Vec3f* posB, Vec3f* posResult, CollisionPoly** outPoly,
                         s32 chkWall, s32 chkFloor, s32 chkCeil, s32 chkOneFace) {
    s32 bgId;

    return T_BGCheck_GeneralLineCheck(colCtx, COLPOLY_IGNORE_NONE, COLPOLY_IGNORE_NONE, posA, posB, posResult, outPoly,
                                 &bgId, NULL, 1.0f, T_BGCheck_MakeGLProcMask(chkWall, chkFloor, chkCeil, chkOneFace, true));
}
 ...
 * returns true if it does, else false
 */
s32 T_BGCheck_LineCheck_poly_chgrp_ai(CollisionContext* colCtx, Vec3f* posA, Vec3f* posB, Vec3f* posResult, CollisionPoly** outPoly,
                         s32 chkWall, s32 chkFloor, s32 chkCeil, s32 chkOneFace, s32* bgId) {
    return T_BGCheck_GeneralLineCheck(colCtx, COLPOLY_IGNORE_NONE, COLPOLY_IGNORE_NONE, posA, posB, posResult, outPoly, bgId,
                                 NULL, 1.0f, T_BGCheck_MakeGLProcMask(chkWall, chkFloor, chkCeil, chkOneFace, true));
}
 ...
 */
s32 T_BGCheck_GeneralSimpleCheck(CollisionContext* colCtx, u16 xpFlags, CollisionPoly** outPoly, s32* outBgId,
                               Vec3f* center, f32 radius, Actor* actor, u16 bciFlags) {
    StaticLookup* lookup;
    ...
    *outBgId = BGCHECK_SCENE;
    ...
    lookup = MeshTableMeshInVertex_nn(colCtx, colCtx->lookupTbl, center);
    if (lookup == NULL) {
        return false;
    } else if (T_Mesh_simple_check(lookup, xpFlags, colCtx, center, radius, outPoly, bciFlags) ||
               DynaPolyInfo_simpleCheck(colCtx, xpFlags, outPoly, outBgId, center, radius, actor, bciFlags)) {
        return true;
    }
    return false;
}
 ...
 */
s32 T_BGCheck_SimpleCheck(CollisionContext* colCtx, Vec3f* center, f32 radius) {
    CollisionPoly* poly;
    s32 bgId;

    return T_BGCheck_GeneralSimpleCheck(colCtx, COLPOLY_IGNORE_NONE, &poly, &bgId, center, radius, NULL,
                                      BGCHECK_IGNORE_NONE);
}
 ...
 */
s32 T_BGCheck_SimpleCheck_Wall(CollisionContext* colCtx, Vec3f* center, f32 radius) {
    CollisionPoly* poly;
    s32 bgId;
...
void T_BGCheckAlloc_Initialize(PlayState* play, SSNodeList* this, s32 tblMax, s32 numPolys) {
    this->max = tblMax;
    this->count = 0;
    this->tbl = THA_allocAlign(&play->state.tha, tblMax * sizeof(SSNode), ALIGNOF_MASK(SSNode));
 ...
 */
SSNode* T_BGCheckAlloc_getShortSlistNode(SSNodeList* this) {
 ...
 */
void ShapeSRT_ct(ScaleRotPos* srp) {
 ...
 * Set ScaleRotPos
 */
void ShapeSRT_set(ScaleRotPos* srp, Vec3f* scale, Vec3s* rot, Vec3f* pos) {
    srp->scale = *scale;
    srp->rot = *rot;
    srp->pos = *pos;
}
 ...
 */
s32 ShapeSRT_sameCheck(ScaleRotPos* a, ScaleRotPos* b) {
 ...
 */
void DynaPolyIndexInfo_clearList(DynaLookup* dynaLookup) {
    short_SLIST_ct(&dynaLookup->ceiling);
    short_SLIST_ct(&dynaLookup->wall);
    short_SLIST_ct(&dynaLookup->floor);
}

/**
 * Reset DynaLookup
 */
void DynaPolyIndexInfo_ct(DynaLookup* dynaLookup) {
    dynaLookup->polyStartIndex = 0;
    DynaPolyIndexInfo_clearList(dynaLookup);
}

/**
 * Reset vtxStartIndex
 */
void DynaVertIndexInfo_ct(u16* vtxStartIndex) {
    *vtxStartIndex = 0;
}
 ...
 * Initialize BgActor
 */
void DynaPolyActor_ct(PlayState* play, BgActor* bgActor) {
    bgActor->actor = NULL;
    bgActor->colHeader = NULL;
    ShapeSRT_ct(&bgActor->prevTransform);
    ShapeSRT_ct(&bgActor->curTransform);
    DynaPolyIndexInfo_ct(&bgActor->dynaLookup);
    DynaVertIndexInfo_ct(&bgActor->vtxStartIndex);
 ...
 * setActor internal
 */
void DynaPolyActor_Init(BgActor* bgActor, Actor* actor, CollisionHeader* colHeader) {
    bgActor->actor = actor;
 ...
 */
s32 DynaPolyActor_sameSRTCheck(BgActor* bgActor) {
    return ShapeSRT_sameCheck(&bgActor->prevTransform, &bgActor->curTransform);
}

/**
 * NULL polyList
 */
void DynaPolyTbl_ct(CollisionPoly** polyList) {
    *polyList = NULL;
}
 ...
 */
void DynaPolyTbl_Init(PlayState* play, CollisionPoly** polyList, s32 numPolys) {
    *polyList = THA_allocAlign(&play->state.tha, numPolys * sizeof(CollisionPoly), ALIGNOF_MASK(CollisionPoly));
 ...
 * NULL vtxList
 */
void DynaVertTbl_ct(Vec3s** vtxList) {
    *vtxList = NULL;
}
 ...
 */
void DynaVertTbl_Init(PlayState* play, Vec3s** vtxList, s32 numVtx) {
    *vtxList = THA_allocAlign(&play->state.tha, numVtx * sizeof(Vec3s), ALIGNOF_MASK(Vec3s));
 ...
 */
void DynaPolyActor_cleanUpSRT(PlayState* play, BgActor* bgActor) {
 ...
 * Is BgActor Id
 */
s32 DynaPolyInfo_checkMoveBG(s32 bgId) {
    if (bgId < 0 || bgId >= BG_ACTOR_MAX) {
        return false;
    }
    return true;
...
void DynaPolyInfo_ct(PlayState* play, DynaCollisionContext* dyna) {
    ...
    DynaPolyTbl_ct(&dyna->polyList);
    DynaVertTbl_ct(&dyna->vtxList);
    short_SLIST_TBL_ct(play, &dyna->polyNodes);
}
...
void DynaPolyInfo_Init(PlayState* play, DynaCollisionContext* dyna) {
    ...
    for (i = 0; i < BG_ACTOR_MAX; i++) {
        DynaPolyActor_ct(play, &dyna->bgActors[i]);
        dyna->bgActorFlags[i] = 0;
    }
    DynaPolyTbl_ct(&dyna->polyList);
    DynaPolyTbl_Init(play, &dyna->polyList, dyna->polyListMax);

    DynaVertTbl_ct(&dyna->vtxList);
    DynaVertTbl_Init(play, &dyna->vtxList, dyna->vtxListMax);

    short_SLIST_TBL_ct(play, &dyna->polyNodes);
    short_SLIST_TBL_Init(play, &dyna->polyNodes, dyna->polyNodesMax);
}
 ...
 */
s32 DynaPolyInfo_setActor(PlayState* play, DynaCollisionContext* dyna, Actor* actor, CollisionHeader* colHeader) {
    s32 bgId;
    s32 foundSlot = false;
    ...
    DynaPolyActor_Init(&dyna->bgActors[bgId], actor, colHeader);
    dyna->bitFlag |= DYNAPOLY_INVALIDATE_LOOKUP;
 ...
 */
DynaPolyActor* DynaPolyInfo_actor_index2pointer(CollisionContext* colCtx, s32 bgId) {
    if (!DynaPolyInfo_checkMoveBG(bgId) || !(colCtx->dyna.bgActorFlags[bgId] & BGACTOR_IN_USE) ||
...
void DynaPolygonInfo_setThrough(PlayState* play, DynaCollisionContext* dyna, s32 bgId) {
...
void DynaPolygonInfo_clearThrough(PlayState* play, DynaCollisionContext* dyna, s32 bgId) {
    if (DynaPolyInfo_checkMoveBG(bgId)) {
        dyna->bgActorFlags[bgId] &= ~BGACTOR_COLLISION_DISABLED;
        dyna->bitFlag |= DYNAPOLY_INVALIDATE_LOOKUP;
    }
...
void DynaPolygonInfo_setRoofOff(PlayState* play, DynaCollisionContext* dyna, s32 bgId) {
    if (DynaPolyInfo_checkMoveBG(bgId)) {
...
void DynaPolygonInfo_clearRoofOff(PlayState* play, DynaCollisionContext* dyna, s32 bgId) {
    if (DynaPolyInfo_checkMoveBG(bgId)) {
        dyna->bgActorFlags[bgId] &= ~BGACTOR_CEILING_COLLISION_DISABLED;
        dyna->bitFlag |= DYNAPOLY_INVALIDATE_LOOKUP;
    }
 ...
 */
void DynaPolyInfo_delReserve(PlayState* play, DynaCollisionContext* dyna, s32 bgId) {
    DynaPolyActor* actor;
    ...
    if (!DynaPolyInfo_checkMoveBG(bgId)) {
        ...
        actor = DynaPolyInfo_actor_index2pointer(&play->colCtx, bgId);
        if (actor != NULL) {
...
void DynaPolygonInfo_setExpand(PlayState* play, DynaCollisionContext* dyna) {
    dyna->bitFlag |= DYNAPOLY_INVALIDATE_LOOKUP;
}
 ...
 */
void DynaPolyInfo_expandSRT(PlayState* play, DynaCollisionContext* dyna, s32 bgId, s32* vtxStartIndex,
                                 s32* polyStartIndex) {
    MtxF mtx;
    Actor* actor;
    s32 pad;
    ...
    ShapeSRT_set(&dyna->bgActors[bgId].curTransform, &actor->scale, &actor->shape.rot, &pos);
    ...
    if (!(dyna->bitFlag & DYNAPOLY_INVALIDATE_LOOKUP) &&
        (DynaPolyActor_sameSRTCheck(&dyna->bgActors[bgId]) == true)) {
        s32 pi;
        ...
        for (pi = *polyStartIndex; pi < *polyStartIndex + pbgdata->numPolygons; pi++) {
            ...
            if (normalY > COLPOLY_SNORMAL(0.5f)) {
                ...
                short_SLIST_Prepend_sst(&dyna->polyNodes, &dyna->bgActors[bgId].dynaLookup.floor, &polyIndex);
            } else if (normalY < COLPOLY_SNORMAL(-0.8f)) {
                if (!(dyna->bgActorFlags[bgId] & BGACTOR_CEILING_COLLISION_DISABLED)) {
                    s16 polyIndex = pi;

                    short_SLIST_Prepend_sst(&dyna->polyNodes, &dyna->bgActors[bgId].dynaLookup.ceiling,
                                                 &polyIndex);
                }
            } else {
                s16 polyIndex = pi;

                short_SLIST_Prepend_sst(&dyna->polyNodes, &dyna->bgActors[bgId].dynaLookup.wall, &polyIndex);
            }
        }
    ...
    for (i = 0; i < pbgdata->numVertices; i++) {
        ...
        xyz_t_move_s_xyz(&vtx, &pbgdata->vtxList[i]);
        Skin_Matrix_MulVector(&mtx, &vtx, &vtxT);
        T_Vertex_FromXyzT(&dyna->vtxList[*vtxStartIndex + i], &vtxT);
    ...
    for (i = 0; i < pbgdata->numVertices; i++) {
        ...
        radiusSq = Math3DLengthSquare(&newVtx, &newCenterPoint);
    ...
    for (i = 0; i < pbgdata->numPolygons; i++) {
        ...
        *newPoly = pbgdata->polyList[i];
        ...
        Math3DVectorProductXYZ(&vtxA, &vtxB, &vtxC, &newNormal);
        newNormMagnitude = Math3DVecLength(&newNormal);
        ...
        if (newNormal.y > 0.5f) {
            ...
            short_SLIST_Prepend_sst(&dyna->polyNodes, &dyna->bgActors[bgId].dynaLookup.floor, &polyId);
        } else if (newNormal.y < -0.8f) {
            s16 polyId = *polyStartIndex + i;

            short_SLIST_Prepend_sst(&dyna->polyNodes, &dyna->bgActors[bgId].dynaLookup.ceiling, &polyId);
        } else {
            s16 polyId = *polyStartIndex + i;

            short_SLIST_Prepend_sst(&dyna->polyNodes, &dyna->bgActors[bgId].dynaLookup.wall, &polyId);
        }
    }
...
void DynaPolyInfo_statusClear(PlayState* play, DynaCollisionContext* dyna, Actor* actor) {
    DynaPolyActor* dynaActor;
    s32 i;

    for (i = 0; i < BG_ACTOR_MAX; i++) {
        if (dyna->bgActorFlags[i] & BGACTOR_IN_USE) {
            dynaActor = DynaPolyInfo_actor_index2pointer(&play->colCtx, i);
            if (dynaActor != NULL && &dynaActor->actor == actor) {
                MoveBG_clearStatus((DynaPolyActor*)actor);
                break;
            }
        }
    }
 ...
 */
void DynaPolyInfo_setup(PlayState* play, DynaCollisionContext* dyna) {
    DynaPolyActor* actor;
    s32 vtxStartIndex;
    s32 polyStartIndex;
    s32 i;

    short_SLIST_TBL_cleanUp(&dyna->polyNodes);

    for (i = 0; i < BG_ACTOR_MAX; i++) {
        DynaPolyIndexInfo_clearList(&dyna->bgActors[i].dynaLookup);
    }
    ...
    for (i = 0; i < BG_ACTOR_MAX; i++) {
        ...
        if (dyna->bgActors[i].actor != NULL && dyna->bgActors[i].actor->update == NULL) {
            ...
            PRINTF(T("DynaPolyInfo_setup():削除 index=%d\n", "DynaPolyInfo_setup(): Delete index=%d\n"), i);
            PRINTF_RST();
            actor = DynaPolyInfo_actor_index2pointer(&play->colCtx, i);
            if (actor == NULL) {
                return;
            }
            actor->bgId = BGACTOR_NEG_ONE;
    ...
    for (i = 0; i < BG_ACTOR_MAX; i++) {
        if (dyna->bgActorFlags[i] & BGACTOR_IN_USE) {
            DynaPolyInfo_expandSRT(play, dyna, i, &vtxStartIndex, &polyStartIndex);
        }
    }
    dyna->bitFlag &= ~DYNAPOLY_INVALIDATE_LOOKUP;
}
 ...
 */
void DynaPolyInfo_cleanUp(PlayState* play, DynaCollisionContext* dyna) {
    s32 i;

    for (i = 0; i < BG_ACTOR_MAX; i++) {
        if (dyna->bgActorFlags[i] & BGACTOR_IN_USE) {
            DynaPolyActor_cleanUpSRT(play, &dyna->bgActors[i]);
        }
    }
}
 ...
 */
f32 DynaPolyInfo_groundCheckList(DynaRaycastDown* dynaRaycastDown, u32 listType) {
    CollisionPoly* polyList;
    SSNode* curNode;
    f32 result;
    f32 yIntersect;
    ...
    while (true) {
        ...
        if (T_Polygon_CrossYLine_sukimaR(&polyList[id], dynaRaycastDown->dyna->vtxList, dynaRaycastDown->pos->x,
    ...
    *dynaRaycastDown->bgId = BGCHECK_SCENE;
    ...
    for (i = 0; i < BG_ACTOR_MAX; i++) {
        ...
        if (dynaRaycastDown->actor == dynaRaycastDown->colCtx->dyna.bgActors[i].actor ||
            ...
            !Math3D_sphereCrossXZLine(&dynaRaycastDown->colCtx->dyna.bgActors[i].boundingSphere, dynaRaycastDown->pos->x,
        ...
        if (dynaRaycastDown->downChkFlags & BGCHECK_RAYCAST_DOWN_CHECK_FLOORS) {
            ...
            intersect2 = DynaPolyInfo_groundCheckList(dynaRaycastDown, DYNA_RAYCAST_FLOORS);
        ...
        if ((dynaRaycastDown->downChkFlags & BGCHECK_RAYCAST_DOWN_CHECK_WALLS) ||
            ...
            intersect2 = DynaPolyInfo_groundCheckList(dynaRaycastDown, DYNA_RAYCAST_WALLS);
        ...
        if (dynaRaycastDown->downChkFlags & BGCHECK_RAYCAST_DOWN_CHECK_CEILINGS) {
            ...
            intersect2 = DynaPolyInfo_groundCheckList(dynaRaycastDown, DYNA_RAYCAST_CEILINGS);
    ...
    dynaActor = DynaPolyInfo_actor_index2pointer(dynaRaycastDown->colCtx, *dynaRaycastDown->bgId);
    if ((result != BGCHECK_Y_MIN) && (dynaActor != NULL) && (dynaRaycastDown->play != NULL)) {
        ...
        if (!isPaused && (dynaRaycastDown->colCtx->dyna.bgActorFlags[*dynaRaycastDown->bgId] & BGACTOR_1)) {
            ...
            Skin_Matrix_SetSRzxyTMatrix(&srpMtx, curTransform->scale.x, curTransform->scale.y,
            ...
            for (i2 = 0; i2 < 3; i2++) {
                ...
                xyz_t_move_s_xyz(&vtx, &vtxList[COLPOLY_VTX_INDEX(poly->vtxData[i2])]);
                Skin_Matrix_MulVector(&srpMtx, &vtx, &polyVtx[i2]);
            }

            Math3DVectorProductXYZ(&polyVtx[0], &polyVtx[1], &polyVtx[2], &polyNorm);
            magnitude = Math3DVecLength(&polyNorm);
            ...
            if (!IS_ZERO(magnitude)) {
                ...
                if (Math3DTriangleCrossYLine_sukimaR_VP0(
 ...
 */
s32 DynaPolyInfo_wallCheckList(CollisionContext* colCtx, u16 xpFlags, DynaCollisionContext* dyna, SSList* ssList,
    ...
    while (true) {
        ...
        planeDist = Math3DSignedLengthPlaneAndPos(nx, ny, nz, poly->dist, &resultPos);
        ...
        if (T_Polygon_CrossZLine(poly, dyna->vtxList, resultPos.x, pos->y, &intersect)) {
            f32 zIntersectDist = intersect - resultPos.z;

            if (fabsf(zIntersectDist) <= radius / temp_f18) {
                if (zIntersectDist * nz <= 4.0f) {
                    if (T_BGCheck_positionWallCollection(colCtx, poly, &resultPos.x, &resultPos.z, nx, ny, nz,
                                                        invNormalXZ, planeDist, radius, outPoly)) {
                        *outBgId = bgId;
                    }
                    result = true;
    ...
    while (true) {
        ...
        if (T_Polygon_CrossXLine(poly, dyna->vtxList, pos->y, resultPos.z, &intersect)) {
            f32 xIntersectDist = intersect - resultPos.x;
...
s32 DynaPolyInfo_wallCheck(CollisionContext* colCtx, u16 xpFlags, f32* outX, f32* outZ, Vec3f* pos, f32 radius,
    ...
    for (i = 0; i < BG_ACTOR_MAX; i++) {
        ...
        if (SQ(r) < (SQ(dx) + SQ(dz)) || (!Math3D_sphereCrossXYLine(&bgActor->boundingSphere, resultPos.x, resultPos.y) &&
                                          !Math3D_sphereCrossYZLine(&bgActor->boundingSphere, resultPos.y, resultPos.z))) {
            bgActor->boundingSphere.radius -= (s16)radius;
            continue;
        }
        bgActor->boundingSphere.radius -= (s16)radius;
        if (DynaPolyInfo_wallCheckList(colCtx, xpFlags, &colCtx->dyna,
 ...
 * `outY` returns the y coordinate needed to not collide with `outPoly`
 */
s32 DynaPolyInfo_roofCheckList(CollisionContext* colCtx, u16 xpFlags, DynaCollisionContext* dyna, SSList* ssList,
    ...
    while (true) {
        ...
        T_Polygon_getABC(poly, &nx, &ny, &nz);
        if (checkHeight < Math3DLengthPlaneAndPos(nx, ny, nz, poly->dist, &testPos)) {
            if (curNode->next == SS_NULL) {
                break;
            } else {
                curNode = &dyna->polyNodes.tbl[curNode->next];
        ...
        if (T_Polygon_CrossYLine(poly, dyna->vtxList, testPos.x, testPos.z, &ceilingY)) {
...
s32 DynaPolyInfo_roofCheck(CollisionContext* colCtx, u16 xpFlags, f32* outY, Vec3f* pos, f32 chkDist,
    ...
    for (i = 0; i < BG_ACTOR_MAX; i++) {
        ...
        if (!Math3D_sphereCrossXZLine(&colCtx->dyna.bgActors[i].boundingSphere, pos->x, pos->z)) {
            continue;
        }
        if (DynaPolyInfo_roofCheckList(colCtx, xpFlags, &colCtx->dyna, &colCtx->dyna.bgActors[i].dynaLookup.ceiling,
 ...
 */
s32 DynaPolyInfo_lineCheckShortSList(DynaLineTest* dynaLineTest) {
    f32 distSq;
    s32 result;
    CollisionPoly* curPoly;
    SSNode* curNode;
    ...
    while (true) {
        ...
        if (COLPOLY_VTX_CHECK_FLAGS_ANY(curPoly->flags_vIA, dynaLineTest->xpFlags)) {
            ...
            if (T_Polygon_CrossLine(curPoly, dynaLineTest->dyna->vtxList, dynaLineTest->posA, dynaLineTest->posB,
                                         &polyIntersect, dynaLineTest->chkOneFace, dynaLineTest->chkDist)) {
                distSq = Math3DLengthSquare(dynaLineTest->posA, &polyIntersect);
 ...
 */
s32 DynaPolyInfo_lineCheckList(CollisionContext* colCtx, u16 xpFlags, Vec3f* posA, Vec3f* posB, Vec3f* posResult,
    ...
    if (bccFlags & BGCHECK_CHECK_WALL) {
        if (DynaPolyInfo_lineCheckShortSList(&dynaLineTest)) {
            result = true;
        }
    }
            ...
            if (actor != colCtx->dyna.bgActors[i].actor) {
                ...
                if (!(ay < colCtx->dyna.bgActors[i].minY) || !(by < colCtx->dyna.bgActors[i].minY)) {
                    if (!(colCtx->dyna.bgActors[i].maxY < ay) || !(colCtx->dyna.bgActors[i].maxY < by)) {
                        line.a = *posA;
                        line.b = *posB;
                        if (Math3D_sphereCrossLineSegment(&colCtx->dyna.bgActors[i].boundingSphere, &line) != 0) {
                            if (DynaPolyInfo_lineCheckList(colCtx, xpFlags, posA, posB, posResult, outPoly, distSq,
 ...
 * `outPoly` returns the pointer of the first poly found that intersects
 */
s32 DynaPolyInfo_simpleCheck_loop(CollisionContext* colCtx, u16 xpFlags, CollisionPoly** outPoly, Vec3f* center,
    ...
    while (true) {
        ...
        if (T_Polygon_simpleCheck(curPoly, dyna->vtxList, center, radius)) {
            *outPoly = curPoly;
            return true;
        }
        if (curNode->next == SS_NULL) {
 ...
 * `outPoly` returns the pointer of the first poly found that intersects
 */
s32 DynaPolyInfo_simpleCheck_list(CollisionContext* colCtx, u16 xpFlags, CollisionPoly** outPoly, Vec3f* center,
                                        f32 radius, s32 bgId, u16 bciFlags) {
    if ((bciFlags & BGCHECK_IGNORE_CEILING) == 0) {
        if (DynaPolyInfo_simpleCheck_loop(colCtx, xpFlags, outPoly, center, radius,
...
s32 DynaPolyInfo_simpleCheck(CollisionContext* colCtx, u16 xpFlags, CollisionPoly** outPoly, s32* outBgId,
    ...
    for (i = 0; i < BG_ACTOR_MAX; i++) {
        ...
        if (!Math3D_sphereCrossSphere(&testSphere, &colCtx->dyna.bgActors[i].boundingSphere)) {
            continue;
        }
        if (DynaPolyInfo_simpleCheck_list(colCtx, xpFlags, outPoly, center, radius, i, bciFlags)) {
            return true;
        }
    }
    return false;
 ...
 */
void DynaPolyUty_inBgdi_SG2KSG(CollisionHeader* colHeader) {
 ...
 */
void DynaPolyUty_bgdi_SG2KSG(void* colHeader, CollisionHeader** dest) {
    *dest = SEGMENTED_TO_VIRTUAL(colHeader);
    DynaPolyUty_inBgdi_SG2KSG(*dest);
}

/**
 * SEGMENT_TO_VIRTUAL all active BgActor CollisionHeaders
 */
void DynaPolyInfo_resetBGDataInfo(CollisionContext* colCtx, PlayState* play) {
    ...
    for (i = 0; i < BG_ACTOR_MAX; i++) {
        ...
        if ((flag & BGACTOR_IN_USE) && !(flag & BGACTOR_1)) {
            Actor_set_segment(play, dyna->bgActors[i].actor);
            DynaPolyUty_inBgdi_SG2KSG(dyna->bgActors[i].colHeader);
        }
    }
}
 ...
 */
u32 T_BGCheck_getPolyInfo(CollisionContext* colCtx, CollisionPoly* poly, s32 bgId, s32 dataIdx) {
    CollisionHeader* colHeader;
    SurfaceType* surfaceTypes;

    colHeader = T_BGCheck_getBGDataInfo(colCtx, bgId);
...
u32 T_BGCheck_getCameraSet_ai(CollisionContext* colCtx, CollisionPoly* poly, s32 bgId) {
    return T_BGCheck_getPolyInfo(colCtx, poly, bgId, 0) & 0xFF;
}

/**
 * BgCam get setting of bgCam
 */
u16 T_BGCheck_getCameraType_index_ai(CollisionContext* colCtx, u32 bgCamIndex, s32 bgId) {
    u16 camSetting;
    CollisionHeader* colHeader;
    BgCamInfo* bgCamList;
 ...
 */
u16 T_BGCheck_getCameraType_ai(CollisionContext* colCtx, CollisionPoly* poly, s32 bgId) {
    CollisionHeader* colHeader = T_BGCheck_getBGDataInfo(colCtx, bgId);
    BgCamInfo* bgCamList;
    SurfaceType* surfaceTypes;
    ...
    return T_BGCheck_getCameraType_index_ai(colCtx, T_BGCheck_getCameraSet_ai(colCtx, poly, bgId), bgId);
}

/**
 * BgCam Get the total count of Vec3s data from bgCamFuncData
 */
u16 T_BGCheck_getCameraDataNum_index_ai(CollisionContext* colCtx, u32 bgCamIndex, s32 bgId) {
    CollisionHeader* colHeader = T_BGCheck_getBGDataInfo(colCtx, bgId);
    BgCamInfo* bgCamList;
 ...
 * BgCam Get the total count of Vec3s data from bgCamFuncData
 */
u16 T_BGCheck_getCameraDataNum_ai(CollisionContext* colCtx, CollisionPoly* poly, s32 bgId) {
    CollisionHeader* colHeader = T_BGCheck_getBGDataInfo(colCtx, bgId);
    BgCamInfo* bgCamList;
    SurfaceType* surfaceTypes;
    ...
    return T_BGCheck_getCameraDataNum_index_ai(colCtx, T_BGCheck_getCameraSet_ai(colCtx, poly, bgId), bgId);
}

/**
 * BgCam Get Vec3s data from bgCamFuncData
 */
Vec3s* T_BGCheck_getCameraPos_index_ai(CollisionContext* colCtx, s32 bgCamIndex, s32 bgId) {
    CollisionHeader* colHeader = T_BGCheck_getBGDataInfo(colCtx, bgId);
    BgCamInfo* bgCamList;
 ...
 * BgCam Get Vec3s data from bgCamFuncData
 */
Vec3s* T_BGCheck_getCameraPos_ai(CollisionContext* colCtx, CollisionPoly* poly, s32 bgId) {
    CollisionHeader* colHeader = T_BGCheck_getBGDataInfo(colCtx, bgId);
    BgCamInfo* bgCamList;
    SurfaceType* surfaceTypes;
    ...
    return T_BGCheck_getCameraPos_index_ai(colCtx, T_BGCheck_getCameraSet_ai(colCtx, poly, bgId), bgId);
}

u32 T_BGCheck_getSceneChangeCode_ai(CollisionContext* colCtx, CollisionPoly* poly, s32 bgId) {
    return T_BGCheck_getPolyInfo(colCtx, poly, bgId, 0) >> 8 & 0x1F;
}

u32 T_BGCheck_getAttributeCode_ai(CollisionContext* colCtx, CollisionPoly* poly, s32 bgId) {
    return T_BGCheck_getPolyInfo(colCtx, poly, bgId, 0) >> 13 & 0x1F;
}

/**
 * SurfaceType Get ? Property (& 0x001C 0000)
 */
u32 T_BGCheck_getRoofCode_ai(CollisionContext* colCtx, CollisionPoly* poly, s32 bgId) {
    return T_BGCheck_getPolyInfo(colCtx, poly, bgId, 0) >> 18 & 7;
}

u32 T_BGCheck_getWallCode_ai(CollisionContext* colCtx, CollisionPoly* poly, s32 bgId) {
    return T_BGCheck_getPolyInfo(colCtx, poly, bgId, 0) >> 21 & 0x1F;
}

s32 T_BGCheck_getWallCodeBit_ai(CollisionContext* colCtx, CollisionPoly* poly, s32 bgId) {
    return Bg_Wall_Code[T_BGCheck_getWallCode_ai(colCtx, poly, bgId)];
}

s32 T_BGCheck_WallNotClib_ai(CollisionContext* colCtx, CollisionPoly* poly, s32 bgId) {
    return (T_BGCheck_getWallCodeBit_ai(colCtx, poly, bgId) & WALL_FLAG_0) ? true : false;
}

s32 T_BGCheck_WallLadder_ai(CollisionContext* colCtx, CollisionPoly* poly, s32 bgId) {
    return (T_BGCheck_getWallCodeBit_ai(colCtx, poly, bgId) & WALL_FLAG_1) ? true : false;
}

s32 T_BGCheck_WallLadderUp_ai(CollisionContext* colCtx, CollisionPoly* poly, s32 bgId) {
    return (T_BGCheck_getWallCodeBit_ai(colCtx, poly, bgId) & WALL_FLAG_2) ? true : false;
}

u32 T_BGCheck_getGroundCode_ai(CollisionContext* colCtx, CollisionPoly* poly, s32 bgId) {
    return T_BGCheck_getPolyInfo(colCtx, poly, bgId, 0) >> 26 & 0xF;
}

u32 T_BGCheck_GroundAutoJumpType_ai(CollisionContext* colCtx, CollisionPoly* poly, s32 bgId) {
    return T_BGCheck_getPolyInfo(colCtx, poly, bgId, 0) >> 26 & 0xF;
}

u32 T_BGCheck_getGroundUpBit_ai(CollisionContext* colCtx, CollisionPoly* poly, s32 bgId) {
    return T_BGCheck_getPolyInfo(colCtx, poly, bgId, 0) >> 30 & 1;
}

u32 T_BGCheck_getHorseKeepOutBit_ai(CollisionContext* colCtx, CollisionPoly* poly, s32 bgId) {
    return T_BGCheck_getPolyInfo(colCtx, poly, bgId, 0) >> 31 & 1;
}

u32 T_BGCheck_getTypeSoundCode_ai(CollisionContext* colCtx, CollisionPoly* poly, s32 bgId) {
    return T_BGCheck_getPolyInfo(colCtx, poly, bgId, 1) & 0xF;
}

u16 T_BGCheck_getSoundGroundLabel(CollisionContext* colCtx, CollisionPoly* poly, s32 bgId) {
    s32 surfaceMaterial = T_BGCheck_getTypeSoundCode_ai(colCtx, poly, bgId);

    if ((surfaceMaterial < 0) || (surfaceMaterial >= ARRAY_COUNT(Bg_PolygonBgSNDCode2SoundGroundLabel))) {
        return SURFACE_SFX_OFFSET_DIRT;
    }
    return Bg_PolygonBgSNDCode2SoundGroundLabel[surfaceMaterial];
}

u32 T_BGCheck_getFriction_ai(CollisionContext* colCtx, CollisionPoly* poly, s32 bgId) {
    return T_BGCheck_getPolyInfo(colCtx, poly, bgId, 1) >> 4 & 3;
}

u32 T_BGCheck_getKankyo_ai(CollisionContext* colCtx, CollisionPoly* poly, s32 bgId) {
    return T_BGCheck_getPolyInfo(colCtx, poly, bgId, 1) >> 6 & 0x1F;
}

u32 T_BGCheck_getEcho_ai(CollisionContext* colCtx, CollisionPoly* poly, s32 bgId) {
    return T_BGCheck_getPolyInfo(colCtx, poly, bgId, 1) >> 11 & 0x3F;
}

u32 T_BGCheck_checkHookShotStick(CollisionContext* colCtx, CollisionPoly* poly, s32 bgId) {
    return T_BGCheck_getPolyInfo(colCtx, poly, bgId, 1) >> 17 & 1;
}
 ...
 */
s32 T_BGCheck_CheckObjNoHit(CollisionContext* colCtx, CollisionPoly* poly, s32 bgId) {
    u32 flags;

    if (T_BGCheck_getBGDataInfo(colCtx, bgId) == NULL) {
        return true;
    }
 ...
 */
s32 T_BGCheck_CheckArrowNoHit(CollisionContext* colCtx, CollisionPoly* poly, s32 bgId) {
    u32 flags;
 ...
 */
s32 T_BGCheck_checkBelt(CollisionContext* colCtx, CollisionPoly* poly, s32 bgId) {
    u32 flags;
...
u32 T_BGCheck_getSlidePowerIndex(CollisionContext* colCtx, CollisionPoly* poly, s32 bgId) {
    return T_BGCheck_getPolyInfo(colCtx, poly, bgId, 1) >> 18 & 7;
}

/**
 * returns a value between 0-63, representing 360 / 64 degrees of rotation
 */
u32 T_BGCheck_getSlideAngle(CollisionContext* colCtx, CollisionPoly* poly, s32 bgId) {
    return T_BGCheck_getPolyInfo(colCtx, poly, bgId, 1) >> 21 & 0x3F;
}

u32 T_BGCheck_CheckDamagePoly(CollisionContext* colCtx, CollisionPoly* poly, s32 bgId) {
    return (T_BGCheck_getPolyInfo(colCtx, poly, bgId, 1) & 0x08000000) ? 1 : 0;
}

/**
 * Zora's Domain WaterBox in King Zora's Room
 */
WaterBox add_water = { -348, 877, -1746, 553, 780, 0x2104 };
 ...
 */
s32 T_BGCheck_WaterSurfaceCheck3(PlayState* play, CollisionContext* colCtx, f32 x, f32 z, f32* ySurface,
                         WaterBox** outWaterBox) {
    if (play->sceneId == SCENE_ZORAS_DOMAIN) {
        s32 pad;

        if (min_pos < x && x < max_pos && sZorasDomainWaterBoxMinY < *ySurface &&
            *ySurface < sZorasDomainWaterBoxMaxY && sZorasDomainWaterBoxMinZ < z && z < sZorasDomainWaterBoxMaxZ) {
            *outWaterBox = &add_water;
            *ySurface = add_water.ySurface;
            return true;
        }
    }
    return T_BGCheck_WaterSurfaceCheck2(play, colCtx, x, z, ySurface, outWaterBox);
}
 ...
 * `ySurface` returns the water box's surface, while `outWaterBox` returns a pointer to the WaterBox
 */
s32 T_BGCheck_WaterSurfaceCheck2(PlayState* play, CollisionContext* colCtx, f32 x, f32 z, f32* ySurface,
                            WaterBox** outWaterBox) {
    CollisionHeader* colHeader = colCtx->colHeader;
    s32 room;
    WaterBox* waterBox;
 ...
 */
s32 T_BGCheck_WaterGetIndex(PlayState* play, CollisionContext* colCtx, Vec3f* pos, f32 surfaceChkDist,
                         WaterBox** outWaterBox) {
    CollisionHeader* colHeader = colCtx->colHeader;
    s32 room;
    s32 i;
 ...
 */
u32 T_BGCheck_WaterGetCameraSet(CollisionContext* colCtx, WaterBox* waterBox) {
 ...
 */
u16 T_BGCheck_WaterGetCameraType(CollisionContext* colCtx, WaterBox* waterBox) {
    s32 bgCamIndex = T_BGCheck_WaterGetCameraSet(colCtx, waterBox);
 ...
 */
u32 T_BGCheck_WaterGetKankyo(CollisionContext* colCtx, WaterBox* waterBox) {
 ...
 * same as T_BGCheck_WaterSurfaceCheck2, but tests if WATERBOX_FLAG_19 is set
 * returns true if point is within the xz boundaries of an active water box, else false
 * `ySurface` returns the water box's surface, while `outWaterBox` returns a pointer to the WaterBox
 */
s32 T_BGCheck_WaterKankyoSurfaceCheck2(PlayState* play, CollisionContext* colCtx, f32 x, f32 z, f32* ySurface, WaterBox** outWaterBox) {
    CollisionHeader* colHeader = colCtx->colHeader;
    s32 room;
    WaterBox* waterBox;
 ...
 */
s32 T_BGCheck_2PlaneLinePosNearPos(CollisionPoly* polyA, CollisionPoly* polyB, Vec3f* point, Vec3f* closestPoint) {
    f32 n1X;
    f32 n1Y;
    f32 n1Z;
    f32 n2X;
    ...
    T_Polygon_getABC(polyA, &n1X, &n1Y, &n1Z);
    T_Polygon_getABC(polyB, &n2X, &n2Y, &n2Z);
    return Math3D2PlaneLinePosNearPos(n1X, n1Y, n1Z, polyA->dist, n2X, n2Y, n2Z, polyB->dist, point,
                                                 closestPoint);
}
 ...
 * returns true if the `closestPoint` exists, else returns false
 */
s32 T_BGCheck_2PlaneLine_LineNearPos(CollisionPoly* polyA, CollisionPoly* polyB, Vec3f* pointA, Vec3f* pointB, Vec3f* closestPoint) {
    f32 n1X;
    f32 n1Y;
    f32 n1Z;
    f32 n2X;
    ...
    T_Polygon_getABC(polyA, &n1X, &n1Y, &n1Z);
    T_Polygon_getABC(polyB, &n2X, &n2Y, &n2Z);
    result = Math3D_2PlaneLine_LineNearPos(n1X, n1Y, n1Z, polyA->dist, n2X, n2Y, n2Z, polyB->dist, pointA, pointB,
                                               closestPoint);
    return result;
}
...
void BgCheck_DrawDynaPolyList(PlayState* play, CollisionContext* colCtx, DynaCollisionContext* dyna, SSList* ssList,
    ...
    if (ssList->head != SS_NULL) {
        ...
        while (true) {
            ...
            T_Vertex_ToXyzT(COLPOLY_VTX_INDEX(poly->flags_vIA) + dyna->vtxList, &vA);
            T_Vertex_ToXyzT(COLPOLY_VTX_INDEX(poly->flags_vIB) + dyna->vtxList, &vB);
            T_Vertex_ToXyzT((s32)(poly->vIC) + dyna->vtxList, &vC);
            if (AREG(26)) {
                nx = COLPOLY_GET_NORMAL(poly->normal.x);
                ny = COLPOLY_GET_NORMAL(poly->normal.y);
                nz = COLPOLY_GET_NORMAL(poly->normal.z);
...
void BgCheck_DrawStaticPoly(PlayState* play, CollisionContext* colCtx, CollisionPoly* poly, u8 r, u8 g, u8 b) {
    ...
    T_Vertex_ToXyzT(COLPOLY_VTX_INDEX(poly->flags_vIA) + colCtx->colHeader->vtxList, &vA);
    T_Vertex_ToXyzT(COLPOLY_VTX_INDEX(poly->flags_vIB) + colCtx->colHeader->vtxList, &vB);
    T_Vertex_ToXyzT(poly->vIC + colCtx->colHeader->vtxList, &vC);
    if (AREG(26) != 0) {
        nx = COLPOLY_GET_NORMAL(poly->normal.x);
        ny = COLPOLY_GET_NORMAL(poly->normal.y);
        nz = COLPOLY_GET_NORMAL(poly->normal.z);
...
void BgCheck_DrawStaticCollision(PlayState* play, CollisionContext* colCtx) {
    ...
    StaticLookup* lookup = MeshTableMeshInVertex(colCtx, colCtx->lookupTbl, &player->actor.world.pos);
