...
#define IS_SEQUENCE_CHANNEL_VALID(ptr) ((u32)(ptr) != (u32)&AG.sequenceChannelNone)
...
typedef struct NotePlaybackState {
    /* 0x00 */ u8 priority;
    /* 0x01 */ u8 waveId;
    /* 0x02 */ u8 harmonicIndex; // the harmonic index for the synthetic wave contained in WAVEMEM_TABLE (also matches the base 2 logarithm of the harmonic order)
    /* 0x03 */ u8 fontId;
    /* 0x04 */ u8 unk_04;
...
typedef struct AudioHeapInitSizes {
    /* 0x00 */ u32 heapSize; // total number of bytes allocated to the audio heap. Must be <= the size of `AUDIOHP` (ideally about the same size)
...
s32 osAiSetNextBuffer2(void*, u32);
...
void Na_AudioPrint(struct GfxPrint* printer);
...
void Na_StartInputPortSe(Vec3f* pos, u16 sfxId, u8);
void Na_SetSariaMelodyTag2(Vec3f* pos, f32);
...
void Na_SetDirectFir(u8);
void Na_SetWaterSeModeFlag(u8);
void Na_SetMessageStatusFlag(s8 flag);
void Na_SwapSeFlagOnDemo(u16 sfxId);
void Na_StopAllSound(u16);
