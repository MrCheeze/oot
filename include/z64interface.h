...
void button_status_set(struct PlayState* play);
void equip_default_set(void);
void yabusame_game_start(struct PlayState* play);
void sword_system_dma(struct PlayState* play);
void item_textuer_dma(struct PlayState* play, u16 button);
void item_textuer_dma1(struct PlayState* play, u16 button);
void a_item_dma_change(struct PlayState* play, u16 flag);
u8 item_get_setting(struct PlayState* play, u8 item);
u8 item_get_non_setting(u8 item);
void item_clear_setting(u16 item, u16 invSlot);
s32 item_change_setting(struct PlayState* play, u16 oldItem, u16 newItem);
s32 findEmptyBottle(void);
s32 bottle_interior_check(u8 bottleItem);
void bottle_getting(struct PlayState* play, u8 item, u8 button);
s32 bottle_fairy_use(struct PlayState* play);
void do_action_point_set(struct PlayState* play, u16 action);
void do_action_navi_set(struct PlayState* play, u16 naviCallState);
void sp_action_set(struct PlayState* play, u16 action);
s32 life_meter_play(struct PlayState* play, s16 amount);
void lupy_increase(s16 rupeeChange);
void item_count_vary(s16 item, s16 ammoChange);
void magic_mode_check(struct PlayState* play);
void magic_cancel_check(struct PlayState* play);
s32 magic_meter_check(struct PlayState* play, s16 amount, s16 type);
void total_event_timer_set(s16 seconds);
void total_event_timer_end(struct PlayState* play);
void event_timer_set(s16 seconds);
void parameter_draw(struct PlayState* play);
void parameter_move(struct PlayState* play);
void parameter_dt(struct PlayState* play);
void parameter_ct(struct PlayState* play);
